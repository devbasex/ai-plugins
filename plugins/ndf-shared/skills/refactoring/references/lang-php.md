...
**PHP 標準に一括演算の基盤はない。** `array_map` は逐次実行の高階反復であり、「反復の実行方式」
表のどの行にも移動しない。`array_column` はコールバックを取らない組み込みの列抽出で、走査は逐次
のままだが PHP レベルのループとは実装が異なるため、**計測して選ぶ**。数値計算向けの拡張や
ライブラリを導入すれば一括演算に移れる場合はあるので、採用するなら**効果を計測してから決める**。
