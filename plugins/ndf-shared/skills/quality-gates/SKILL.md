...
| モード | 必須の段階 | 追加で要るもの |
| --- | --- | --- |
| `light` | 1 と 3 | 変更箇所が読み込まれる経路を 1 度実行する |
...
| 変更前から失敗していた | 変更前でも失敗することを示し、範囲外として明記する |
...
| 段階 | コマンド | 結果 |
| --- | --- | --- |
| 限定的な検証 | `pytest -k orders` | 12 passed / exit=0 |
| 全体テスト | `pytest` | 318 passed, 2 skipped / exit=0 |
| 静的解析 | `ruff check .` | exit=0 |
| 型検査 | `mypy src` | exit=0 |
...
未検証・未実行がある場合は、この表の下に必ず列挙する。**空欄にして触れないことを禁じる。**
