...
def _check_early_error(
    ...
    """err.log (claude は stdout.log も) に致命パターンがあれば EARLY_ERROR を返す。

    明確な致命 (FATAL) のみ kill する。曖昧パターン (生 Error: / Traceback) は
    この関数では扱わない（呼び出し側で WARN として警告ログのみ出す）。
    """
...
def _check_result_age_fallback(
    pid: int,
    paths: AgentPaths,
    status: AgentStatus,
    started_wall: float,
    ...
    """result.json の age fallback で完了判定する。

    sentinel 機構を持たない agent (gemini) 向け。result.json が書かれた後も
    プロセスがハングするケース (MCP サーバー切断待ち等で exit しない) の
    fallback: result.json の mtime が RESULT_AGE_GRACE 秒以上前であれば
    ...
    """
    if status.sentinel_seen:
        return None
    if not cmdline_validated:
        return None
    if not (paths.result.exists() and paths.result.stat().st_size > 0):
        return None
...
def _check_codex_sentinel_complete(
    pid: int,
    paths: AgentPaths,
    status: AgentStatus,
) -> Optional[AgentStatus]:
    """codex sentinel + result.json が揃っていれば OK を返す。
...
def monitor_agent(
    ...
    while True:
        ...
        if alive:
            result = _check_codex_sentinel_complete(pid, paths, status)
            if result is not None:
                _emit_log(log_prefix, agent, result)
                return result

            result = _check_result_age_fallback(
        ...
        if not no_early_error:
            result = _check_early_error(pid, paths, status, agent, alive)
            if result is not None:
                _emit_log(log_prefix, agent, result)
                return result
