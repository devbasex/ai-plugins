...
def _check_codex_sentinel_done(
    agent: str,
    pid: int,
    paths: AgentPaths,
    status: AgentStatus,
...
def monitor_agent(
    ...
    while True:
        ...
        if done_status := _check_codex_sentinel_done(
            agent, pid, paths, status, alive, log_prefix
        ):
            return done_status
        ...
        done_status, warned_early_error = _check_early_error(
            agent, pid, paths, status, alive, no_early_error, warned_early_error, log_prefix
        )
        if done_status:
            return done_status
