...
def check_early_error(
    ...
    """early error の fatal/warn を判定し、fatal 時は終了 status を返す。"""
...
def update_progress_and_check_stall(
    agent: str,
    paths: AgentPaths,
    status: AgentStatus,
    pid: int,
    ...
    """ログ進捗を更新し、stall_timeout 超過なら終了 status を返す。"""
...
def monitor_agent(
    ...
    while True:
        ...
        if not no_early_error:
            early_status, warned_early_error = check_early_error(
                agent, paths, status, pid, alive, warned_early_error, log_prefix,
            )
        ...
        stalled, last_progress_size, last_progress = update_progress_and_check_stall(
            agent, paths, status, pid, alive, stall_timeout,
