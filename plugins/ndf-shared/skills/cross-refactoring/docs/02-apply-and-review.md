...
| 検証 | 方法 |
| --- | --- |
| 着手前にテストが成功している | `baseline_test` が `green` でなければ適用に着手せず全項目を `blocked` にする。`red` だけでなく「確認していない」も通さない |
...
テストの成功を要求する。満たさないコミットは取り込まず、**その改善項目の指摘を
解決済みにしない**。適用側だけ厳しくして修正側を素通しにすると、「レビュー指摘への
