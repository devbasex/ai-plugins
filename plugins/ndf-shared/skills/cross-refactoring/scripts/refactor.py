    ...
    for item_id in entry["items"]:
        ...
        if got is None:
            ...
            facts = collect_commit_facts(
                state["worktrees"]["work"], _reported_shas(got), in_range,
                test_command, head_branch, timeout,
            )
...
def _reconcile_resolved_threads(
    payload: dict[str, Any],
    repo: str,
    pr: int,
) -> set[str]:
    """修正結果の解決申告を GitHub の実状態と突き合わせる。"""
...
def _build_review_record(
    ...
    """LLM レビュー結果から状態ファイル用の正規化 record を作る。"""
...
def cmd_merge_fix(args: argparse.Namespace) -> None:
    ...
    resolved = _reconcile_resolved_threads(payload, state["repo"], state["current_pr"])
