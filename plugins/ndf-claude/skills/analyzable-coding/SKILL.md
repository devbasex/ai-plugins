...
| 誤用 | なぜ悪化するか |
| --- | --- |
| 条件分岐を機械的に対応表へ置き換える | 単純なガード節は対応表より意図が明確。網羅性を静的に検査できていた分岐を対応表へ移すと、検査が弱くなる（型情報を伴わずに実行時ロードする対応表は、静的解析の視界から外れる） |
...
**型情報を伴わずに実行時ロードすると、その対応表は静的解析の視界から外れる**（実測は
[references/language-notes.md](references/language-notes.md)）。ただし外部化と静的検査は排他で
はない。**データを実行時にロードするか否か**で分ける。

1. **データごとビルド時に組み込める**なら、スキーマから型・定数を生成して取り込む。外部化しても
   静的検査が残るので、これが選べるなら最初に選ぶ
2. **データを実行時にロードする**なら、**ロード境界をスキーマ検証で守る**。型を生成していても
   同じで、**型生成は実行時のスキーマ検証の代わりにならない**（生成した型は、ロードした値が
   その形である保証を与えない）
