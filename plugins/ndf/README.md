...
# => NDF統合開発エージェント（Kiro CLI用 / v10.17.4）
```
...
あわせて、`.md` の文言を照合するテストを削り、書かない方針をテストの規約へ入れました（#885）。
Skill の数は変わりません。引数・Skill・スクリプトの削除や改名は無く、状態ファイルの移行も要りません。
変更点の一覧は [CHANGELOG.md](../../CHANGELOG.md) にあります。

**正式版です。** `main` に載ります。中身は開発版 `10.17.4-dev.1` と同じで、版数の接尾辞だけを
外しました。
...
| **`.md` の文言を照合するテストを書かない規約にしました**（#885） | `tdd-cycle/references/test-quality.md` の例 8 と `quality-gates` の「受け入れ条件との対応」に、文書の言い回し・見出し・表の並びを `assert "..." in text` で照合するテストを書かず、受け入れ条件の検証手段にもしないと足しました。参照切れは検査スクリプト、正しさはレビュー、埋め込んだ bash は取り出して実行するテストで見ます。配布物のテストからも該当の照合を削りました（全体テストは 5365 件 → 4641 件） |
...
反映されない**ため、更新したあとは起動し直してください。alias は次に開いたシェルから効きます。開発版を試すために
`develop` を登録した場合は、
...
どれもファイルを読むだけで、課題もファイルも書き換えません。`$SCRIPTS` はプラグインの
`scripts/` の絶対パスで、決め方は
[development-workflow/references/scripts-lookup.md](skills/development-workflow/references/scripts-lookup.md)
にあります。

```bash
grep -q '"version": "10.17.4"' "$SCRIPTS/../.claude-plugin/plugin.json"; echo "exit=$?"   # 0 なら この版が入っている
...
grep -qF '### 8. `.md` の文言を照合する' "$SCRIPTS/../skills/tdd-cycle/references/test-quality.md"; echo "exit=$?"   # 0 なら 文言照合を書かない規約が入っている
```
...
```text
# 動く: 実体パスを示して読ませる
~/.codex/plugins/cache/ai-plugins/ndf/10.17.4/skills/deploy/SKILL.md を読んで、その手順どおりに qa/staging へ deploy PR を作成してください。
...
# ~/.codex/plugins/cache/ai-plugins/ndf/10.17.4/skills/deploy/SKILL.md
```
...
# => ndf@ai-plugins  installed, enabled  10.17.4  <path>
```
