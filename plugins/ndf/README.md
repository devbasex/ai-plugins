...
# => NDF統合開発エージェント（Kiro CLI用 / v10.0.0-dev.1）
```
...
受け付けなくなり、`gemini` CLI しか持たない環境では収束ループが動きません。Google が
Gemini CLI を Antigravity CLI へ移す方針を告知したことによる、計画された移行です
（[#214](https://github.com/devbasex/ai-plugins/issues/214)）。

**配布先が 1 つ増えます（agy 向け 31 個）。** 既存の 3 つの公開 Skill の数は変わりません
（Claude Code 33 / Codex 31 / Kiro 32）。agy 向けの定義は `plugins/ndf/dev.agy/` にあり、
`agy plugin install` で導入します（[#215](https://github.com/devbasex/ai-plugins/issues/215)）。

**工程の進み方に 2 つの手当てが入ります。** 必須の工程を記録しないまま配布へ進むと、記録の
無い工程が案内に出ます（[#221](https://github.com/devbasex/ai-plugins/issues/221)）。設計
Pull Request のマージは、`design-approved` ラベルが無いと止まります
（[#266](https://github.com/devbasex/ai-plugins/issues/266)）。**このラベルをリポジトリへ
定義するまで、承認のゲートは働きません。** 定義そのものが有効化の宣言を兼ねます。

```bash
gh label create design-approved --description "設計レビューを人間が承認した" --color 0e8a16
```

**振り返りの記録先が変わります。** `docs/development-history/` へファイルを新しく作るのを
やめ、起点の issue か配布した Pull Request へのコメント 1 件にします。既存の記録は残ります
（[#242](https://github.com/devbasex/ai-plugins/issues/242)）。あわせて、範囲外の課題を
どのリポジトリへ起票するかの判断が `out-of-scope` に入りました
（[#229](https://github.com/devbasex/ai-plugins/issues/229)）。

**配布の完了を確かめる手順が `release` に入ります。** ログの文字列を推測して待つのではなく、
配布先の状態が変わったことで確かめます（[#228](https://github.com/devbasex/ai-plugins/issues/228)）。

**`worktree` の手順が動くようになります。** 定義の無い `$NDF_SCRIPTS` を参照していた 20 行を、
解決手順を持つ `$SCRIPTS` へ揃えました（[#193](https://github.com/devbasex/ai-plugins/issues/193)）。
...
### `cross-review` が、投稿の届いていないレビューを収束へ数えなくなりました

レビュアーが投稿に失敗すると、**判定だけが残り、指摘の中身が Pull Request に無いまま修正の
工程が起動していました**。修正の担当は Pull Request のコメントを読んで直すため、指摘が無ければ
直すものが無く、ラウンドだけが 1 つ増えます。投稿が失敗し続ける環境では、上限に達するまで
空のラウンドを繰り返します。

結果ファイルに投稿の失敗が記録されているとき、投稿先の参照が空のとき、参照の指すレビューが
GitHub 側に無いときを「結果なし」として扱い、**同じラウンドで 1 度だけ起動し直します**。
GitHub 側へ問い合わせできないときは申告を採用し、確認できなかったことを出力へ残します。
...
本文が同じ）で、いずれか 1 つで結びつけば同じ箇所として数えます。閾値は 0.5 のままです。
出力に内訳が出ます。

```text
振動検知: overlap=2/3 (67%) 位置=1 近傍=1 本文=0
```

### `cross-review` が、巻き直しの後も正しいブランチへ揃えます

巻き直しは `<枝名>-r<時刻>` という新しいブランチを作りますが、状態ファイルの枝名は最初の値の
ままでした。**巻き直しの直後に再開すると、巻き直し前のブランチへ作業ツリーを合わせようと
します。** 巻き直しの側で枝名を書き戻すようにしました。

### `cross-review` の手順書に余白ができました

`SKILL.md` が 500 行の上限に張り付いており、判定に関わらない説明を削らないと 1 行も足せない
状態でした。実行の途中で読まなくてよい 4 節（レビュー出力の制約・継続的統合の失敗の分類・
アンチパターン・`monitor.py` の誤検知）を [docs/03-review-output.md](skills/cross-review/docs/03-review-output.md)
へ移し、391 行にしています。**内容は変わりません。**

### 後片付けが、閉じ忘れた issue を拾います

GitHub の自動クローズは**既定ブランチへマージしたときにだけ**働きます。開発の起点が既定
ブランチでないリポジトリでは、本文に `Closes #<番号>` と書いてもマージで閉じません。完了した
課題が課題の一覧に残り、次に着手する担当が完了済みの課題を拾います。

`/ndf:merged` の手順に「閉じ忘れた issue を閉じる」を足しました。マージした Pull Request の
本文から閉じる語が指す番号を取り出し、**まだ OPEN のものだけ**を一覧で示して、同意を得てから
閉じます。**閉じる語は番号ごとに要ります**（`Fixes #12, #13` は 12 だけが対象です）。

### 進行を盤面へ記録できる工程に「設計レビュー」が入りました

`standard` と `architecture` は設計だけを載せた Pull Request をレビューへ通しますが、その工程を
盤面へ記録しようとすると値の検査で弾かれていました。**設計をレビューへ出したのか、まだ書いて
いる途中なのかが盤面から読めません。**

値の一覧へ `設計レビュー` を足し、工程表・対応表の 2 つの列・値の一覧の 4 つが同じ並びである
ことを機械で突き合わせるようにしました。片方だけが更新された状態は検査で止まります。

### テストが、実行されないまま緑になる経路を塞ぎました

`bash` / `jq` / `git` が無い実行環境では、テストが 1 件も実行されないまま終了コード 0 で
終わっていました。実測では 9 割以上のテストが消えています。読み飛ばしをやめ、**収集の時点で
不足を示して止めます**。読み飛ばしたい場合は `NDF_TESTS_ALLOW_MISSING_COMMANDS=1` を指定します。

`cross-refactoring` のテスト 12 件が実行環境の全体設定にある git の身元へ依存していた点も
直しました。テストが自分で身元を用意し、実行中は全体設定を読みません。
...
```text
# 動く: 実体パスを示して読ませる
~/.codex/plugins/cache/ai-plugins/ndf/10.0.0-dev.1/skills/deploy/SKILL.md を読んで、その手順どおりに qa/staging へ deploy PR を作成してください。
...
# ~/.codex/plugins/cache/ai-plugins/ndf/10.0.0-dev.1/skills/deploy/SKILL.md
```
...
# => ndf@ai-plugins  installed, enabled  10.0.0-dev.1  <path>
```
