...
- `foreground_sleep(cmd, limit)`: 前景の `sleep` で待つか。ループの本体の `sleep` と、`limit` 秒を超える `sleep` を当てる。
  `&` で背景へ回したもの・ヒアドキュメントとコメントの中は当てない。置換の中・`bash -c` / `sh -c`・`eval` の中は前景で動く
- `starts_plan(cmd)`: `supervise.py` の `queue` か `run`（プランを起こす副命令）を起動するか。コマンドの位置で起動して
...
def sleep_seconds(word: str) -> float | None:
...
def foreground_sleep(cmd: str, limit: float, in_loop: bool = False, depth: int = 0) -> bool:
    if depth > MAX_DEPTH or not cmd:
        return False
    return _sleep_in(sp.parse_bash(cmd), limit, in_loop, False, depth)


def _sleep_in(n: sp.Node, limit: float, loop: bool, bg: bool, depth: int) -> bool:
    t = n.type
...
def starts_plan(cmd: str) -> bool:
    if "supervise.py" not in cmd:
        return False
    return _plan_in(sp.parse_bash(cmd))


def _plan_in(n: sp.Node) -> bool:
    ...
    if n.type == "command":
        ...
        if i + 1 < len(words) and words[i].endswith("supervise.py") and words[i + 1] in ("queue", "run"):
            return True
    return any(_plan_in(c) for c in n.children if c.child_count)
