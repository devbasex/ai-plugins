...
| [review_criteria.py](review_criteria.py) | 指摘の基準の正本（基準 1〜4・書かないもの・見送りの種類・見送りの返信の雛形）と、レビューの重点の宣言（`.ndf/review.json`）の読み取り。レビュー担当への節・修正担当への節・見送りの返信を組む。宣言が読めなくても例外を上げず基準 1・2・4 で返す（#1287） | `cross-review`（`review_lib/commands/init.py`・`launch-reviewer.sh`） / `fix-steps.py` |
