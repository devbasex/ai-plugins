...
# `worktree-common.sh` と工程の通過記録の `workflow-common.sh` の 2 つで、どちらも自分の
...
# `mkdir` に成功した直後、目印を書く前に落ちた持ち主を救うために使う。
...
# 見始めたときの目印を `seen` で受け取り、判定の間に持ち主が替わっていれば捨てない。
#
...
_ndf_lock_is_stale() {
  ...
  # 目印が無いロックは、作った直後に落ちた可能性がある。古ければ捨ててよい。
...
# 目印と持ち主の番号を書く。目印を書いてから読み直す手は採らない。長さの違う目印が同じ
...
ndf_lock_acquire() {
  ...
  # 目印は識別のためだけに使う。持ち主の決定には使わないため、桁をそろえる必要はない。
