...
# (詳細設計 #215 の決定 5)。あわせて、tool 実行前の hook が記録へ積んだ案内をここで
...
# tool 実行前の hook が積んだ案内を取り出し、記録を空にする。取り出した案内を残すと、
...
「主ディレクトリに残った変更を移す」にあります。"
  ...
  [ "$FOLLOW" = 1 ] && message="${message}主ディレクトリのブランチは、
...
# 記録へ積まれた案内を最後に足す。開始時の提示より後ろへ置くのは、直前の操作への案内が
# 読み手にとって新しい情報だからである。
if [ "$AGY" = 1 ]; then
