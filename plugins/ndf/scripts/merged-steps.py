#!/usr/bin/env python3
"""merged-steps.py: merged の後片付けの決まった手順（#857。試作は #827 の phase-steps.py）。

    python3 merged-steps.py cleanup <PR番号>... [--root <dir>]

マージ済みの PR の作業ツリーとローカルブランチを外し、主ディレクトリを取り込む。
結果は lib/step_result.py の形の 1 行の JSON。終了コードは 0 = ok / 10 = `git branch -D` が要る
ブランチがある（同意が要る。提示物を書く）/ 1 = 取り込みが失敗 / 2 = 読めない。
"""
...
from step_result import (StepError, approval_present, common_parser, emit, git, git_root,  # noqa: E402
                         main_with, repo_slug, result, run)
...
def cmd_cleanup(a):
    ...
    if need_force:
        ...
        emit(result(TOOL, "gate", summary, items, metrics, path,
                    "同意を得たら git branch -D " + " ".join(names)))
