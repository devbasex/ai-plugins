...
#   - tool 実行前の hook (agy): 案内をセッションの記録へ積み、次のモデル呼び出しの前に
...
# --- セッション状態のキャッシュ ---------------------------------------------------
# tool 実行のたびに git を起動しないためのキャッシュ。解決したときの作業ディレクトリを
...
# agy はこの事象でモデルへ文言を返せない。案内は記録へ積み、次のモデル呼び出しの前に
# worktree-session.sh が取り出して渡す。ここで返すのは操作を止めない判定だけである。
# 記録を作れないとき (セッションの識別子が無いとき) は案内を落とす。案内が届かなくても
