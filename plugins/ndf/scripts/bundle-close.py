#!/usr/bin/env python3
"""bundle-close.py: まとまりを閉じる（progress-tracking の「まとまりを閉じる」の手順）。

    python3 bundle-close.py --record-pr <PR番号> [--prs 1,2] [--with-verification]
                            [--label "<マイルストーン>の<工程名>"] [--dry-run] [--root <dir>]

1. 記録の PR の本文とコメントを読み、最後の「## 配布の記録」ブロックを取る
2. まとまりの PR の一覧（--prs、無ければブロックの `まとまり:` の行）の本文から、閉じる語が
...
def parse_record(text):
    ...
    戻り値: {"found", "stage", "version", "bundle_prs", "verify_block"}。
    ...
    for i, ln in enumerate(lines):
        if ln.startswith(DIST):
            dist_start = i
    out = {"found": dist_start is not None, "stage": None, "version": None, "bundle_prs": [],
           "verify_block": None}
    ...
    for ln in block:
        ...
        elif ln.startswith("まとまり: "):
            out["bundle_prs"] = [int(x) for x in re.findall(r"#(\d+)", ln)]
...
def bundle_issues(root, repo, prs):
    seen, out = set(), []
    for n in prs:
        p = run(["gh", "pr", "view", str(n), "--repo", repo, "--json", "body", "-q", ".body"], cwd=root, check=False)
        if p.returncode != 0:
            raise StepError(f"まとまりの PR #{n} を読めない: {p.stderr.strip()[:300]}", EXIT_UNREADABLE)
...
def cmd_close(a):
    ...
    prs = a.prs or rec["bundle_prs"]
    if not prs:
        emit(result(TOOL, "stopped", "まとまりの PR の一覧が取れない。推測せず運用者に一覧を聞く",
                    [], {"issues": 0}, next="運用者に一覧を聞き、--prs で渡して打ち直す"), EXIT_UNREADABLE)
    issues = bundle_issues(root, record_repo, prs)
    ...
    items, notes = [], []
    comment = f"まとまり（{a.label}）を通りました" if a.label else "まとまりの終わりの工程を通りました"
...
def build_parser():
    ap = Parser(prog="bundle-close.py", description=__doc__, formatter_class=argparse.RawDescriptionHelpFormatter)
    ap.add_argument("--record-pr", type=int, required=True, help="配布の記録を置いた PR の番号")
    ap.add_argument("--prs", type=pr_list, help="まとまりの PR の番号（カンマ区切り）。省けば配布の記録から読む")
