...
do_verify() {
  ...
  if [ "$loaded" = "$branch" ]; then
    printf '一致: ローカル環境に %s が載っています\n' "$branch"
    return 0
  fi
  printf '不一致: ローカル環境に載っているのは %s で、対象は %s です\n' "$loaded" "$branch"
  return 1
}
...
# ローカル環境が指すコードを対象の作業ツリーへ向ける。実行系を叩くため、単体テストでは
