...
# 多ランタイム・リファクタリング（想定最大時間に収める 1 回の改修計画実行）

**参加者の全員が 1 度だけ多面的に提案し、選ばれた 1 者が想定最大時間に収まる分を
改修計画・テスト追加・実装・検証する。** ラウンドを上限まで回す形はとらない（#933）。
...
- [docs/02-plan-and-implement.md](docs/02-plan-and-implement.md) — 改修計画・テスト追加・実装
...
| 語 | 意味 |
| --- | --- |
| 想定最大時間 | `--budget-minutes`。改修計画はこの中に収まるように立てる。**時間に関わる数値はすべてここから逆算する**（手順の上限・テスト 1 回の上限・無音の打ち切り） |
| 手順 | 提案・改修計画・テスト追加・実装・検証/修正の 5 つ。**それぞれ 1 回だけ**行う（検証と修正の繰り返しを除く） |
| 実装担当 | 改修計画以降の 4 つの手順を通して担う 1 ランタイム |
| 候補 | 提案を鍵（`path` + `symbol` + `smell`）でまとめたもの。改修計画へ渡す |
| 改善項目 | 改修計画が採った候補。`I-001` の形の ID を持ち、**1 改善項目 = 1 コミット**（テストを足す項目は 2 コミット） |
| 等級 | 候補ごとに付ける適用の価値（`tier`: high / medium / low）。Jev か実装担当が付け、改善項目の順位の最初の鍵にする |
| 配分テーブル | 種類（`test` / `structure/<手法>` / `verify` / `fix`）ごとの 1 件あたりの所要。履歴の直近 10 回から改修計画のたびに集計する |
| 範囲テスト | 項目が触った箇所に限って走らせるテスト。改修計画の `test_targets` から進行側が組み立てる |
| 危険フラグ | 範囲テストでは覆えない変更（D1〜D5）。立ったら全体のテストを 1 度だけ走らせる |
| 改修計画 | 採る改善項目と、見送った提案とその理由を決める手順と、その出力（PR のコメントか `--plan-file`） |
| 予備時間 | 改修計画の見積りで、想定最大時間から経過を引いた後に差し引いておく時間（危険フラグと最終ゲートの全体のテスト・修正 1 回・最終ゲートの修正 1 回） |
...
| `--implementer NAME` | 改修計画以降を担う 1 者。参加者に無ければ `init` が止める | ホスト → 先頭 |
...
| 改修計画・テスト追加・実装・修正・最終ゲートの修正 | `--implementer` → ホスト（参加者にいれば）→ 参加者の先頭 | 多くはホスト |

- **agy は既定に入らない**（#664）。戻すときは `--include agy` を渡す
- **確認を通らない者は外して続ける。** 全員が揃わないなら始めたくないときは `--require-all`
- **再開で参加者を作り直し、実装担当が外れたら**、改修計画の前なら決め方を当て直し、改修計画の後なら止める
...
flowchart TD
    ...
    MP -->|はい| Gate
    MP -->|いいえ| Plan["改修計画（実装担当）<br/>等級・足すテスト・範囲テスト"]:::phase
...
| 改修計画の後に判断のために LLM を呼ぶ（Jev・判定の CLI） | 改修計画で見積もった時間の外で所要が伸び、結果が再現しない。判断は改修計画で済ませ、後はスクリプトの規則で決める |
