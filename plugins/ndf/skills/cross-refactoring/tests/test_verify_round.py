...
def _test_run(patch_lib, refactor, monkeypatch, code=0, timed_out=False):
    ...
    patch_lib("run_with_timeout", fake)
...
def test_a_passing_test_closes_the_apply_round(patch_lib, refactor, cmd_converge, tmp_path, env_tmp_dir, monkeypatch):
...
def test_a_failing_test_sends_the_apply_round_to_the_fix_loop(patch_lib, refactor, cmd_converge, tmp_path, env_tmp_dir, monkeypatch):
    """B1 / B3 — 失敗は適用ラウンドの単位で見る。項目までは特定しない。"""
    state_path = _state(tmp_path)
    env_tmp_dir(state_path)
    _test_run(patch_lib, refactor, monkeypatch, code=1)
...
def test_a_failing_test_records_the_fix_base(patch_lib, refactor, cmd_converge, tmp_path, env_tmp_dir, monkeypatch):
...
def test_a_timeout_counts_as_a_failure(patch_lib, refactor, cmd_converge, tmp_path, env_tmp_dir, monkeypatch):
    """打ち切りは通す側に倒さない。待ち続けると進行全体が止まる。"""
    state_path = _state(tmp_path)
    env_tmp_dir(state_path)
    _test_run(patch_lib, refactor, monkeypatch, code=None, timed_out=True)
...
def test_no_reviewer_is_launched(patch_lib, refactor, cmd_converge, tmp_path, env_tmp_dir, monkeypatch):
    """**2 CLI のレビューは起動しない**（決定 3）。判定はテストの結果だけで決まる。"""
    state_path = _state(tmp_path)
    env_tmp_dir(state_path)
    _test_run(patch_lib, refactor, monkeypatch, code=0)
...
def test_the_verification_uses_the_configured_timeout(patch_lib, refactor, cmd_converge, tmp_path, env_tmp_dir, monkeypatch):
...
def test_verifying_a_dropped_apply_round_is_refused(patch_lib, refactor, cmd_converge, tmp_path, env_tmp_dir, monkeypatch):
...
def test_the_phase_stays_on_apply_while_groups_remain(patch_lib, refactor, cmd_converge, tmp_path, env_tmp_dir, monkeypatch):
    groups = [
...
def test_the_phase_returns_to_propose_after_the_last_group(patch_lib, refactor, cmd_converge, tmp_path, env_tmp_dir, monkeypatch):
    state_path = _state(tmp_path)
    env_tmp_dir(state_path)
    _test_run(patch_lib, refactor, monkeypatch, code=0)
    cmd_converge.cmd_verify_round(_args())
...
def test_should_abandon_only_at_the_limit(cmd_converge, tmp_path, env_tmp_dir):
    ...
    with pytest.raises(SystemExit) as e:
        cmd_converge.cmd_should_abandon(args)
    assert e.value.code == 2

    state = read_state(state_path)
    state["rounds"][0]["fix_rounds"] = 3
    state_path.write_text(json.dumps(state), encoding="utf-8")
    cmd_converge.cmd_should_abandon(args)  # 上限到達で正常終了 = 見送りへ


# ---------- 外へ出す文章の規約（#436 決定 6-b） ----------

def test_a_passing_verification_names_the_items_and_the_plan(patch_lib, refactor, cmd_converge, tmp_path, env_tmp_dir, monkeypatch, capsys):
...
def test_a_failing_verification_still_names_the_plan(patch_lib, refactor, cmd_converge, tmp_path, env_tmp_dir, monkeypatch, capsys):
    url = "https://github.com/devbasex/ai-plugins/pull/130#issuecomment-7"
    state_path = _state(tmp_path, plan_mode="comment", plan_file="",
                        plan_comment={"id": 7, "url": url})
    env_tmp_dir(state_path)
