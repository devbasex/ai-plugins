...
def test_one_commit_for_the_apply_round_passes(verify):
    assert verify.verify_apply_round([item()], [fact()]) is None


def test_several_items_share_the_one_commit(verify):
...
def test_two_commits_in_one_apply_round_fail(verify):
...
def test_the_granularity_message_shows_the_count(verify):
...
def test_test_gap_still_needs_the_characterization_test_in_the_commit(verify):
...
def test_test_gap_does_not_buy_a_second_commit(verify):
    """テストを分けたい場合も、群のコミットは 1 つである。"""
    facts = [fact(sha="aaa", touches_tests=True), fact(sha="bbb")]
    problem = verify.verify_apply_round([item(test_gap=True)], facts)
    assert problem is not None and "適用ラウンド = 1 コミット" in problem


def test_a_broken_commit_is_reported_before_the_granularity(verify):
    """粒度は最後に見る。トレーラーの問題を粒度で覆い隠さない。"""
    broken = fact(sha="bbb", trailers=trailers(model=""))
    problem = verify.verify_apply_round([item()], [fact(sha="aaa"), broken])
    assert problem is not None and "トレーラーが欠けています" in problem


def test_the_budget_is_reported_before_the_granularity(verify):
...
def test_the_apply_check_does_not_look_at_the_test_result(verify):
