...
def test_items_on_different_files_share_one_apply_round(proposals):
    """書き換えるファイルが重ならない項目は、同じ群へ入る。"""
    groups = proposals.assign_apply_rounds(
...
def test_items_on_the_same_file_go_to_different_apply_rounds(proposals):
...
def test_a_third_item_on_the_same_file_needs_a_third_apply_round(proposals):
    """群の数に上限は置かない。**重なり方が決める。**"""
    groups = proposals.assign_apply_rounds([prop(path="src/a.py")] * 3)
    assert len(groups) == 3


def test_the_split_follows_the_design_example(proposals):
    ...
    groups = proposals.assign_apply_rounds([x, y, z, w])
    assert [[i["symbol"] for i in g] for g in groups] == [["X", "Z", "W"], ["Y"]]


def test_the_group_order_follows_the_adoption_order(proposals):
    ...
    groups = proposals.assign_apply_rounds([first, second, third])
    assert [g[0]["symbol"] for g in groups] == ["first", "second", "third"]


def test_only_the_path_decides_the_split(proposals):
...
def test_no_adopted_item_makes_no_apply_round(proposals):
    assert proposals.assign_apply_rounds([]) == []
...
def test_next_apply_round_resets_the_fix_rounds(patch_lib, paths, 
    ...
    entry["fix_rounds"] = 3
    state_path = _open_next(patch_lib, refactor, tmp_path, env_tmp_dir, monkeypatch, entry)
