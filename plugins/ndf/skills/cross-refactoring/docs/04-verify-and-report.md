...
1. 実装済みの項目ごとに、**HEAD で範囲テスト**を走らせる。語の並びが同じ項目どうしは
   ...
   雛形へ渡る
2. 修正に使える残り時間（開始 + 想定最大時間 − 全体のテスト 2 回分の予備時間 − 最終ゲートの修正の予備時間 `final_fix` − 今）が予備時間の
...
- **D5 は改修計画の手順で問い、検証は `items[].public_io` を読むだけである**（決定 25）。Jev へ送るのは
...
やり直す（実装計画 I6）。広げても積み直せなければ、改修計画の項目をすべて取り消す。半端な
...
- 同名のチェックは名前ごとの最新の実行（`completed_at` と `started_at` の新しい方）で判定する。
  別の実行で成功したチェックの、前の実行の失敗は数えない（畳み方は共通層の `gh_parts` が持つ）
...
打ち切らない**（決定 26）。修正 1 回分の予備時間（`final_fix`）を改修計画の時点で予算から差し引いて
あり、予算を使い切った後に落ちても必ず 1 度は直しを試みる。1 回目の監視の上限は残りと予備時間の
長い方 + 余裕である。
...
**配分テーブルは保存しない。** 改修計画のたびに、利用者の手元の履歴から集計する（決定 7）。
...
無い・読めないときは初期値で改修計画し、1 行知らせる（`plan.table_source` が `defaults`）。
