...
- **D5 は計画の手順で問い、検証は `items[].public_io` を読むだけである**（決定 25）。Jev へ送るのは
...
- 同名の検査は名前ごとの最新の実行（`completed_at` と `started_at` の新しい方）で判定する。
  別の実行で成功した検査の、前の実行の失敗は数えない（畳み方は共通層の `gh_parts` が持つ）
...
打ち切らない**（決定 26）。修正 1 回分の予備時間（`final_fix`）を計画の時点で予算から差し引いて
...
**`fix` フェーズと `merge-fix` は使い回せない。** どちらも落ちた項目の記録を読み、
