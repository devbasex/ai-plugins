...
1. 実装済みの項目ごとに、**HEAD で範囲テスト**を走らせる。語の並びが同じ項目どうしは
   ...
   雛形へ渡る
2. 修正に使える残り時間（開始 + 想定最大時間 − 全体テスト 2 回分のバッファ − 最終ゲートの修正のバッファ `final_fix` − 今）がバッファの
...
5. 全体テストが落ちたら、**落ちたテストだけ**を走らせ直して見分ける（決定 22）。全体の
   ...
   - まだ落ちるものを着手前の HEAD（`init` が残した SHA）で走らせる。作業ディレクトリは
     動かさず、一時の detach の worktree で走らせて消す。落ちれば**元からの失敗**
   - 揺れと元からの失敗は取り消さず、`whole_test` に分類して残す
   - 残り（**変更が原因**）は、2 と同じ締め切りの内なら危険フラグの項目を修正へ回す
...
| D4 | 範囲テストが触った本番のファイルを覆うと示せない | 範囲テストのファイルに、拡張子を除いたファイル名と `symbol` の名前のどちらも現れない |
...
終わったらフラグを消す。フラグが残ったまま再開したら、取り込みと検証の入口で同じ取り消しを
...
| 返す値 | 何に使うか |
| --- | --- |
| `FINAL_GATE=failing` | 最終ゲートの修正へ入ることを示す |
...
である。ただし**起動し直しても解けない結末（利用上限）では打ち切りのフラグ（`no_relaunch`）を
...
リファクタリング計画にも載る）・検証の中の全体テストと落ちたときの見分けと結末・Jev の使用・
...
**リファクタリング計画は実行の記録であって、リポジトリの知識ではない**（#436 決定 6）。既定の
