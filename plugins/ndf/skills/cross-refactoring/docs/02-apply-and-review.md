...
```bash
# 起動する担当は状態が決める。初回は担当 2 者、修正の後は変更要求を出した担当だけになる。
# ⚠ コマンド置換の終了コードは変数で受けてから読む
TARGET_VARS=$("$SCRIPTS/refactor.py" review-targets "$ID" "$ROUND") || exit $?
eval "$TARGET_VARS"
...
初回はレビュー担当 2 者を並列起動し、**ラウンドの差分全体**に対して 1 回レビューさせる。
指摘は AI 自身が `gh api` でインラインコメントとして直接投稿する。

### 再レビューは変更要求を出した担当だけにする

修正は指摘への対応である。**承認した担当が読むのは、自分が通した内容に対する他者の指摘への
対応であり、同じ差分をもう一度読むことになる。** `review-targets` が状態を見て、初回なら
担当 2 者を、修正の後なら変更要求を出した担当だけを返す。

| ラウンドの状態 | 返る担当 |
| --- | --- |
| `fix_reviewers` が無い（初回。既存の状態ファイルを含む） | 担当 2 者 |
| `fix_reviewers` がある（修正の後） | 変更要求を出した担当だけ |
| 差し戻し（`invalid`）の後 | 担当 2 者 |

**差し戻しは絞らない。** 結果の形が判定に使えない状態は修正の成否とは別で、承認した担当の
結果も読めていない可能性がある。差し戻し上限からの落ちこみでも絞らない。合成した指摘は
誰が出したものでもなく、対象が決まらない。

**起動しなかった担当の判定は、前のレビューの結果ファイルから引き継ぐ。** 結果ファイルの
パスはラウンド番号で決まるため、書き直さなければその場に残る。`judge-review` はそれを読み、
承認として数える。

**失う検出は、進行側の機械検証と後段の `cross-review` が補う。** 範囲外を触った修正は
`--scope` の逸脱・差分予算・テストの検証が拾い、Pull Request 全体の整合は Step 7 が見る。
...
同じ前提（採用 5 件・修正 1 回）で、1 者だけが変更要求を出した場合である。

| 再レビューの範囲 | 内訳 | 起動回数 |
| --- | --- | --- |
| 担当 2 者 | 提案 3 + 適用 1 + レビュー 2 + 修正 1 + 再レビュー 2 | 9 |
| **変更要求を出した担当だけ** | 提案 3 + 適用 1 + レビュー 2 + 修正 1 + 再レビュー **1** | **8** |

2 者とも変更要求を出したラウンドでは変わらない。**減る量は、2 者の判定が分かれる頻度で
決まる。** 7 回目の実機試行（4 ラウンド）では判定の一致率が 0.40〜0.67 で、分かれる方が
多かった。標本が 1 本のため、削減量の実測は前後を同じ Pull Request で取り直すまで確定しない。
