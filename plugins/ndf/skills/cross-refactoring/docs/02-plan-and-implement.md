...
**計画を読めなくても止めない。** 段は既定（または Jev）、テストは足さず、範囲テストは
`--round-test` をそのまま使う形で進める。止めると提案に使った時間が丸ごと無駄になる。

### `merge-plan` がすること

1. **段を決める。** Jev が使えるときは候補ごとに `score`（low / medium / high）を問い、
   確信度 0.6 以上なら Jev の段、未満か失敗なら実装担当の段（失敗は `judge.failures` に数える）
...
5. **順位を決める。** (段, 賛同した者の数, 重要度) の降順、同じなら見積りの昇順
