...
**改修計画以降の 4 つの手順は実装担当 1 者が通す**（決定 1）。1 回の起動の中で項目ごとに
コミットさせ、所要の測定と項目の単位の取り消しを git から行う（決定 12）。

## 改修計画

```bash
rf_eval start-phase "$ID" plan               # PHASE_TIMEOUT = 改修計画の枠の終わりまでの残り + 余裕
...
**改修計画を読めなくても止めない。** 等級は既定（または Jev）、テストは足さず、範囲テストは
...
6. **時間に収まる件数を選ぶ。** 使える時間 = 想定最大時間 − 経過 − 予備時間。順位の順にたどり、
...
予備時間（`plan.reserve`）は次の 4 つの和である。

| 予備時間 | 値 |
| --- | --- |
| `danger_whole_test` | 着手前の全体のテストの秒（危険フラグが立ったときの 1 回） |
| `final_whole_test` | 同上（`--ci-check` があれば 0。最終ゲートの全体のテスト） |
| `fix` | 配分テーブルの `fix`（修正の起動 1 回分） |
| `final_fix` | 同上（最終ゲートの修正 1 回分。検証の直しには使わせない。決定 26） |

**改修計画は叩き直しても作り直さない。** 採用の件数・締め切り・予備時間はこの時点の予算で固定する。
...
**差し替えずに `--baseline-test` を項目の検証に使うことはない**（全体のテストは危険フラグの
1 回だけ）。対象の語の見分けは `scope.round_test_roots` と同じ規則である。

### 締め切り

予算の末尾から逆算する。採った項目を順位の順に 1..n、終わり `T = 開始 + 想定最大時間 − 予備時間` とする。
...
開始は `started_at`、R は予備時間（危険フラグと最終ゲートの全体のテスト、修正 1 回、最終ゲートの修正 1 回）である。
...
| 最終ゲートの修正の 1 回目の長さの下限（`final_fix_seconds`） | 予備時間の `final_fix` | `merge-plan` |
...
`verify` が時計を見る（予備時間の `fix` より残りが短ければ打ち切る）。回数の上限は持たない。
...
**足すのは改修計画が「足すテスト」を持つ項目のテストだけである**（網羅の提案はしない）。
...
**テストの差分は 一次と二次の 2 回で判定する**（#443、決定 25）。改修計画の後に判断のために LLM を起動しない。
