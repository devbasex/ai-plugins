...
1. **引数の検査** — `--budget-minutes` は 1 以上の整数でなければ止める（終了コード 4。
   ...
   使った後で、全項目が「限ったテストを組み立てられない」（`no_target`）になるためである
...
12. **着手前のテスト** — `--baseline-test` を実行し、`--round-test` を渡したときはそれも
    ...
    `baseline_test.seconds` に残し、危険の印と最終ゲートの全体のテストの控えに使う
...
| 版 2 で終わっていない | **再開する。** `PHASE` を返し、駆動は終わったフェーズを飛ばす。各取り込みは取り込み済みの印で冪等 |
