...
| `IMPL` / `IMPL_MODEL` | 実装担当（改修計画・テスト追加・実装・修正・最終ゲートの修正）とそのモデル |
...
1. **引数のチェック** — `--budget-minutes` は 1 以上の整数でなければ止める（終了コード 4。
   ...
   `uv run` / `poetry run` / `npx` は読み飛ばす）でなければ止める。提案と改修計画に時間を
...
| 版 2 で終わっていない | **再開する。** `PHASE` を返し、駆動は終わった手順を飛ばす。各取り込みは取り込み済みの目印で冪等 |
...
| `--budget-minutes` | **改修計画の前**（`phase` が `propose` か `plan` で改修計画が無い）だけ置き換え、上限の表（`limits`）を組み直す。後は知らせるだけ。採用の件数・締め切り・予備時間は改修計画の時点の予算で固定されている |
| `--implementer` | 知らせるだけ（置き換えない） |
| `--exclude` / `--include` / `--require-all` | 参加者を作り直す。**実装担当が外れたら**、改修計画の前は決め方を当て直し、改修計画の後は止める |
...
[取り込みの機械検証](02-plan-and-implement.md#取り込みのチェック)である。
...
**提案は参加者の全員が 1 度だけ、並行して行う。** 提案と改修計画は想定最大時間の中の枠で
行い、枠の終わり（提案 `開始 + 0.20·B`、改修計画 `開始 + 0.30·B`）を監視の上限にする
...
4. 残りを（賛同した者の数、重要度）の降順に並べ、`path` + `symbol` の組の単位で
   **上位 30 組**を取る。取った組の中は**上位 3 件まで**改修計画へ渡す（最大 90 件）。
   外れた組の提案と、組の中の 4 件目以降は `rank` で見送る

**件数ではなく組で切る**（決定 17）。件数で切ると、意味の上で同じ提案が枠を占め、改修計画の
中で統合された後に空いた枠を埋められない。「同じ変更か」は同じ組の中でしか問わない
ため、組で切れば統合で組の数は減らない。

**意味の上で同じ提案かはここで決めない。** 改修計画の中で Jev か実装担当が問う
