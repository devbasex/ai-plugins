...
1. **ホストの確定** — `--host` の明示指定を第一とし、未指定時のみ環境変数
   ...
   ファイルの両方へ残す
2. **母集合の確定** — 提案・レビュー（全 − ホスト）と適用（全 − agy）を**別々に**確定する。
...
agy は NDF の配布先ではないため「標準の配置先」を持たないが、**兆候と手法の語彙を
読ませないと提案が語彙外になって全件降格する**ので、agy が作業領域の中で読む
`.agents/skills` へ置き、プロンプトの明示パスで読ませる（kiro と同じ扱い）。
