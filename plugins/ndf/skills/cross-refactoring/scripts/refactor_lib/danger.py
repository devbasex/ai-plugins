"""危険フラグ（D1〜D5）を git の事実から判定する（#933 の「危険フラグ」）。

危険フラグは、項目ごとの範囲テストで覆えない変更を表す。危険フラグが 1 つでも立てば、
検証の中で全体のテストを 1 度だけ走らせる。

**判定は git だけを情報源にする。** 実装担当の申告で検証を減らさない。例外は D5 で、
担当の `risk` は危険フラグを**立てる側にだけ**使う（ここでは真偽を受け取るだけ）。
...
def d4(work: str, test_files: Optional[list[str]], touched: str, symbol: str) -> bool:
    """範囲テストが触った本番のファイルを覆うと**示せない**か。

    範囲テストのファイルのどれかに、拡張子を除いたファイル名か `symbol` の名前
...
def item_flags(
    ...
    """項目に立った危険フラグ（`D1`〜`D5`）と、D3 で当たった語を返す。
