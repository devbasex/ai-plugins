...
直ったかどうかを判定できず、同じ不具合の再発も検出できない。**再現テストの RED/GREEN の
詳しい進め方は `tdd-cycle` の「バグ修正は再現テストから」が持つ**（未導入なら対象
プロジェクトの既存テスト手順）。ここで写しを持たず、そちらに従う。
...
壊しても気づけない。**現状固定テストの手順は `refactoring` の現状固定テストが持つ**。
不具合を含む現状もそのまま固定される点、修正後に不具合を固定していた期待値を正しい値へ
更新する点も、そちらに従う。
