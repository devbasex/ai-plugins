...
| --- | --- | --- |
| ドメイン用語 | `概要` の直後に `用語` として独立させる | プロジェクトに用語集の宣言（`.ndf/glossary.json`）があれば、宣言の `document` の文書を指し、定義を写さない（用語集が正で、写しが 2 つ目の正になる）。宣言が無ければ定義をそのまま残す。定義した経緯は落とす |
...
- `stopped`（1 = コミットする変更が無い・git が失敗 / 3 = 仕様書か設計のファイルが無い、または用語集の宣言か正本が読めない・語の形が崩れている）なら `summary` を読んで直し、もう一度打つ
