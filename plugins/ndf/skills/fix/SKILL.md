...
出力は件数と参照だけで、本文を出さない。
送信が認証で落ちたときの退避（#524）もこのコマンドが行う。値は共通層の
`scripts/lib/git-credential.sh` 1 か所が持つ。
...
**修正で設計文書の決定を変えると、Pull Request の本文が古いまま残る。** 本文を書き換える
手順を持たないと、承認する人が古い決定を読む。対象かどうか（head が `design/` で始まるか）は
スクリプトが決めるため、ここでは判定せずに呼ぶ。

```bash
# $SCRIPTS はプラグインの scripts/ の絶対パス。決め方は
# development-workflow/references/scripts-lookup.md にある
bash "$SCRIPTS/pr-body-decisions.sh" sync "$PR_NUMBER"; echo "exit=$?"
```

**コミットが無くても実行する。** 進行側が先に設計文書を直していた場合も、ここで本文が揃う。
節の形は `/ndf:pr` の「設計 Pull Request の本文」にある。終了コードは作業完了報告に載せ、
2（読めなかった）を一致と書かない。

**flaky テストの扱い**: PR の変更範囲外で発生している flaky テストも、見つけ次第この PR で
修正する。放置するとリポジトリ全体のコード品質が下がり、後続 PR の CI 信頼性も損なわれる。

**範囲外の指摘と flaky テストは分けて扱う。** どちらも PR の変更範囲の外にあるが、
行き先が違う。

| 対象 | この PR で直すか | 残し方 |
| --- | --- | --- |
| flaky テスト・CI の失敗 | 直す | 修正コミットに含める |
| 上記以外で範囲外と判断した指摘 | 直さない | `/ndf:out-of-scope` で起票し、番号を返信へ書いてから resolve する。**起票先のリポジトリはその Skill が決める** |

flaky テストを例外にするのは、直さないと後続の PR が CI の結果を読めなくなるためである。
それ以外の範囲外の指摘は、直しても PR の目的から外れる。
...
- `pr-body-decisions.sh sync` の終了コード（0 一致・対象外 / 1 書き直しても食い違う / 2 読めなかった）
- **PR URL を最後に必ず記載**
