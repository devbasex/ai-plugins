...
| `status` | 「ミッションを閉じる」だけ | `Done` | 書かない（盤面だけ） |
...
**終わりの工程で盤面の `Status` を `Done` にするのは、下の「ミッションを閉じる」だけである。**
...
- スクリプトが行うこと: 配布の記録の読み取り → 閉じる語が指す課題の収集 → 閉じる条件の判定 →
  課題ごとに「状態を読む → 記録のリポジトリの課題なら盤面を Done → 読み直して OPEN なら閉じる →
  読み直して CLOSED を確かめる」。他のリポジトリの課題には盤面を書かない
...
`would_close` も出る）。`kind: "board_note"` は盤面の NOTE で、報告にそのまま載せる。

| `result` | 意味 | 報告に載せるもの |
| --- | --- | --- |
| `closed` | 閉じた（盤面の自動化が閉じた場合を含む） | 番号と `cmd`（戻し方 `gh issue reopen`） |
