...
```bash
python3 "$SCRIPTS/bundle-close.py" --record-pr <記録のPR番号> \
...
| `stopped`（2） | まとまりの PR の一覧が取れない。推測せず運用者に一覧を聞き、`--prs` で打ち直す |
...
| # | 条件 | 満たさないとき |
| --- | --- | --- |
| 1 | 最後の「## 配布の記録」の `段階:` が `本番` または `配布なし` で始まる | まとまりの課題すべてを OPEN のまま残す |
...
**配布の記録は Pull Request に置く。** 書く側はこの形で書き、`bundle-close.py` はこの形だけを読む。

| 記録 | 書く形 | 置く側 | 置き場所 |
| --- | --- | --- | --- |
| 配布の記録 | 見出し `## 配布の記録` の直後に `段階: <検証 / 本番 / 配布なし>（本番なら承認を得た時点、止めたなら理由、配布なしなら飛ばした理由）`・`版: <前の版> → <新しい版>（<根拠>）`（`配布なし` のときは書かない）・`まとまり: PR #<番号> / #<番号>` の行が続くブロック。終わりは次の `## ` 見出しか入力の終わり | `release` | 配布の Pull Request の本文。配布の形が Pull Request を作らないときと配布を飛ばしたときは、まとまりの最後にマージした Pull Request へ同じブロックをコメントで投稿する |
