...
| ファイル | 書き手 | 役割 |
| --- | --- | --- |
| 宣言（`.ndf/glossary.json`） | `glossary.py init` が初めの 1 回。後はプロジェクトの変更 | 用語集の置き場・形式・検査の対象 |
| 用語集（宣言の `source`） | プロジェクトの変更（Pull Request） | 語・意味・コンテキスト・廃止した語・正本。**正** |
| 人が読む文書（宣言の `document`） | `glossary.py render` だけ | 用語集から作る写し。手で直さない |
...
```json
{
  ...
  "terms": [
    {"term": "注文", "context": "ordering", "meaning": "顧客が買うと決めた品の組",
     "deprecated": ["オーダー"], "source": "docs/ordering.md"},
    {"term": "注文", "context": "shipping", "meaning": "倉庫へ出す配送の依頼"}
  ]
}
```

`注文` は 2 つのコンテキストで別の意味を持つ。1 語 1 意味はコンテキストごとに守る。
...
| `check.source_paths` | 文字列の配列 | 許す | 語の正本（`terms[].source`）に認めるパスの glob。確定した文書（確定仕様・配布した手順）だけを書く。空は「正本の置き場を見ない」。当たらないパスは `unconfirmed_source` で落とす（URL は見ない） |
...
`render` が、コンテキストごとの見出しと 4 列の表（語・意味・廃止した語・正本）を書く。セルの中の
`|` は `\|` に、改行は空白に置き換える。用語集を変えて `render` を打っていない変更は、`check` が
`stale_document` で落とす。

## 語のチェックの規則

`glossary.py check` の規則。`--rules structure` は上の 3 つだけ、`all`（既定）は 5 つを見る。

| 規則 | 見るもの | 当たる条件 |
| --- | --- | --- |
| `schema` | 用語集 | 必須の項目が無い・廃止した語が同じコンテキストの生きた語と重なる・`context` が宣言されていない・1 文字の廃止した語 |
...
- 照合から外すもの: コードブロック・インラインコード・宣言の `document` の文書。廃止した語を説明の
  ために書くときはインラインコードで囲む
...
| 副命令 | すること | 終了コード |
| --- | --- | --- |
| `gate --mode M` | 設計の工程の入口の検査。止めたときは作る手順の 3 行を `items` に出す | 0 = 通す（`M` が `standard` / `legacy-refactor` 以外、または揃っている）。1 = 宣言・用語集・文書のどれかが無い。2 = 読めない |
...
| `check [--diff BASE \| --file P...] [--rules all\|structure]` | 上の規則 | 0 = 当たりなし（宣言が無いときも 0）。1 = 当たりあり。2 = 読めない |
| `diff --base REF [--head REF]` | 2 つの版の間の `added` / `meaning_changed` / `deprecated` / `removed` を返す。`--head` の既定は作業ツリー | 0。無い版は空の用語集として比べる |
