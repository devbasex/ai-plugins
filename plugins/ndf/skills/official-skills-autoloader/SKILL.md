...
配布先は `plugins/ndf/manifests/claude-skills.txt` のみとする。Codex / Kiro の manifest には載せない。
...
リポジトリを直接 clone して作業している場合は `plugins/ndf/scripts/install-official-skills.sh`
（編集元は `plugins/ndf/scripts/install-official-skills.sh`）を使う。
