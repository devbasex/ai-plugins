...
**合否は終了コードで判定する。出力は理由を読むためのものである。** チェックは集計や進捗を
標準出力へ書きながら、判定は終了コードだけで返すことがある。末尾の行が集計に見えると、
落ちていても通ったと読める。

```bash
python3 <チェック> >/tmp/out 2>&1; echo "exit=$?"   # 判定はこの値で行う
tail -5 /tmp/out                                # 落ちた理由を読む
```

- **`&&` や `|` でつながない。** `チェック | tail` は `tail` の終了コードを返し、チェックの失敗が
  消える。変数で受けるか、実行の直後に `$?` を読む
- 終了コードを証跡へ書く。**「通った」という語だけを書かない**

## 禁止

- **実行していないテストを「通った」と書かない。** 実行していないなら、実行していないと書く
- **出力の末尾だけを読んで合否を決めない。** 判定は終了コードで行う
...
**`operation` はプロダクションコードを触らないため、段階 1 と 2 に実行するものが無い。** 代わりに
確かめるのは**外部の系へ何が届いたか**である。単位ごとの終了コードと、反映を別の経路で
確かめた結果が証跡になる。確かめられない単位は、その旨を残して満たす。

**`documentation` はプロダクションコードを持たないため、段階 1 と 2 に実行するものが無い。** 段階 3 に
あたるのは**リンク・記法・行数のチェック**である。代わりに求めるのが**事実確認**で、
書かれた値が出典と一致することを確かめる。

**事実確認は書いた本人だけでは完了しない。** 書いた側が確かめると、同じ思い込みで通る。
**`cross-review` のレビュワーが、出典の節と本文の値を突き合わせる。** 新しい仕組みは作らず、
レビューの観点として渡す。
...
**「人手の確認」として残す前に、機械で打てないかを確かめる。** 端末・画面・操作を伴う条件でも、
擬似端末（`script` / `expect` / pty）に本物のクライアントをつなぎ、その端末へ入力を打てば、振る舞いは
マージ前に確かめられる。使い捨ての環境（実装で建てたイメージ・コンテナ）があれば、その中で打つ。
人手の確認として残すのは、実際の端末エミュレータや画面での描かれ方のように、打っても確かめられない
部分だけにする。振る舞いと見た目が 1 つの条件にまとまっていれば、分けて振る舞いの側を確かめる
（分け方は `requirements-design` の `references/acceptance-criteria.md`）。

**文書の文言を照合するテストを、受け入れ条件の検証手段にしない。** 文書（`.md`）の変更を
求める条件は、該当の節を読んで確かめ、その節を証跡に書く。文言を固定しても振る舞いは
保証されない（`tdd-cycle` の `references/test-quality.md` の例 8）。
...
`/ndf:out-of-scope` を呼んで起票し、番号を報告へ書く。**起票先を自分で決めない。** 開発中の
リポジトリとは別のところへ出す課題があり、判断表はその Skill が持つ。
...
この工程に入ったら進捗記録 `bash "$SCRIPTS/projects-sync.sh" <issue番号> stage "完了判定"` を 1 行打つ（issue の本文とボードの両方に残る。`$SCRIPTS` の決め方は `development-workflow` の `references/scripts-lookup.md`、3 層では起動指示の「進捗記録」を使う）。
