...
**合否は終了コードで判定する。出力は理由を読むためのものである。** 検査は集計や進捗を
標準出力へ書きながら、判定は終了コードだけで返すことがある。末尾の行が集計に見えると、
落ちていても通ったと読める。

```bash
python3 <検査> >/tmp/out 2>&1; echo "exit=$?"   # 判定はこの値で行う
tail -5 /tmp/out                                # 落ちた理由を読む
```

- **`&&` や `|` でつながない。** `検査 | tail` は `tail` の終了コードを返し、検査の失敗が
...
| `documentation` | 3 | **事実確認**（書かれた値と出典の突き合わせ）、リンクと記法の検査 |

`light` は振る舞いを変えない変更なので、段階 1 と 2 を必須にしない。代わりに変更箇所が
読み込まれる経路を 1 度実行し、壊れていないことを示す。振る舞いを変えるなら `light`
ではない。

**`operation` は本番コードを触らないため、段階 1 と 2 に実行するものが無い。** 代わりに
確かめるのは**外部の系へ何が届いたか**である。単位ごとの終了コードと、反映を別の経路で
確かめた結果が証跡になる。確かめられない単位は、その旨を残して満たす。

**`documentation` は本番コードを持たないため、段階 1 と 2 に実行するものが無い。** 段階 3 に
あたるのは**リンク・記法・行数の検査**である。代わりに求めるのが**事実確認**で、
