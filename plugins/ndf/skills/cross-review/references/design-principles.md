...
| 観点 | 方針 |
|---|---|
| 投稿の担い手 | **GitHub と git へ書くのはレビューを回す側だけ**。担当は指摘の控えと結果ファイルを書き、取り込み（`read-result`）が組み立てて待ち行列から送る。修正の担当はコミットまでで、送信・返信・決着・まとめは `merge-fix` が行う。書き込みと記録が同じ手順で続くため、担当が途中で止まっても投稿だけが残らない |
