...
def _resolve_pr_and_ownership(
    ...
    repo: str,
    worktree: str,
    args_worktree: str | None,
    meta: PrMetadata | None,
) -> _InitPRContext | None:
    # 新規 init: プリチェック。
...
def _prepare_review_instructions(
    changed_files: list[str], manual_extra_review: str
) -> _InitReviewContext:
...
def _ensure_worktree(worktree: str, pr: object, head_branch: str) -> None:
...
def _prepare_worktree_and_comments(
    worktree: str, pr: object, repo: str, tmp_dir: pathlib.Path
...
def _init_new_state(
    ...
    meta = _fetch_pr_metadata(pr, repo)
    pr_ctx = _resolve_pr_and_ownership(pr, repo, worktree, args.worktree, meta)
    if pr_ctx is None:
        return

    changed_files = _fetch_changed_files(pr, pr_ctx.repo)
    review_ctx = _prepare_review_instructions(changed_files, manual_extra_review)
    _ensure_worktree(pr_ctx.worktree, pr, pr_ctx.meta.head_branch)
    tmp_dir = _tmp_dir(pr_ctx.worktree)
    ws_ctx = _prepare_worktree_and_comments(
        pr_ctx.worktree, pr, pr_ctx.repo, tmp_dir
    )
