...
# ---- execute --mode light の前提ファイル検証（R2-003、現状固定） ----
#
# execute_light は prepare.json / newtext.json の有無と title / body の欠落を 4 本の分岐で
# 弾く。いずれも `git push` と旧 PR の close より前で止まるため、gh は 1 度も呼ばれず
# 旧 PR は open のまま残る。既存の _Rotation は両ファイルを揃えて書くので、ここでは
# 欠けさせる・書き換える操作を足してから実行する。正しさを主張しない現状固定テスト。


def _prepare_path(rotation: _Rotation) -> pathlib.Path:
    return rotation.tmp / f"rotate-pr{_STATE_PR}-prepare.json"


def _newtext_path(rotation: _Rotation) -> pathlib.Path:
    return rotation.tmp / f"rotate-pr{_STATE_PR}-newtext.json"


def _assert_stopped_before_touching_the_old_pr(rotation: _Rotation) -> None:
    assert rotation.gh_calls() == []
    assert rotation.pr_states() == {str(_OLD_PR): "open"}


def test_light_mode_stops_when_prepare_json_is_missing(rotation: _Rotation) -> None:
    """prepare.json が無いと終了コード 1 で止まり、gh は呼ばれない。"""
    _prepare_path(rotation).unlink()

    out = rotation.run(create_ok=True)

    assert out.returncode == 1
    assert "prepare.json not found" in out.stderr
    _assert_stopped_before_touching_the_old_pr(rotation)


def test_light_mode_stops_when_newtext_json_is_missing(rotation: _Rotation) -> None:
    """prepare.json はあっても newtext.json が無いと終了コード 1 で止まり、gh は呼ばれない。"""
    _newtext_path(rotation).unlink()

    out = rotation.run(create_ok=True)

    assert out.returncode == 1
    assert "newtext.json not found" in out.stderr
    _assert_stopped_before_touching_the_old_pr(rotation)


def test_light_mode_stops_when_the_title_is_empty(rotation: _Rotation) -> None:
    """newtext.json の title が空文字列だと終了コード 1 で止まり、gh は呼ばれない。"""
    _newtext_path(rotation).write_text(
        json.dumps({"title": "", "body": "x"}), encoding="utf-8")

    out = rotation.run(create_ok=True)

    assert out.returncode == 1
    assert ".title がない" in out.stderr
    _assert_stopped_before_touching_the_old_pr(rotation)


def test_light_mode_stops_when_the_body_is_null(rotation: _Rotation) -> None:
    """newtext.json の body が null だと終了コード 1 で止まり、gh は呼ばれない。"""
    _newtext_path(rotation).write_text(
        json.dumps({"title": "x", "body": None}), encoding="utf-8")

    out = rotation.run(create_ok=True)

    assert out.returncode == 1
    assert ".body がない" in out.stderr
    _assert_stopped_before_touching_the_old_pr(rotation)
