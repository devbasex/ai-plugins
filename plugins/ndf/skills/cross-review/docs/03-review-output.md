...
レビュー出力の制約は `scripts/launch-codex.sh` / `scripts/launch-agy.sh` の
...
`HTTP 422` を返し、**インラインだけでなくレビュー本体も作られない**（要求ごとに全件が拒まれ、
応答はどのインラインが原因かを指さない）。担当はこれを気にせず、指す行が分かる指摘には
`path` と `line` を書く。**投稿する側（取り込み）が、拒まれた要求のインラインをすべて総評へ
移して送り直す。** 移す契機は応答が行やファイルを解決できないこと（`could not be resolved`）を
示すときだけで、判定の値や基準のコミットの誤りによる 422 は失敗として止める（#730）。
...
- ❌ **担当に GitHub へ投稿させる** — 投稿と記録を別の相手が行うと、担当が途中で止まった
  ...
  担当は指摘のファイルと結果ファイルを書くだけにし、取り込みが待ち行列を通して送る（#730）
...
`monitor.py` が EARLY_ERROR で codex / agy を即時 kill してしまい、`result.json` が
