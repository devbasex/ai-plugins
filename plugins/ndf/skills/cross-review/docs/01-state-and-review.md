...
round エントリを保存する前に**既存コメントのスナップショットを取り直す**（#542。失敗しても止めない。条件と形は [04-contracts.md](04-contracts.md) の「ラウンドの開始時に担当へ渡すもの」）。
...
**書き込みと記録を 1 つの部分命令に閉じる**（#730）。`read-result` は「指摘のファイルを読む → 投稿を
