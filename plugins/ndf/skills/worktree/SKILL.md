...
進行を盤面へ記録する場合は `bash "$SCRIPTS/projects-sync.sh" <issue番号> stage 作業場所の用意` を
実行する（`.ndf/projects.json` が無いリポジトリでは何も起きない。詳細は [references/projects-tracking.md](../development-workflow/references/projects-tracking.md)）。判定したモード（`mode`）と作業ツリーのパス（`worktree`）も同じ形で記録する。
