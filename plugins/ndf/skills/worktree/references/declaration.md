...
作業ツリーの起点がこのブランチになる（`follow_branch: true` のときは主ディレクトリの追従先にもなる）。
...
## 主ディレクトリのブランチを稼働中の作業ツリーへ追従させる

**セッション開始時の hook は、既定では主ディレクトリの HEAD を動かさない。** 並列に動く
エージェントのどれが開始・再開しても、他の担当が読んでいる主ディレクトリの内容が入れ替わらない
ためである。1 人が 1 つの作業ツリーで作業し、主ディレクトリを開いたエディタでその内容を見たい
...
| 項目 | 型 | 既定 | 意味 |
| --- | --- | --- | --- |
| `follow_branch` | boolean | `false` | `true` のときだけ、セッション開始時の hook が主ディレクトリを稼働中の作業ツリーへ追従させる。真偽値以外（`"true"` / `1` / `null`）は `false` と同じ |

有効にしたときの追従先は [SKILL.md](../SKILL.md) の「主ディレクトリのブランチ」にある。作業
ツリーが 1 つならそのブランチのコミットへ、0 個または複数なら `base_branch`（無ければ既定
ブランチ）へ detached HEAD で合わせる。主ディレクトリに未コミットの変更があるときは追従しない。
