...
画面を触って動作を確かめるサービス一式（ローカル環境）を持つリポジトリで、worktree の
変更を検証する手順である。

worktree は**追跡されているファイルしか持たない**。依存物も環境ファイルも無いため、
...
**ホストとコンテナでパスが一致しない構成がある。** コンテナを操作する場所とコンテナ
...
**作業ディレクトリの指定を落とすとメインディレクトリのコードが検証される。** データベースは
共有のままなので、同じ接続先を使うテストは同時に 1 本だけ走らせる。分離して走らせる
手順は [`test-execution.md`](test-execution.md) にある。

### 画面で確認するとき

コードの位置を worktree へ向ける。**同時に 1 本だけで、その間はメインディレクトリの
コードが画面に出ない。**
...
実行中のプロセスがコードの位置を保持している場合は、再読み込みを促すシグナルを送る
（`reload_signal`）。コンテナの最初のプロセスと再読み込みを受け取るプロセスが別の
構成があるため、送り先はプロセス名で指定する。

`direct` 型ではマウント元を変数にした定義を用意し、その変数を worktree へ向けて対象
サービスだけを作り直す。`host` 型では worktree を作業ディレクトリにして起動し直す。
...
worktree を削除する前に、同じ手順でメインディレクトリを指し直す。
...
3. **番号を採番し、worktree の環境ファイルへ書く。** 既存の行があれば置換し、無ければ
   追記する。**末尾への追記では上書きできない構成がある**（環境ファイルを先勝ちで読む）。
   採番の手順は [`test-execution.md`](test-execution.md) と共有する
...
6. **撤収はローカル環境の破棄を先、worktree の削除を後にする。** 順序を逆にすると定義と
   環境ファイルを失い、プロジェクト名を手で復元することになる
...
- コードの位置の切り替えは、同じローカル環境を共有する他のセッションにも及ぶ。排他の仕組みは持たない
