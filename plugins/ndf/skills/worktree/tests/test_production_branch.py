...
**開発の起点と本番のチャネルは別物である。** `base_branch` は作業ツリーの分岐元を
決める鍵（`follow_branch: true` のときは主ディレクトリの追従先にもなる）で、この
リポジトリでは `develop` を指す。本番のチャネルとして定めるのは `main` であるため、
**流用すると開発版のチャネルへのマージが本番の扱いになる**（決定 10）。
