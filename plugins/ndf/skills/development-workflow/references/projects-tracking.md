...
| 通過工程の控え | 位置を順に探し、`stages` の値と `mode` の値を書き換える |

**控えは実行のたびに作り直されない。** 課題ごとに 1 つのファイルが残り、記録のたびに
`stages` へ 1 件ずつ積む。旧い名前のまま残すと、**実際に通った工程が「記録なし」として
出る**。廃止したモード名が残るとさらに広く、モードが解決できないため**欠落の検知そのものが
働かない**（`mode` が旧い値の控えでは必須の工程が 1 つも挙がらない。2026-09-07 に実測）。

**控えは事象の記録であるため、新しい工程を挿入しない。** 課題の本文の `## 進行` の節は
工程の一覧であり、足りない行を足す。控えは「記録が書かれた工程の並び」であり、通っていない
