...
**値は工程表の行名と一致させる。** 綴りの違う値を書き込むと、ボードの側に工程表に無い値が増える。
スクリプトは一覧に無い値を弾く。
...
| 書き換える先 | 手段 |
| --- | --- |
| ボードの単一選択 | 利用者が値を足し、既存のアイテムを付け替える |
| 課題の本文の `## 進行` の節 | 節の中の工程の一覧だけを差し替える。**節の外は書き換えない** |
| 通過記録 | 位置を順に探し、`stages` の値と `mode` の値を書き換える |

**通過記録は実行のたびに作り直されない。** 課題ごとに 1 つのファイルが残り、記録のたびに
`stages` へ 1 件ずつ積む。旧い名前のまま残すと、**実際に通った工程が「記録なし」として
出る**。廃止したモード名が残るとさらに広く、モードが解決できないため**欠落の検知そのものが
働かない**（`mode` が旧い値の通過記録では必須の工程が 1 つも挙がらない。2026-09-07 に実測）。

**通過記録は事象の記録であるため、新しい工程を挿入しない。** 課題の本文の `## 進行` の節は
工程の一覧であり、足りない行を足す。通過記録は「記録が書かれた工程の並び」であり、通っていない
工程を書き加えると、通ったことにならない工程が通ったものとして数えられる。

置き場所と書式は [stage-completeness.md](stage-completeness.md) の「置き場所」にある。
**その版で何を何へ書き換えるかは、更新の案内が持つ。** ここに置くと、版ごとの置換の一覧が
この文書へ積み上がる。
...
引用せずに呼ぶと引数のチェックで終了コード 2 になり、ボードには何も書き込まれない。空白を含まない値も
同じ形で書き、呼び方を 1 つに揃える。
...
ボードは組織単位で持てる。`devbasex` のボードには ai-plugins / devbase / devbase-samples の
...
- `gh` または `jq` が無い
- ボードへの問い合わせや更新が失敗した（権限不足を含む。`project` スコープが要る）
- いま開いているリポジトリの名前を取得できない（上の「対象のアイテムの選び方」）
- ボードに対象のアイテムが無い（取得が上限で切れた場合を除く。下の「既知の制約」）

**呼び出し側の誤りだけは 2 を返す。** 知らないキー・工程表に無い値・引数の不足がこれにあたる。
引用を落として空白を含む値が分割された場合も、引数の数が合わなくなるためここへ入る。
黙って進むと、綴りの違う値がボードへ入るか、書き込んだつもりの値が入らない。

## 既知の制約

### アイテムの取得は 1000 件で切れる

`projects-sync.sh` は `gh project item-list --limit 1000` でボードを読む。閉じたアイテムも
残るため、長く使うボードでは総数がこの上限を超えうる。超えると、ボードに載っている issue でも
対象が見つからない。

見つからない理由は 2 つあり、ボードへ登録していない場合と、取得が上限で切れた場合である。
前者は正常な状態なので黙って抜ける。**ボードの総数が上限を超えるときだけ、標準エラーへ知らせる。**
総数は `gh project item-list --format json` の `.totalCount` が持ち、`--limit` の値に左右されない。
取得した件数を上限と比べると、ちょうど上限と同じ件数のボードで、切れていないのに知らせてしまう。

```console
NOTE: ボードのアイテムは 1200 件あり、取得の上限 1000 を超えています。#186 が見つからないのは取り漏れの可能性があります
```

**終了コードは 0 のままである。** 進行管理が理由で工程を止めない。上限に達するボードでは、
`--limit` の値を上げるか、ボードを分けることになる。
...
このため、**ボードから現在の値を読み返す用途にこの出力を使わない**。フィールドの名前と
