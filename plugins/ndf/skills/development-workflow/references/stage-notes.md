...
起きていない。`release` は 3 モードすべてで通す。要否がモードで変わらないのは、配布が要るか
...
設計は 3 モードのうち 2 つで `design` を通る。作る文書と必須の節はモードで変わり、その振り分けは
`design` が持つ。`light` だけが工程ごと対象外である。

**要求と受け入れ条件は 3 モードのうち 2 つで通る。** `light` にも掛かるのは、**モードを判定
する単位が Pull Request** であり、そこに何が入るかが判定の入力になるためである。掛からない
のは `legacy-refactor` だけで、こちらは受け入れ条件の代わりに現状の振る舞いを基準にする。
