...
`ndf-next` のブロックを出さない普段の利用では、印が書かれないのでラッパーは何もせず、claude を終えると同じ
終了コードでシェルへ戻る。
...
| 手段 | 効き目 |
| --- | --- |
| `python3 ~/.claude/ndf/relay.py stop`（別の端末から） | 動いているラッパーすべてに停止の印を置く。次の印を受けても `/exit` を入力しない。動いているラッパーが無ければ終了コード 1 |
...
| 静止（印・会話の記録・利用者の入力が動かない秒数。目標が未達の判定の後は会話の記録を数えない） | 5 | `NDF_RELAY_QUIET` | この秒数がたつまで `/exit` を入力しない |
...
| 目標が未達のとき | 判定が止めを拒んで応答が続く。カットポイントでは未達が当然なので、ラッパーは切り替える。印の後に目標の判定（`goal_status` の `met: false`）の行があれば、会話の記録の更新を静止に数えず、利用者の入力の静止だけを待つ。Esc を 1 回書いて応答を止め、1 秒おいて `/exit` を書く。利用者の入力・質問・背景の処理の起動があれば切り替えない |
