...
次のコマンドの貼り付け」を、ラッパー（`scripts/relay.py`）が行う。** 人が入力するのは関門の答えだけになる。
...
`claude` と打っても、次の起動はラッパーを挟まずに本物の claude をそのまま exec する（素通し）。
...
名前と版を読めないときは、`ndf-relay: ラッパーを始めない（<理由>）…` の 1 行を出してから素通しする。

`ndf-next` のブロックを出さない普段の利用では、合図が書かれないのでラッパーは何もせず、claude を終えると同じ
終了コードでシェルへ戻る。
...
| 手段 | 効き目 |
| --- | --- |
| `python3 ~/.claude/ndf/relay.py stop`（別の端末から） | 動いているラッパーすべてに停止の合図を置く。次の合図を受けても `/exit` を入力しない。動いているラッパーが無ければ終了コード 1 |
| `touch <作業ディレクトリ>/stop` | 同じ（1 つのラッパーだけ） |
| 区間の中で `/exit` か Ctrl-C を 2 回 | ラッパーも次の区間を起動せずに終わる |
...
claude が再開用のコマンドを `ndf-next` のブロックで出して応答を終え、ラッパーが静止を待ってから
...
| 合図の後に質問が出た | `PreToolUse` hook が質問の時刻を `asked` へ書く。合図の `written_at` 以後なら書かず、次のブロックの無い Stop が合図を消す |
| 合図の後に利用者が入力した・背景の処理を起動した | 会話の記録に、合図より後の利用者の入力の行か、`run_in_background` が真の Tool の呼び出しの行がある。次の Stop が合図を書き直すまで待つ |
| 合図の後に応答が再開した（目標が未達の判定が無いとき） | 会話の記録に、合図より後の `assistant` か `user` の行がある。次の Stop が合図を書き直すまで待つ |

**合図を書いた後は、切り替えを確定とする。** ブロックの無い Stop は、背景の処理が動いているときと
合図の後に質問が出たときだけ合図を消し、それ以外は前の合図を残す。
...
- 書いた `/exit` が質問の答えの後に働く形になったら、質問が消えるまで SIGTERM までの秒を数えない。子が終わった後は合図を読み直し、無ければ次の区間を起動しない
...
| 上限 | 既定 | 変え方 | 超えたとき |
| --- | --- | --- | --- |
| 1 日の起動回数（全部のラッパーの合計） | 20 | `NDF_RELAY_MAX_STARTS` | 次の区間を起動しない |
| 空回り（区間が 3 つ続けて、起動から 120 秒未満でカットポイントに達した） | — | — | 3 つ目のカットポイントで次の区間を起動しない |
| 静止（合図・会話の記録・利用者の入力が動かない秒数。目標が未達の判定の後は会話の記録を数えない） | 5 | `NDF_RELAY_QUIET` | この秒数がたつまで `/exit` を入力しない |
...
（supervisor を含む）が動いているあいだの応答では合図を書かない。
...
**本番の配布の後は、カットポイントの 3 つを本番の queue と同じ背景の Bash で続けて流す。** 関門 2 の承認から次の区間の起動までに、conductor が組み立てる文は無くなる。conductor は完了の通知を受けたら `relay.py notice` の告知と、出力の `ndf-next` の囲みをそのまま出す。ラッパーがプラグインを本番の版へ更新し、次の区間を起動する。
...
**次の区間を止めずに続けるには、`/goal` の雛形を特定の課題に縛らない。** 雛形には「効果の順の残りから次のミッションを選び、同じチェインで流し、最後に同じ雛形で `ndf-next` を出す」ことを書く。止まるのは関門 2 つだけになる。
...
ラッパーが次の区間を起動しないと決めたときは、`ndf-relay:` で始まる 1 行が画面に出る。

| 1 行 | 状態 | 続け方 |
| --- | --- | --- |
| `次の区間を起動しない（<理由>）。このまま続けるか、/exit して示されたコマンドを手で入力する` | 上限・空回り・停止の合図。今の区間は動いたまま | そのまま続けるか、`/exit` してから conductor のブロックの中身で `claude` を起動する |
| `次の区間を起動できない（<理由>）。次のコマンド:` の後に中身 | 更新か起動に失敗し、ラッパーは終わった（終了コード 2） | 表示された中身で `claude` を起動する |

## fork したセッションで進めたとき

例: ラッパーが区間 2 を子 pid 1883（セッション `ba764798`）で起動し、利用者が別の入口から同じ会話を
...
- Stop hook の `mark` は合図を書かない。`ndf-next` のブロックを出しても区間は切り替わらない
...
| `event` | いつ | 主なキー |
| --- | --- | --- |
| `start` | 区間を起動した | `section`・`pid`・`command`・`from_session`・`plugin_version`（起動の直前に読んだ版）・`cwd`（合図の作業ディレクトリが消えていたら `cwd_fallback` に元の値）・`carried`（2 つ目以降の区間だけ。ブロックの中身の前に付けた引数） |
| `end` | 区間が終わった | `seconds`（起動から合図まで。合図なしなら終わりまで）・`ended_by`（`mark` / `no-mark` / `sigterm` / `sigkill`） |
...
| 目標が未達のとき | 判定が止めを拒んで応答が続く。カットポイントでは未達が当然なので、ラッパーは切り替える。合図の後に目標の判定（`goal_status` の `met: false`）の行があれば、会話の記録の更新を静止に数えず、利用者の入力の静止だけを待つ。Esc を 1 回書いて応答を止め、1 秒おいて `/exit` を書く。利用者の入力・質問・背景の処理の起動があれば切り替えない |
