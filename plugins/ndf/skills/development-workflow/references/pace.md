...
**前回の検査は、origin のブランチ `check-done/review`（実装レビュー）と `check-done/check`（構造改善を含む
検査）が持つ。** `record` は結果が `merged` か `no_change` のとき、見終えた位置（`to`）をこのブランチへ送る
（構造改善を含む検査は両方、`--review-only` は `check-done/review` だけ）。その位置が次の範囲の `from` になり、
時刻が期限の起点になる。ブランチが無ければ、手元の記録の最新の行、`--since <ref>`、配布の宣言（`.ndf/supervise.json`
の `release`。`package-plugin` なら `<plugin>--v*`）が決める最新の正式版のタグ、起点のブランチとの分岐点の順に使う。

**見終えた位置を origin に置くのは、検査を通らずに配布された変更を次の検査で必ず見るためである。** 手元の記録は
置き場が消えれば失われ、別のマシンからは見えない。失われたまま正式版のタグへ戻ると、トリガーが立たずに配布された
Pull Request が範囲から外れる。ブランチはリポジトリにあるため、PR のマージの仕方（merge / squash）にも依らない。
**初めて使うリポジトリでは、どこまで見たかを渡す**（計画は `new check --since-last --since-ref <ref>`、直接なら `eval --since <ref>`。渡さなければ最新の正式版のタグから）。
