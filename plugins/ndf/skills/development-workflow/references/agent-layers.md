...
| 返す形 | `## 作業の報告` の 5 項目 |
| 置き場所 | 長い出力と**報告の写し**を書くファイルの絶対パス（報告にはパスだけを載せる）。**worker は最後の応答の前に、長い出力の有無に依らず `## 作業の報告` の節をこのファイルの末尾へ同じ中身で書き、書き終えた後に空のファイル `<置き場所>.done` を作る（`: > "<置き場所>.done"`）。** supervisor は「無し」にせず、worker ごとに新しいパスで空のファイルを作ってから（`: > <置き場所>`）渡す（[waiting.md](waiting.md) の「途中の通知を受けたとき」） |
