...
5. 待ち方は [waiting.md](waiting.md) に従う。背景の処理を残したまま応答を終えない。`Monitor` で待つときも同じで（`Monitor` の通知で再開はされるが、supervisor には途中の通知が届く）、
   supervisor が `置き場所` を渡したときは、長い出力が無くても報告の写しを書き、その後に `<置き場所>.done` を作る
...
| 置き場所 | 長い出力と報告の写しのファイルの絶対パス。supervisor が置き場所を渡したときは、長い出力の有無に依らずそのパスを書く | `無し`（supervisor が置き場所を渡さなかったときだけ） |
