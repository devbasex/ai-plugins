...
ため、メインディレクトリのままで済む。開発の変更は clone したディレクトリではなく
`.worktrees/<ブランチ名>` の worktree の中で行い、判定が済んでから用意する。後から移すと、
メインディレクトリに変更が残ったまま並行作業が始まる。`light` でも、本番コードの文言やテストを
...
**リファクタリングとコードレビューは、通す工程であって任意ではない。** `standard` と `legacy-refactor` の
...
次の工程を始める引継ぎの 1 行（`/ndf:development-workflow #<課題>`。今のセッションを `/goal` で始めていたときだけ先頭に `/goal `）を、
...
この Skill の frontmatter の `hooks` が、進捗記録のコマンドを通過工程として積み（記録の無い工程は案内するだけで
...
| 本番の系へ届く操作 | リリースの工程、および `operation` の実装の工程 | **制作物承認** | **届く先が本番の系かどうかで決まる** |
...
**承認するのはリリースの可否ではなく、この設計で実装へ進んでよいかである。** マージ先が
...
| 何が届くか | 何で判定するか |
| --- | --- |
| リリース（マージと公開） | マージ先が本番のチャネルか。**チャネルは系の一種である** |
...
のときはメインディレクトリの追従先にもなる）。このリポジトリの値は `develop` であり、本番のチャネルと
...
- 設計 Pull Request のマージ（`standard`）— 承認を得るまで実装の工程へ
  進まない
- 本番の系へ届く操作 — 承認を得るまで進めない。**検証リリースまでは自動で進めてよい**。
...
- [references/pace.md](references/pace.md) — 進め方 `pace: fast` の条件・宣言・プランのステージ・検査のトリガー・MVV 判定
