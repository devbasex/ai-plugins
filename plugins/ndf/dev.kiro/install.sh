...
# Usage: bash plugins/ndf/dev.kiro/install.sh [--project PATH] [--scope workspace|global]
#                                            [--set-default] [--yes]
#                                            [--with-slack] [--with-codex] [--dry-run]
#
# このスクリプトは Agent Plugins 仕様 §8.2 のクライアント拡張ディレクトリ（dev.kiro/）に
# 置く。プラグインルートは 1 つ上の階層で、Skill と scripts はそちらから読む。Kiro CLI 固有の
# エージェント定義とプロンプトだけがこのディレクトリにある。
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
PLUGIN_DIR="$(cd "$SCRIPT_DIR/.." && pwd)"
AGENT_NAME="ndf"
TEMPLATE_FILE="$SCRIPT_DIR/agents/$AGENT_NAME.json.template"
PLUGIN_SKILLS_DIR="$PLUGIN_DIR/skills"
PLUGIN_PROMPTS_DIR="$SCRIPT_DIR/prompts"
POLICY_SKILL_FILE="$PLUGIN_SKILLS_DIR/ndf-policies/SKILL.md"
SKILL_MANIFEST="$PLUGIN_DIR/manifests/kiro-skills.txt"
MANIFEST_FILE="$PLUGIN_DIR/.claude-plugin/plugin.json"
# 版数はルートのマニフェストだけが持つ。Kiro 向けに複製した VERSION ファイルは置かない
...
while [ "$#" -gt 0 ]; do
  case "$1" in
    ...
    --help|-h)
      echo "Usage: bash plugins/ndf/dev.kiro/install.sh [OPTIONS]"
      echo ""
      echo "Options:"
...
# 掃除するのは現在の checkout（$PLUGIN_SKILLS_DIR 配下）を指す既存リンクだけに限る。
# 別の場所を指す利用者のリンクは残す。旧 installer が別 checkout から張ったリンクは
# ここでは掛からず、後段の migrate_policy_skill_entry などが名前単位で扱う。
...
# 旧 installer が別 checkout から張ったリンクは remove_managed_skill_links の掃除
# （現在の $PLUGIN_SKILLS_DIR 配下を指すものだけ削除）に掛からないため、ここで
...
# manifest 掲載の Skill を .kiro/skills/ へリンクする。張った本数はグローバルの
# SKILL_COUNT へ明示して返し（標準出力は "  linked:" 等の案内だけに保つ）、呼び出し側は
# その変数を読む。ndf-policies は migrate_policy_skill_entry に委ね、リンクは張らない。
install_manifest_skills() {
  SKILL_COUNT=0
  local src_dir skill_name
  while IFS= read -r src_dir; do
    ...
    if [ "$skill_name" = "ndf-policies" ]; then
      migrate_policy_skill_entry "$skill_name"
      continue
    fi
...
# --set-default の各段階を、入出力を明示した関数へ分ける。呼び出し側は下の
# --set-default ブロックで順に呼び、標準出力・標準エラー・終了コード・kiro-cli を
# 実行するディレクトリは分割前と変えない。

# scope から kiro-cli を実行するディレクトリを決め、グローバルの KIRO_CWD へ返す。
...
# KIRO_CWD で kiro-cli agent list を実行し、現在の既定エージェント名を標準出力へ返す。
# kiro-cli 2.16.1 の agent list は一覧を標準エラー出力へ書くため 2>&1 でまとめ、
# ANSI エスケープを除いてから先頭が "*" の行の 2 番目の語を取る。
kiro_default_agent() {
  local esc
  esc="$(printf '\033')"
...
# 利用者に既定エージェントの変更可否を尋ねる。--yes・非対話のときは確認を省いて
# 承認とみなす。承認なら 0、拒否なら 1 を返す（呼び出し側の proceed 判定に使う）。
confirm_set_default() {
...
# 既定エージェントを $AGENT_NAME へ変更し、agent list で反映を検証する。
# 引数は復帰案内に使う変更前の既定エージェント名。検証に失敗したらエラーで終了する。
apply_set_default() {
  local previous_default="$1"
  ...
  echo "既定エージェントを $AGENT_NAME に変更しました（元に戻す: kiro-cli agent set-default ${previous_default:-kiro_default}）"
}
...
header = (
    ...
    "<!-- 編集元: plugins/ndf/skills/ndf-policies/SKILL.md -->\n"
)
...
if [ "$SET_DEFAULT" = true ]; then
  ...
  if confirm_set_default; then
    apply_set_default "$current_default"
  else
...
echo ""
echo "前提: NDF は 1M コンテキストのモデルを対象に配布 Skill を組んでいます。"
echo "  起動時に読み込む文脈は $SKILL_COUNT 個の SKILL.md と AGENTS.md / README.md / 常時指示です。"
echo "  既定の auto はこの条件を満たします。モデルを固定する場合も 1M のものを選んでください。"
echo ""
