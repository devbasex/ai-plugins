...
基準を満たさないコミットは取り込まず、**その改善項目の指摘を解決済みにしない**。
適用側だけ厳しくして修正側を素通しにすると、「レビュー指摘への対応」という名目で
手順を外れた変更が入り、そのまま収束済みになる。

さらに、**どの改善項目のものか特定できない不正コミットが 1 件でもあれば、解決の
申告を一切採らない**。項目を特定できない不正は「その項目の話」として扱えないため、
指摘側の ID と突き合わせる方式では素通りしてしまう。
