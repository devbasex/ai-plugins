...
| 開発版 | `10.8.0` | 検証中。入れたくない利用者は取得を控えられる |
...
- 順序は semver に従い `10.8.0` < `10.8.0-rc.1` < `10.8.0` になる
...
**NDFプラグイン**は、このマーケットプレイスの主要プラグインです（v10.8.0）。plugin 名は全ランタイムで `ndf` を維持し、配布物は `plugins/ndf/` の1ディレクトリにまとまっています。
