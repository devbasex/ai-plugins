...
- Fix: 配布の PR のチェックを 5 秒おきに読み直し、通ってからマージまでの遅れを縮める（#1052）
- Add: 1 ミッションを queue --then の鎖にし、supervise.py wait で関門だけ conductor が起きる（#1053）（#1056）
- Update: 配布の PR の CI を差分で絞り、develop で通ったコミットを main で試し直さない（#1055）（#1057）
- Add: 計画の指示文と配布の説明文を機械で組み、PR を全体テストの前に出す（#1054）（#1058）

## [ndf 10.17.16] - 2026-09-25
...
## [ndf 10.17.15] - 2026-09-25

### 追加

- **`supervise.py queue` に `--then` と `--done` がある**。`--then` は前の計画がすべて完了したときだけ後続の計画を続けて流し、
  1 本でも止まった・関門に当たったときは流さずに理由を残す。`--done` は queue の終わりに結果の JSON をファイルへ書く。
  queue の終わりはこのファイルを待ち、プロセス名（`pgrep -f`）では待たない
- **中継の下で ndf-next を出したとき背景の作業が残っていれば、Stop hook が 1 度だけ止めて知らせる**。
  知らせには作業の id とコマンドと、`TaskStop` での止め方が載る
- **印を書かなかった Stop は中継の `log.jsonl` に `mark_skipped` として残り、`experimental/resume.py` が並べる**

### 変更

- **corder・qa エージェントと qa-security-scan の Codex 連携は `external-ai.py run` の 1 行で、起動・上限つきの待ち・回収を行う**

### 修正

- **`merged-steps.py merge-when-green` は、実行が終わっているのに GitHub 側で pending のまま取り残されたチェックを見分け、
  そのジョブを 1 度だけ再実行する**。再実行でも動かなければ止まって知らせる
- **ランナー待ちの間は待ち行列の件数を出し、`supervise.py` の alive の行に run の段の最後の出力が `last_output` として載る**
