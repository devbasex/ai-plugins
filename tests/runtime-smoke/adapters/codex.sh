...
"$assertions/assert-plugin-files.sh" codex
"$assertions/assert-mcp-config.sh" codex "$REPO_ROOT/plugins/mcp/mcp-bigquery/.mcp.json"
"$assertions/assert-hook-fixtures.sh" codex
"$REPO_ROOT/tests/runtime-smoke/fixtures/hooks-positive-control/assert-characterization.sh"
"$assertions/assert-authenticated-smoke.sh" codex
"$assertions/assert-hook-definitions.sh" codex
"$assertions/assert-no-host-contamination.sh" codex
write_junit codex
