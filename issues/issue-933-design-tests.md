...
| 項目 | 内容 | いつ決まるか |
| --- | --- | --- |
| Jev の応答の形 | #841 の本文の試験（2026-09-22）から書いた。**2026-09-24 に実測で確かめた**（`questions` の辞書で問い、`answers.<名前>` に `boolean` は `probability`、`score` は `probabilities` と `confidence` が返る。実装計画の前提 2） | 確認済み |
