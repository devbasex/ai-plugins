...
文献、手元の実測データ、検証計画を残す。進捗は
[issue #158](https://github.com/devbasex/ai-plugins/issues/158) で追跡する。設計へ反映した結果は
[issue-113-cross-refactoring.md](old/issue-113-cross-refactoring.md) にある。
...
**統合後 26 件のうち、2 者以上が挙げたものは 4 件（15.4%）。** 残る 22 件（84.6%）は
1 者だけが挙げたものだった。採用された 19 件のうち、2 者以上が挙げたものは 4 件（21.1%）である。
...
見つけた改善が 84.6% を占めるなら、単一モデルで進めた場合に取り逃がす量は大きい。
...
| 問い | 交絡 | 分離の方法 |
| --- | --- | --- |
| 重複率 15.4% は独立性か探索空間の広さか | 対照条件が無い | 同一モデル・同一ハーネスを 3 プロセス並列で走らせて比較する |
...
1. **AI エージェント開発におけるアンサンブルの価値は、同じ結論への投票ではなく、
   ...
   手元の重複率 15.4% と、Wisdom and Delusion の理論上限 +83% が支持する
