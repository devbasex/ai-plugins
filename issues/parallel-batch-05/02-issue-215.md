...
- `plugins/ndf/skills/<Skill 名>/` の本文（他の担当が持つ）。**例外は「`$SCRIPTS` を決める」節の
  候補の一覧の 1 箇所で、これは担当 B が持つ**（`00-overview.md` の「解決手順の候補への agy の
  追加は担当 B が持つ」）
...
| A14 | `development-workflow/tests/test_projects_scripts_lookup.py` へ agy の枝を足す。既存の 3 ランタイムと同じ形で `tmp_path` に `~/.gemini/config/plugins/ndf/scripts` の配置を作り、節の bash を読み出して実行する |
