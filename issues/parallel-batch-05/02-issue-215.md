...
| A14 | agy へ導入した配置で「`$SCRIPTS` を決める」の解決手順を実行すると、`~/.gemini/config/plugins/ndf/scripts` が返る |

対象範囲に含まないもの:

- 参加 CLI としての agy（#214、担当 A が持つ）
- `plugins/ndf/skills/<Skill 名>/` の本文（他の担当が持つ）。**例外は 2 つで、どちらも担当 B が
  持つ。** 1 つは「`$SCRIPTS` を決める」節の候補の一覧（`00-overview.md` の「解決手順の候補への
  agy の追加は担当 B が持つ」）、もう 1 つは `worktree/tests/test_scripts_reference.py` への
  agy の配置の追記（同じ文書の「`worktree/tests/` は 3 担当が触る」）
...
| `development-workflow/references/projects-tracking.md` の「`$SCRIPTS` を決める」節 | 候補の一覧と対応表へ agy の導入先を足す。担当 C の持つファイルの中の 1 箇所（境界表の例外） |
| `development-workflow/tests/test_projects_scripts_lookup.py` | agy の配置を作る枝を足す |
| `worktree/tests/test_scripts_reference.py` | 担当 F が新設したファイルへ agy の配置を足す（`00-overview.md` の「`worktree/tests/` は 3 担当が触る」の例外） |
...
**「`$SCRIPTS` を決める」節へ触るのは担当 F のマージの後である。** 担当 F は既存の候補の並びを
前提に `worktree` の手順書を直す。マージの順序は F → A → B のため、順番どおりに進めれば衝突
しない。
...
| `list_dir` | `DirectoryPath` | 一覧の対象。誘導の対象にしない |
| `run_command` | `CommandLine`、`Cwd` | 実行するコマンドと、その実行ディレクトリ |

**書き込みの権限の名前は `write_file` で、この表の tool 名とは別である。** `01-issue-214.md` の
決定 11 が挙げる `write_file` は `--mode plan` の拒否の理由と `settings.json` の
`permissions.allow` に出る名前で、`PreToolUse` の `toolCall.name` には届かない。agy 1.1.24 で
同じ書き込みを 2 通りに測り、拒否の理由は `write_file`、hook が受け取った名前は `write_to_file`
だった。**`matcher` に書くのは後者である。**
...
| A14 | 2 つのテストへ agy の枝を足す。`development-workflow/tests/test_projects_scripts_lookup.py` は、既存の 3 ランタイムと同じ形で `tmp_path` に `~/.gemini/config/plugins/ndf/scripts` の配置を作り、節の bash を読み出して実行する。`worktree/tests/test_scripts_reference.py`（担当 F が新設）は、手順書と解決手順の対応を見る側で、同じ配置を足す。**候補を 1 つ足したのに片方のテストしか直さないと、もう片方が 3 ランタイム分のまま通り続ける** |
...
| `browser_*` などの tool 名 | 実測で観測できたのは `write_to_file` / `replace_file_content` / `view_file` / `list_dir` / `run_command` の 5 つ。ドキュメントには `edit_file` と `browser_*` の記載がある。編集を伴う tool を取りこぼした場合、その操作では誘導が出ない |
...
| `agy plugin install` で導入した実体を、こちらから更新する手段が見当たらない。`agy plugin uninstall` と `install` の組み合わせになる | `agy plugin --help` の一覧 | 起票済み（#289）。利用者が新しい版を受け取る手順の問題で、配布の入口（決定 1）とは別に決める必要がある |
