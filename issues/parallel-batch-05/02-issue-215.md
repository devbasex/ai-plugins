...
| A2 | `tests/runtime-smoke/adapters/agy.sh` が `agy plugin install` と `agy plugin list` を実行し、`assert-plugin-files.sh` の `agy` の枝が導入先の `plugin.json` / `skills/*/SKILL.md` / `hooks.json` の存在を見る |
