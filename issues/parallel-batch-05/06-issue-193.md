...
| 配布先ランタイム | Skill を配る先。この変更の時点では Claude Code / Codex / Kiro CLI の 3 つ。同じバッチで担当 B が agy を 4 つ目として加える |
...
| agy | `~/.gemini/config/plugins/ndf/scripts`（担当 B が候補へ足す） | 未導入。複製の先のディレクトリだけを確認した |
...
- [ ] 条件 4: 3 ランタイムの配置を作って解決手順を実行すると、`worktree-setup.sh` を持つ
      ディレクトリの絶対パスが返る。**agy の配置は担当 B が同じ形で足す**（後述の突き合わせ）
...
| 候補の一覧への agy の追加 | 担当 B の持ち物。agy を配布先ランタイムへ加えることに伴う変更をまとめて持つ（`00-overview.md` の「解決手順の候補への agy の追加は担当 B が持つ」） |
...
同じバッチで担当 B（#215）が agy を配布先ランタイムへ加える。**配布先が 4 つになると、解決手順が
試す候補も 4 つ要る。** agy へ導入した配置から `worktree` の手順を実行しても、候補が 1 つも
当たらないためである。導入先は `~/.gemini/config/plugins/ndf/scripts` になる（`02-issue-215.md`
の実測。`agy plugin install` が `dev.agy/scripts` の symlink を実体へ解決して複製する）。

**候補を足すのは担当 B である。** agy を配布先ランタイムへ加えることに伴う変更を担当 B が
まとめて持つ決まりで、`00-overview.md` の「解決手順の候補への agy の追加は担当 B が持つ」に
書いた。担当 F はこの変更を含まない。

**担当 F の設計は 3 ランタイム前提のままでよい。** 候補が 1 つ増えても、担当 F が置く 3 つの
前提（節の位置と見出し・変数名 `SCRIPTS`・決まらないときに空の値を残すこと）は崩れない。
条件 3（見出しが 1 つだけ）と条件 5（決まらないときに止まる）も、候補の数を見ていない。

**マージの順序は F → A → B である。** 担当 F のマージの時点では配布先は 3 つで、条件 4 の
テストも 3 ランタイム分でよい。担当 B が候補を足すときに、**担当 F が新設した
`worktree/tests/test_scripts_reference.py` へ agy の配置を足す。** 担当 B の指示書の A14 と
構成要素に、この追記を受け取る側として書いてある（`02-issue-215.md`）。既存のファイルは担当 A が
持つという決まりの例外にあたるため、`00-overview.md` の「`worktree/tests/` は 3 担当が触る」に
例外として明記した。**担当 F はこの追記を含まない。**
