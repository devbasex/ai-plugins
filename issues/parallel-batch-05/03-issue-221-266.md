...
`projects-sync.sh` は対象のアイテムが無いとき、何も出力せず終了コード 0 で終わる。
このバッチの課題のうち #221 は盤面に載っているが、#266 / #242 / #229 / #228 は載っていない
（4 件とも記録の呼び出しが何も書かずに終わった）。**盤面を唯一の根拠にすると、これらの課題では
検知が働かない。**
...
この設計を書いている間に GraphQL が利用上限へ達し、`gh pr view --json` が落ちた。
同じ値は REST で 1.0 秒で取れた。
...
2. 拒否は `gh pr merge` と REST（`PUT /repos/{所有者}/{リポジトリ}/pulls/{番号}/merge`）の両方の書き方で起きる
3. 承認の印が付いているときは通る
4. `design/` で始まらない Pull Request のマージは通る
5. リポジトリに承認の印が定義されていないときは拒否しない
6. 判定に要る値を取得できないとき（`gh` が無い・通信が失敗する・利用上限）は拒否しない
7. すべての tool を自動で通す設定（`bypassPermissions` / `--dangerously-skip-permissions`）でも拒否が効く
8. 拒否の理由に、**通すために何をするか**が含まれる
...
3 つ目は**何も出力しない**である。判定の対象でないコマンド、値を取得できなかった場合、
条件を満たした場合のすべてがここへ入る。

`permissionDecisionReason` は進行する AI へ渡る。理由だけでは次の一手が決まらないため、
通すための操作をそのまま書く。

```text
設計 Pull Request #268 は承認の印（ラベル design-approved）が付いていないためマージしません。
人間の承認を得てから、GitHub の画面でこの Pull Request に design-approved を付けてください。
付いた後で同じコマンドを実行すれば通ります。
...
| --- | --- | --- |
| マージの対象の番号 | コマンドの本文（`gh pr merge <番号>` / REST の経路の番号） | 現在のブランチ名で代替する |
| head のブランチ名 | `GET /repos/{所有者}/{リポジトリ}/pulls/{番号}` の `.head.ref`。番号が取れないときは `git branch --show-current` | 通す |
| 承認の印 | 同じ応答の `.labels[].name` に `design-approved` があるか | 通す |
| 印の定義の有無 | `GET /repos/{所有者}/{リポジトリ}/labels/design-approved` | 通す |
| 工程の値 | 進行の記録のコマンドの 3 番目の引数 | 控えへ書かない |
| モード | 同じコマンドの `mode` の値 | 必須の工程を判定しない |

**問い合わせは REST に限る。** 番号が取れているときは 1 回、印が無いときだけもう 1 回で、
判定の対象でないコマンドでは 1 回も呼ばない。
...
`projects-sync.sh` が何も書かないため、盤面側には記録そのものが生まれない
（このバッチでは少なくとも 4 件がこれにあたる）。記録を書く**コマンド**は盤面の有無にかかわらず
発行されるため、そちらを観測すると両方の制約を受けない。
...
#161 の実測では `レビュー` と `後片付け` が 2 回ずつ現れる。設計レビューの工程が
`pr` → `cross-review` → `merged` を呼ぶためで、工程表どおりの動きである。隣り合う 2 つの値を
比べる判定にすると、この動きが毎回ひっかかる。集合で見れば、抜けた `確定仕様化` だけが残る。

### 決定 3: 記録の無い工程は拒否せず案内として出す

必須の工程が記録されていないことは、その工程を通っていないことと同じではない。記録の側が
遅れているだけの状態でマージや配布が止まると、正当な操作が止まる。`worktree` の hook が
案内にとどめているのと同じ扱いにする。
...
`HTTP 422` になる。`.ndf/` 配下のファイルを印にする案は、印そのものがマージの対象に含まれる
ため、印を足すコミットとマージが同じ変更の中で完結する。ラベルは Pull Request の履歴へ
実行者付きで残り、マージの操作の副産物としては付かない。
...
範囲は `development-workflow` と `merged` であるため、承認の所在を 2 つに分けない。
`main` 宛の Pull Request は `scripts/check-pr-base.sh` が継続的統合で塞いでおり、そちらは
別の仕組みとして働き続ける。
...
発生した）。同じ値は REST で取れ、1.0 秒で返った。マージそのものも REST で行われることが
あるため、判定の対象とするコマンドの形も 2 つ拾う。

### 決定 10: hook は Skill の frontmatter で登録する

`development-workflow` は工程の入口であり、この Skill が呼ばれた会話の単位が、工程を通している
範囲とほぼ重なる。プラグイン全体の hook の定義（`plugins/ndf/hooks/`）へ置く案は、
そのプラグインを入れたすべての会話の単位に tool 実行前の判定を 1 つ増やすことになり、工程を
通していない場面でも走る。
...
| 起きうること | どう扱うか |
| --- | --- |
| `gh` / `jq` / `git` が無い | 何も出力せず通す |
| 問い合わせが失敗する（通信・権限・利用上限） | 通す |
| Pull Request の番号を取れない | 現在のブランチ名で見分け、それも取れなければ通す |
| 承認の印がリポジトリに定義されていない | 通す |
| `design/` で始まるが設計ではない Pull Request | 拒否する。理由にブランチ名を変える手順を書く |
| 控えが読めない・壊れている | 記録が無いものとして扱い、欠落を並べない |
| 進行の記録を書かずに工程を通した | 「記録が無い」として案内する。実施済みなら記録を書けば消える |
...
配布物の同期（`bash scripts/build-runtime-plugins.sh`）と、
`python3 scripts/check-skill-frontmatter.py` / `bash scripts/validate-runtime-plugins.sh` /
`uv run --with pytest pytest scripts/tests plugins/ndf -q` は完了判定の証跡として実行する。

## 担当をまたぐ調整

| 相手 | 何を調整するか |
| --- | --- |
| 担当 B（#215） | `scripts/check-skill-frontmatter.py` の許可する項目名へ `hooks` を足す。**この変更が無いと検査が落ちる**（実測済み）。`scripts/` は担当 B が持つ |
| 担当 F（#193） | Skill が持つスクリプトの位置の決め方。`merged` はいま `$SCRIPTS/../skills/merged/scripts/` の形で辿っている |

### 担当 F への提案

共通の置き場所を作るなら、プラグインの根を 1 つの値として持たせ、Skill が持つスクリプトを
`<プラグインの根>/skills/<Skill 名>/scripts/…` で指せる形にする。いまの `$SCRIPTS` は
プラグインの `scripts/` を指すため、Skill 側のスクリプトは 1 つ上へ戻ってから辿ることになり、
`merged` と、この変更で `stage-check.sh` を呼ぶ箇所の 2 つが同じ書き方を持つことになる。

**この設計は `development-workflow/references/projects-tracking.md` の
「`$SCRIPTS` を決める」の本文を変えない前提で書いている。** 担当 F が移す場合は、
参照先の位置だけが変わる。tool 実行前の hook は
`${CLAUDE_PLUGIN_ROOT}` を使うため、この解決手順を通らない。
...
| Codex / Kiro での扱い | `SKILL.md` は 3 ランタイムで同じ実体を読む。`hooks` の項目が Codex と Kiro でどう扱われるかは未確認。読み飛ばされない場合は、配布の対象を分ける手段を検討する |
| `${CLAUDE_PLUGIN_DATA}` | hook のコマンドの中で展開されるかは未確認。展開されない場合に備えて置き場所を 3 段に持つ |
| 拒否の実績 | 手順を書いた #265 の後に設計レビューが飛ばされた実測は無い。拒否へ踏み込む根拠は決定 8 の 2 点に置いている |
...
| このバッチの課題のうち少なくとも 4 件が盤面に載っていない。盤面へ載せる手順を持つ Skill が無い | 進行の記録の抜けは #221 の対象だが、盤面へアイテムを追加する操作はどの Skill の手順にも無く、別の変更になる | 起票する |
