...
9. 判断表が `out-of-scope` にあり、3 つの分類（工程と Skill の手順そのもの / 開発対象の
    コード・データ・設定・運用 / 両方）と、それぞれの起票先を持つ
10. 判断表はこのリポジトリの中で 1 か所にしかない。`retrospective` は参照だけを持つ
11. `retrospective` の落とし先の表の直後に、判断表への参照が 1 行ある
12. `out-of-scope` の手順に「起票先を決める」の段があり、「3 択で決める」の直後に来る
13. 重複の確認と起票が、決めた起票先に対して行われる（`gh` の呼び出しに `--repo` が付く）
14. 起票先のリポジトリの決め方が 3 段（環境変数 / 取得元の clone / 利用者に聞く）で書かれ、
    3 段目に達したときは推測で起票せずに止まる
15. 起票の前に同意を取る提示へ、起票先が含まれる
16. 両方にまたがる課題で、双方の番号が互いから辿れる手順がある
17. `uv run --with pytest pytest scripts/tests plugins/ndf -q` が通る
...
| 要素 | 責務 |
| --- | --- |
| `retrospective/SKILL.md` の「記録を残す」 | 投稿先を決め、コメントとして投稿し、本文へ 1 行を足す |
...
| 8 | `python3 scripts/check-markdown-links.py --root .` |
| 9 | 判断表の 1 列目が 3 種、2 列目が起票先であることを見るテスト |
| 10 | 判断表の 1 列目の値が `retrospective/SKILL.md` に現れないことを見るテスト |
| 11 | 落とし先の表の直後の行が `issue-target.md` への相対リンクを持ち、その先が実在することを見るテスト |
| 12 | `out-of-scope/SKILL.md` の手順の見出しの並びを固定するテスト |
| 13 | 判断表の参照の中の `gh issue list` と `gh issue create` に `--repo` が付くことを見るテスト |
| 14 | 3 段の表の行数と 1 列目を固定するテスト。段 3 の行に「止まる」ことが書かれているかはレビューで読む |
| 15 | 同意を取る提示の項目に「起票先」が含まれることを見るテスト |
| 16 | 両方にまたがる場合の手順の順序をレビューで読む |
| 17 | `uv run --with pytest pytest scripts/tests plugins/ndf -q` |
...
| `quality-gates` / `fix` / `pr-review` / `tdd-cycle` / `refactoring` / `problem-solving` の `out-of-scope` の案内 1 行に、起票先の言及が無い | 起票する | 6 Skill は担当 D の範囲外である。参照だけで足りるかを実運用で見た上で、足りなければ 1 行ずつ足す |
