...
`agy --help` は `--add-dir  Add a directory to the workspace (repeatable)` と書く。関係の無い
作業ディレクトリから、`--dangerously-skip-permissions` を付けずに 2 つ渡した（agy 1.1.24）。
...
後に渡した側が前を上書きすることはない。受け入れ条件 4 の置き場所は、3 の作業ツリーと
併記して渡せる。
...
### 決定 11: 編集系の tool 名の一覧は担当 A では変えない

`worktree-common.sh` の一覧には gemini の `replace` と `run_shell_command` が入っている。
**`write_file` は権限の名前で、hook に届く tool の名前ではない。** agy 1.1.24 で 2 つを別々に
測った。`--mode plan` の拒否の理由と `settings.json` の `permissions.allow` に出るのは
`write_file` で、`PreToolUse` の hook が受け取る `toolCall.name` は `write_to_file` だった。
**`matcher` が照合するのは後者である。** 一覧に `write_file` があっても agy の書き込みには
当たらない。当たる名前を足すのは担当 B で、`02-issue-215.md` の「入出力の契約」が持つ
（`write_to_file` と `replace_file_content`）。担当 A は注釈だけを直す。
...
| 見つけた場所 | 内容 | 判断 |
| --- | --- | --- |
| `plugins/ndf/skills/cross-review/scripts/lib/models.py` の `observed_model` | 実際に動いたモデル名を取れるのは claude だけである。agy の `--output-format json` にもモデル名は載らない（実測）。計測の目的に対して、4 CLI 中 1 つしか実測値を持てない | 起票済み（#284） |
| `plugins/ndf/skills/cross-review/scripts/lib/assignment.py` の置き場所 | 収束ループの共通層が `cross-review` の下にあり、`cross-refactoring` が相対パスで登っている（`launch-cli.sh` の 23 行）。Skill をまたぐ参照が片方向に固定される | 起票済み（#285） |
| `plugins/ndf/skills/pr-review/SKILL.md` の委譲の手順 | 同梱されていないランタイム向けの要点が、`external-ai` の参照と別に書かれており、2 箇所で同じ起動の約束を持つ | 起票済み（#286） |

3 件とも `out-of-scope` の手順で起票済みである。**実装担当が起票し直す必要は無い。**
