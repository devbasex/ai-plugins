# 工程の抜けを塞ぎ、事後レビューの指摘を直す（#418 / #417 / #420 / #422）
...
- [ ] B5: 控えの採番の鍵が Pull Request を指せる。1 つの Pull Request が複数の課題を
      閉じるとき、判定した 1 つのモードがその Pull Request に紐づく
- [ ] B6: 記録済みの控え（課題番号で採番したもの）を読めなくしない。移行の規則が
...
- [ ] C8: 今回の失敗が再現できる。`standard` を記録した課題を閉じる `gh pr create` に対し、
      必須の工程の欠落が案内に出る
- [ ] C9: Pull Request の控えがまだ無く、課題ごとの控えだけがあるとき、**全課題の控えから
      最も高いモードを取り**、それを基準に工程の充足を見る。記録が課題ごとに食い違うときも
      最も高いものを採る（B5 へ移行するまでの経路）
...
束ねる規約は置かないが、**モードの高さそのものは残る**（記録済みの控えの読み替えと、
`stage-check.sh` の報告で使う）。`_wf_mode_column` の戻り値で比較すると、`WF_MODES` の
...
| B5〜B6 | 新しいテスト。Pull Request を鍵にした控えを読み書きでき、課題番号で採番した既存の控えも読めること |
...
| C9 | 新しいテスト。Pull Request の控えが無く、課題ごとの控えだけがあるとき、最も高いモードを基準に案内が出る（移行の期間の経路） |
