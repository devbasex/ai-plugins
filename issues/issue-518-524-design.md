...
| 名前 | `ndf_git_credential_fallback_args` |
| --- | --- |
| 入力 | 無し |
| 出力 | 退避に使う `git` のオプションを 1 行 1 語で出す |
| 退避の形 | `git -c credential.helper= -c credential.helper='!gh auth git-credential' <引数...>` |
| 失敗の形 | 無し（値を出すだけで `git` を実行しない） |
| 互換性 | 新設のため既存の呼び出し側は無い |

**共通層が持つのは値だけで、退避の振る舞いは持たない。** 失敗→退避→1 度だけ再試行という
分岐は退避する側（`gitfacts.py`）が 1 か所で持つ。共通層にも同じ分岐を置くと、呼び出し側の
無いシェル関数が残り、**同じ振る舞いがシェルと実装の 2 か所に分かれて片方だけが直る余地が
残る**。値を 1 か所にするという目的は、値を出す関数だけで満たせる。
