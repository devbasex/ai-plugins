...
| 13 | 骨組みが、取り込みの終了コードで判定の手前に止まらない | `SKILL.md` の Step 2 の取り込みの 2 行に `\|\| true` がある |
...
| `SKILL.md` | 全体フローの図、Step 2 の監視の行（230 行）、Step 2 の取り込みの行（232 行 / 233 行）、Step 3 の判定の行 |
...
| `SKILL.md` の骨組み | 取り込みの終了コードは読まずに判定へ進み、判定の終了コードで分岐して、起動し直しを 1 度だけ行う |
...
```bash
  # Step 2 の取り込み: 終了コードは骨組みでは読まない。
  #   使える結果が無いときも、結果なしの記録を残して判定へ進める。
...
Step 2 の取り込みの行（232 行 / 233 行）には `|| true` を足す。**`cmd_read_result` は結果なしを
記録した後も、終了コード 1 または 3 で終わる。** この手順の bash に `set -e` は無いが、骨組みを
実行するのは AI であり、非 0 で終わった行を失敗として扱えば、判定と起動し直しの経路へ入らない
まま止まる。`|| true` は、この行の終了コードを読ませないための印でもある。起動し直しの中の
取り込み（上のコードの `for a in $RELAUNCH_AGENTS` の行）は既に `|| true` の形で、そちらと揃う。
骨組みの側を変えるため、`SKILL.md` のこの 2 行も担当 B の対象範囲に含める。

`|| true` は最終スイープの `verify-sweep`（`docs/02-fix-and-rotation.md`）とは逆向きの扱いに
なる。あちらは検証そのものが失敗したときに完了報告へ進ませないため、終了コードを読んで
`exit` する。取り込みは判定を行わないため、読む値は状態ファイルに残り、進む先を決めるのは
次の行の判定である。
...
### 決定 6: 取り込みの終了コードは変えず、骨組みの側で読み捨てる

`cmd_read_result` は結果なしを記録した後も、現在と同じ終了コード（無い・値を持たないときは 1、
JSON として読めないときは 3）で終わる。骨組みの取り込みの行に `|| true` を足し、判定へ進める。

**取り込みの行は、`--only` の指定があるときも非 0 で終わる。** 形が
`[ "$ONLY" != "gemini" ] && <取り込み>` であるため、条件が偽になれば取り込みを呼ばないまま
行のステータスが 1 になる。この行の終了コードは、指定の有無と結果の有無を区別できない。
**進む先を決めるのは次の行の判定である**（決定 5）。

記録したときの終了コードを 0 にする案は採らない。取り込みは、結果なしを記録した回も
成功と同じ値を返すことになる。骨組みは判定へ進むため動くが、記録・ログ・直接呼び出しの
いずれからも、使える結果が残ったかどうかが読めなくなる。既存の 1 と 3 が指す事象も、
結果なしを外れた別のものへ移る。

`SKILL.md` の 232 行と 233 行を書き換えるため、骨組みの側も担当 B の対象範囲に入る。
担当 C が触るのは `cmd_start_round` と作業ツリーの同期の説明で、行は重ならない。
...
- [ ] 骨組みの Step 2 の取り込みの 2 行に `|| true` があり、取り込みの終了コードは 1 と 3 のまま
