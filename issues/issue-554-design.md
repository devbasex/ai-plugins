...
**2 つ目は `ai-plugins` の `f56c90d9` で実際に出る形である**（AC60）。
...
検査は同じに動く（AC33）。**そのため、書き始めるのは配布のタグを打った後でよい** ── 定義ファイルを新設する
...
当たらない。**実装では 4 ランタイムの導入先それぞれから起動して確かめる**（AC56）。
...
対象の有無によらず「確かめられなかった」である（AC3 と AC46 の順序）。
...
**取れないときは終了コード 2 で止める。** 判定できないことを通過にしない（AC46）。
