...
**2 つ目は `ai-plugins` の `f56c90d9` で実際に出る形である**（AC71）。
...
検査は同じに動く（AC43）。**そのため、書き始めるのは配布のタグを打った後でよい** ── 定義ファイルを新設する
...
**`imports` のキーは、そのスコープの根からの相対パスである。** プロジェクトのスコープなら `--root` からの相対で、
根の指示書は `CLAUDE.md`、配下の指示書は `docs/AGENTS.md` と書く。**ユーザーとプラグインのスコープは、
宣言の `scopes` が挙げた位置からの相対で書く**（そこがそのスコープの根である）。**ファイル名だけにしない**
 ── 同じ名前の指示書が複数あると、許可が別の場所へも効く。
...
対象の有無によらず「確かめられなかった」である（AC3 と AC56 の順序）。
...
**取れないときは終了コード 2 で止める。** 判定できないことを通過にしない（AC56）。
