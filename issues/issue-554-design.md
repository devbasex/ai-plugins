...
`line` が `None` になるのは、許可の一覧の指し先が無い指摘だけである（ファイルの行ではなく検査の定数を指すため）。
...
| 指摘あり | 終了コード 1。標準エラーへ指摘ごとに 1 行。行を持つ指摘は `ERROR: <ファイル>:<行>: <理由>`、行を持たない指摘は `ERROR: <理由>`。例は下の表 |
...
**行の種類で 2 つに分ける**（決定 4）。分岐は行頭が `#` 1〜6 個と空白かどうかだけで決まり、1 つの行が両方に当たることは無い。

| 行の種類 | 見る位置 | 版数として拾う形 |
| --- | --- | --- |
| 見出しの行（`#` 1〜6 個と空白で始まる） | 見出しの文字列の**どこでも** | `v<版数>`。直後が ` の次の版` なら次の表の 2 行目、それ以外は 1 行目の条件で見る |
| それ以外の行（本文・箇条書き） | **書き出しだけ**。任意の箇条書き記号（`- ` / `* `）と任意の強調（`**`）を読み飛ばした直後 | `v<版数> で` または `v<版数> の次の版で` |

| 形 | 出た版として指摘する条件 | 例（最新が 10.14.0） |
| --- | --- | --- |
| `v<版数> で`（見出しでは `v<版数>`） | 版数の基底（接尾辞を除いた 3 つの数）が最新**以下** | `v10.14.0 で` / `v10.14.0-dev.1 で` / `v9.8.0 で` / `## v10.5.0 の判断` は指摘、`v10.15.0 で` / `v10.15.0-dev.1 で` は通す |
| `v<版数> の次の版で` | 版数の基底が最新**未満** | `v10.14.0 の次の版で` は通す、`v10.13.0 の次の版で` は指摘 |

**見出しの行で書き出しを問わないのは、見出しが節全体の版を名指すためである**（AC6）。本文の行で書き出しに限る理由は決定 4 にある。
...
`@` の直前が**行頭・空白・`*`・`_`・`~`** のどれかであるものを参照とする。

**参照の名前は、`@` の直後から続く ASCII のパスの文字（英数字と `.` `_` `-` `/` `~`）の並びである。** 並びの外の文字
（空白・`*`・句読点 `。` `、`・括弧）で終わる。並びの末尾の `.` / `_` / `~` は落とす（`**@AGENTS.md**。` も `__@AGENTS.md__` も名前は `AGENTS.md`）。
並びが空（`@` の直後が日本語や空白）なら参照としない。
...
| 大項目 | 要求の条件 | 実現方式 | 確かめ方 |
| --- | --- | --- | --- |
| 運用・保守性 | 指摘は `ERROR: <ファイル>:<行>: <理由>`（行を持たない指摘は `ERROR: <理由>`）の 1 行で、直し方を含む。外部への通信・CLI の認証を要さない | 入出力の契約の表の形で出す。読むのはローカルのファイルだけ | テストが出力の行と文言の要素（行番号・版数・退避先）を見る |
...
| AC17 | テスト: `@docs/foo.md。` が名前 `docs/foo.md` として指摘され、`**@AGENTS.md**。` は通る |
| AC18 | テスト: スクリプトを `importlib` で読み込み（`test_check_markdown_links.py` と同じ形）、`allowlist_findings` に存在しない指し先を持つ許可の辞書を渡して指摘が 1 件返る。**`AGENTS.md` を消す形では確かめない**（`AGENTS.md` は指示書でもあるため、AC20 の終了コード 2 が先に出る） |
| AC19 | テスト: `CHANGELOG.md` が無い根と、`## [ndf …]` の節が無い根で終了コード 2 |
