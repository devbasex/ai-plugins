...
| AC31 | 手順: `release` の差分と、新設した参照を読む。`$SCRIPTS` を解決してから呼び、末尾で終了コードを戻す 4 行があることを見る |
| AC32 | 手順: 4 ランタイムの導入先それぞれからスクリプトを起動し、一時リポジトリを `--root` に渡して終了コードを記録する。**手順に書いたブロックをそのまま実行し、検査が 0 / 1 / 2 / 3 を返す 4 通りでブロックの終了コードが一致すること**も確かめる |
