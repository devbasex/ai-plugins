...
**この 2 つ目は `ai-plugins` の `f56c90d9` で実際に出る形である**（AC30。判定規則の試作を同じ状態へ通し、
...
| それ以外の行 | **段落の先頭行だけ**（直前が空行・見出し・フェンスの終わりのいずれか） | 書き出し（箇条書き記号と強調を読み飛ばした直後） | 同上 |
...
**取れないときは終了コード 2 で止める。** 判定できないことを通過にしない（AC17）。
...
| # | すること | 受け入れ条件 |
| --- | --- | --- |
| 1 | `.ndf/instructions.json` を置く（「データ構造」の例のとおり） | AC29 / AC30 |
| 2 | `.github/workflows/runtime-plugin-validate.yml` へジョブ `instruction-files-check` を足す。Pull Request では絞り込まずに起動し、push の絞り込みへ `CHANGELOG.md` と `.ndf/**` を足す | AC31 / AC32 |
| 3 | `CLAUDE.md` の「版ごとの判断の記録」へ、版が決まる前の印と検査のコマンドを書く | AC33 |
| 4 | `docs/versioning-and-distribution.md` の「バージョン更新時の手順」へ検査を足し、手順 4 の置き場所を直し、「必須の検査 11 個」の数を直す | AC34 / AC35 |
| 5 | `CHANGELOG.md` の冒頭の置き場所を直す | AC36 |
...
テストから読まない。**実例の再現（AC30）は実装の Pull Request の本文へ手順と出力を残す。**
...
| AC13 / AC15 | テスト: `changelog` の宣言と、最新以下・最新より新しい版の段落 |
| AC14 | テスト: 出た版の版数で始まる行を段落の 2 行目へ置き、指摘されない（`f56c90d9` で 9 件に増える形） |
| AC16 | テスト: `tags` の宣言でタグを 2 つ打ち、最大値が最新になる |
| AC17 / AC21 | テスト: 版を取れない宣言、壊れた JSON、未対応の `version` で終了コード 2 |
| AC18 | テスト: `pending_marker` を宣言し、基底が最新・最新未満の 2 通り |
| AC19 | テスト: 指摘の文面に `decisions` の値が出る |
| AC20 | テスト: `budget.bytes` を超える指示書で終了コード 1 |
| AC22 / AC23 / AC24 | テスト: 出力の形、指摘が無いときの標準出力、知らない引数で終了コード 3 |
| AC25 | テスト: `plugins/ndf/skills/` に新しいディレクトリが無いことを、配る先の一覧の行数で見る |
| AC26 | 手順: `release` の差分と、新設した参照を読む |
| AC27 / AC28 / AC37 | 手順: 既存の検査とビルドの確認を実行し、終了コード 0 を見る |
| AC29 / AC30 | 手順: このリポジトリの根での実行と、`git archive f56c90d9 …` を展開した一時ディレクトリでの実行 |
| AC31 / AC32 | 手順: ワークフローの差分を読み、実装の Pull Request の checks を見る |
| AC33 / AC34 / AC35 / AC36 | 手順: 差分を読む |
| AC38 | 手順: `uv run --with pytest pytest scripts/tests plugins/ndf -q` の失敗の件数が `develop` と同じ（既知の環境要因の 6 件を除く） |
