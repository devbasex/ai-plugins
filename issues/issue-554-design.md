...
**2 つ目は `ai-plugins` の `f56c90d9` で実際に出る形である**（AC75）。
...
| `scopes` | オブジェクト | 無し | `--scope user` / `--scope plugins` で見に行く先。**`user` は位置の文字列の配列**、**`plugins` は位置と配布元を持つオブジェクトの配列**（項目の正本は [issue-554-design-scope.md](issue-554-design-scope.md) の「起票先と、書き込めないときの退避」）。**無ければそのスコープは走査されない** |
...
対象の有無によらず「確かめられなかった」である（AC3 と AC60 の順序）。
...
**取れないときは終了コード 2 で止める。** 判定できないことを通過にしない（AC60）。
