...
**終了コードを潰さない。** `コマンド; echo "exit=$?"` と 1 行に書くと、その複合コマンド自身の終了コードは
`echo` の 0 になり、**自動で実行する側が 1 / 2 / 3 を成功として通す**。値を変数へ受けてから表示し、
判定にはその値を使う。
...
| AC32 | 手順: 4 ランタイムの導入先それぞれからスクリプトを起動し、一時リポジトリを `--root` に渡して終了コードを記録する。手順に書いた呼び出しの形が、検査の 0 / 1 / 2 / 3 をそのまま返すことも確かめる |
