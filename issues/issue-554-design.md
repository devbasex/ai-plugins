...
**この 2 つ目は `ai-plugins` の `f56c90d9` で実際に出る形である**（AC46。判定規則の試作を同じ状態へ通し、
出た版の段落 7 件と許可していない即時読み込み 1 件を得た）。

## 指示書はどうあるのが適切か

**検査する観点は、外部の一次情報 23 件から出た 21 の観点から選ぶ。** 選び方と入れない理由は
...
**入れないものの理由は 3 つに分かれる。** 機械で判定できない（意味の判定になる）、リポジトリごとに
違う（何を hook で強制できるか）、助言が割れている（文体の指示）。**判定できないものを入れると、
誤検知を消すための除外の一覧が育ち、検査そのものが読まれなくなる。**
...
`progress-tracking` が同じ参照を指す）。`release` の配下へ写しを置かない。手順へ書くのは次の 4 行である。
...
どれか 1 つでしか当たらない。**実装では 4 ランタイムの導入先それぞれから起動して、終了コードを確かめる**（AC42）。
...
| 読み取れない | 終了コード 2。宣言の構造が不正（JSON として読めない・`version` が無いか未対応・項目の型が違う・`pattern` が正規表現として読めない）・**観点のデータを読めない**・`released` から版を 1 つも取れない・`git` が使えない。`--refresh` では通信できないときもこの値 |
...
対象の有無によらず「確かめられなかった」である（AC3 と AC32 の順序）。
...
**数えるのは、箇条書きの項目と、行頭から始まる文の 2 つである**（観点 2）。コードブロックと表の行は数えない。
...
**取れないときは終了コード 2 で止める。** 判定できないことを通過にしない（AC32）。
...
| AC14 | テスト: 宣言の無い根で、出た版の段落を書いても指摘されず、`NOTE:` の行が出る |
| AC15 | テスト: 指示書ごとに指示の数が出る。コードブロックと表の行が数に入らない |
| AC16 / AC17 | テスト: 観点のデータを壊すと終了コード 2。`enforce: report` の観点は値が大きくても 0 のまま |
| AC18 / AC19 | テスト: `checked_at` を 90 日より古くすると `NOTE:` が出て終了コードは変わらない。`review_interval_days` を変えると出方が変わる |
| AC20 / AC21 / AC22 / AC23 | テスト: `--refresh` は取得先を差し替えた偽の出典で動かし、提示の形・ファイルを書き換えないこと・取得の失敗の一覧を見る。通信できないときは 2 で、同じ環境の検査は 0 |
| AC24 | テスト: `$schema` を書いた宣言が、書かない宣言と同じに読まれる |
| AC25 / AC26 / AC27 | テスト: `imports` を宣言した根で、許可の外の参照と、指し先の無い許可がそれぞれ指摘される。`"imports": {}` では参照がすべて指摘され、未指定では指摘されない |
| AC28 / AC30 | テスト: `changelog` の宣言と、最新以下・最新より新しい版の段落 |
| AC29 | テスト: 出た版の版数で始まる行を段落の 2 行目へ置くと指摘されず（`f56c90d9` で 9 件に増える形）、連続する箇条書きの 2 件目（`- v10.1.0 …`）は指摘される |
| AC31 | テスト: `tags` の宣言でタグを 2 つ打ち、最大値が最新になる |
| AC32 / AC36 | テスト: 版を取れない宣言（壊れた即時読み込みがあっても 2 で止まり、その指摘が出ない）、壊れた JSON、`version` の欠落と未対応、`files` の型違い、読めない `pattern` で終了コード 2 |
| AC33 | テスト: `pending_marker` を宣言し、基底が最新・最新未満の 2 通り |
| AC34 | テスト: 指摘の文面に `decisions` の値が出る |
| AC35 | テスト: 根の指示書 1 本が `budget.bytes` を超えると終了コード 1。2 本の合計が超えても、どちらも上限以下なら通る |
| AC37 / AC38 / AC39 | テスト: 出力の形、指摘が無いときの標準出力、知らない引数で終了コード 3 |
| AC40 | テスト: `plugins/ndf/skills/` に新しいディレクトリが無いことを、配る先の一覧の行数で見る |
| AC41 | 手順: `release` の差分と、新設した参照を読む。`$SCRIPTS` を解決してから呼び、末尾で終了コードを戻す 4 行があることを見る |
| AC42 | 手順: 4 ランタイムの導入先それぞれからスクリプトを起動し、一時リポジトリを `--root` に渡して終了コードを記録する。**手順に書いたブロックをそのまま実行し、検査が 0 / 1 / 2 / 3 を返す 4 通りでブロックの終了コードが一致すること**も確かめる |
| AC43 / AC44 / AC53 | 手順: 既存の検査とビルドの確認を実行し、終了コード 0 を見る |
| AC45 / AC46 | 手順: このリポジトリの根での実行と、`git archive f56c90d9 …` を展開した一時ディレクトリでの実行 |
| AC47 / AC48 | 手順: ワークフローの差分を読み、実装の Pull Request の checks を見る |
| AC49 / AC50 / AC52 | 手順: 差分を読む |
| AC51 | 手順: ruleset へ足した後に、その一覧の件数と文書の数字を突き合わせる（実装 Pull Request の差分には入らない） |
| AC54 | 手順: `uv run --with pytest pytest scripts/tests plugins/ndf -q` の失敗の件数が `develop` と同じ（既知の環境要因の 6 件を除く） |
