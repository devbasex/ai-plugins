...
**2 つ目は `ai-plugins` の `f56c90d9` で実際に出る形である**（AC76）。
...
検査は同じに動く（AC46）。**そのため、書き始めるのは配布のタグを打った後でよい** ── 定義ファイルを新設する
...
対象の有無によらず「確かめられなかった」である（AC3 と AC61 の順序）。
...
| 接尾辞どうしの順序 | **接尾辞を `.` で割り、数字として読める要素は数として、読めない要素は文字として比べる**（semver の prerelease の規則）。`10.15.0-dev.2` は `10.15.0-dev.10` より前になる |
...
**取れないときは終了コード 2 で止める。** 判定できないことを通過にしない（AC61）。
