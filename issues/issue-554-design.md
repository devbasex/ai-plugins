...
**この 2 つ目は `ai-plugins` の `f56c90d9` で実際に出る形である**（AC32。判定規則の試作を同じ状態へ通し、
...
graph TD
    ...
    N -->|ある| MAX{released の宣言から最新の版を取れるか}
    MAX -->|宣言があり取れない| E2
    MAX -->|取れた・宣言が無い| IMP[即時読み込みを見る]
    IMP --> VER[版の段落と見出しを見る]
    VER --> BUD[量を数える]
    BUD --> R{指摘があるか}
    R -->|ある| E1[終了コード 1]
...
**取れないときは終了コード 2 で止める。** 判定できないことを通過にしない（AC19）。
...
テストから読まない。**実例の再現（AC32）は実装の Pull Request の本文へ手順と出力を残す。**

| 受け入れ条件 | 何で確かめるか |
| --- | --- |
| AC1 / AC2 / AC3 / AC4 | テスト: 根と配下に指示書を置いた一時リポジトリ、未追跡のファイル、指示書の無いリポジトリ、`files` を足した宣言の 4 通り |
| AC5 / AC8 / AC9 / AC10 | テスト: 参照の書き方 8 通りを 1 つの指示書へ書き、指摘される 2 通りだけが出る |
| AC6 / AC7 | テスト: 配下の指示書に書いた `@x.md` が同じディレクトリのファイルを指す。`@~/x.md` と `@../../x.md` は存在を判定せず、許可を宣言すると判定の対象になる |
| AC11 | テスト: 根の指示書ごとに 1 行出る。2 本が同じ先を引いても 1 回だけ数え、配下の指示書の分は入らない |
| AC12 | テスト: 宣言の無い根で、出た版の段落を書いても指摘されず、`NOTE:` の行が出る |
| AC13 / AC14 | テスト: `imports` を宣言した根で、許可の外の参照と、指し先の無い許可がそれぞれ指摘される |
| AC15 / AC17 | テスト: `changelog` の宣言と、最新以下・最新より新しい版の段落 |
| AC16 | テスト: 出た版の版数で始まる行を段落の 2 行目へ置き、指摘されない（`f56c90d9` で 9 件に増える形） |
| AC18 | テスト: `tags` の宣言でタグを 2 つ打ち、最大値が最新になる |
| AC19 / AC23 | テスト: 版を取れない宣言（壊れた即時読み込みがあっても 2 で止まり、その指摘が出ない）、壊れた JSON、`version` の欠落と未対応、`files` の型違い、読めない `pattern` で終了コード 2 |
| AC20 | テスト: `pending_marker` を宣言し、基底が最新・最新未満の 2 通り |
| AC21 | テスト: 指摘の文面に `decisions` の値が出る |
| AC22 | テスト: 根の指示書 1 本が `budget.bytes` を超えると終了コード 1。2 本の合計が超えても、どちらも上限以下なら通る |
| AC24 / AC25 / AC26 | テスト: 出力の形、指摘が無いときの標準出力、知らない引数で終了コード 3 |
| AC27 | テスト: `plugins/ndf/skills/` に新しいディレクトリが無いことを、配る先の一覧の行数で見る |
| AC28 | 手順: `release` の差分と、新設した参照を読む |
