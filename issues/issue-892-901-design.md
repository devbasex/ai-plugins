...
| 3 | supervisor | 途中の通知を受け、「2 回目を待つ」で応答を終える。**誰にも起こされない** | 途中の通知を受け、`置き場所` に完了の目印の行が現れるまでの待ちを**自分の背景の処理**として起動してから応答を終える |
| 4 | worker | 再開して報告を出す | 再開し、報告の写しを `置き場所` の末尾へ書き、最後の行に完了の目印 `<!-- 作業の報告の終わり -->` を書いてから報告を出す |
...
**待つのは見出しではなく、完了の目印の行 `<!-- 作業の報告の終わり -->` である。** worker は報告の写しを
書き終えた後、最後の行としてこの 1 行を書く。

```bash
timeout 3600 bash -c 'until grep -qsx "<!-- 作業の報告の終わり -->" "$1"; do sleep 5; done' _ "<置き場所>"; rc=$?; echo "exit=$rc"; exit "$rc"
```

| 終了コード | supervisor の動き |
| --- | --- |
| 0（目印の行が現れた） | `置き場所` の最後の `## 作業の報告` から目印の行までを読み、持ち場を進める |
...
| 項目 | 中身 |
| --- | --- |
| 置き場所 | 長い出力と**報告の写し**を書くファイルの絶対パス（報告にはパスだけを載せる）。**worker は最後の応答の前に、`## 作業の報告` の節をこのファイルの末尾へ同じ中身で書き、その後ろに最後の行として `<!-- 作業の報告の終わり -->` を 1 行書く。** supervisor は「無し」にせず、worker ごとに新しいパスで空のファイルを作ってから渡す |
...
sequenceDiagram
    ...
    W->>W: 背景の待ちの完了で再開
    W->>F: 報告の写しと完了の目印の行を末尾へ書く
    W-->>S: 2 回目の通知（読み直さない）
    F-->>S: 写しの待ちの完了通知（目印の行が現れた）
    S->>F: 最後の報告の節から目印の行までを読む
    S->>S: 次の段へ進む
```
...
| AC11 | `claude -p --output-format stream-json` で、supervisor 役のサブエージェントが worker 役（背景で `sleep 30` を起動して応答を終える）を起動する 2 段を動かす。worker 役が報告の写しの後に完了の目印の行を書くこと、supervisor の写しの待ちが目印の行で終わること、supervisor の最後の応答に worker の報告が畳まれていること、supervisor の結果が 1 回で `## 作業の報告` の中身を含むことを確かめる。記録を実装の Pull Request に残す |
