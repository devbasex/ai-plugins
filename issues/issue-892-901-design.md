...
| 2 | worker | 背景の待ちを残したまま応答を終える（規則 5 に反する） | 同じ |
| 3 | supervisor | 途中の通知を受け、「2 回目を待つ」で応答を終える。**誰にも起こされない** | 途中の通知を受け、完了の目印のファイル `<置き場所>.done` が現れるまでの待ちを**自分の背景の処理**として起動してから応答を終える |
| 4 | worker | 再開して報告を出す | 再開し、報告の写しを `置き場所` の末尾へ書き終えた後、空のファイル `<置き場所>.done` を作ってから（`: > "<置き場所>.done"`）報告を出す |
...
| `plugins/ndf/skills/development-workflow/references/waiting.md` | F4 | 見出し `### 途中の通知を受けたとき` を「待つ相手ごとの手」の節の直後に新設し、conductor の手と supervisor の手をそこに書く。「待つ相手ごとの手」の直後の段落の「2 回目の通知を待つ」はこの節へ移す（入出力の契約） |
| 同 `agent-layers.md` の supervisor の規則 4 | F4 | 1 文を足す。規則の数は 10 のまま |
| 同 `agent-layers.md` の worker の規則 5・起動指示の `置き場所` の行 | F5 | 規則 5 に `Monitor` の場合と写しを書く場合を足し、`置き場所` に報告の写しと `<置き場所>.done` を足す。「作業の報告」の項目の表の `置き場所` の行を、supervisor が置き場所を渡したときは長い出力の有無に依らずそのパスを書く形へ直す。規則の数は 5 のまま。`prompt` の行の「守る規則 4 個」を 5 個へ直す |
...
### supervisor の待ち方（F4。`waiting.md` の `### 途中の通知を受けたとき` へ書く中身）

**途中の通知**は、背景の処理を残したまま応答を終えた worker について届く 1 回目の通知である。
見分けは通知の注記（「background work of its own still running」「may be interim」）で行う。

**worker の規則 5（背景の処理を残したまま応答を終えない）は保つ。** 規則 5 を守る worker では途中の通知は起きない。
下の supervisor の手は、規則 5 を守れなかった worker（`Monitor` や背景の待ちを残して応答を終えた worker）への備えである。
...
**待つのは `置き場所` の中身ではなく、完了の目印のファイル `<置き場所>.done` である。** worker は報告の写しを
`置き場所` の末尾へ書き終えた後、空のファイル `<置き場所>.done` を作る（`: > "<置き場所>.done"`）。
supervisor が起動前に空に作るのは `置き場所` だけで、`.done` はこの時点では無い。
...
| --- | --- |
| 0（`<置き場所>.done` が現れた） | `置き場所` の最後の `## 作業の報告` から末尾までを読み、持ち場を進める |
...
> `Monitor` で待つときも同じで（`Monitor` の通知で再開はされるが、親には途中の通知が届く）、supervisor が `置き場所` を渡したときは、長い出力が無くても報告の写しを書き、その後に `<置き場所>.done` を作る

この 1 文は規則 5 を緩めない。写しと `.done` は、規則 5 を守れなかった worker を supervisor が待つための備えである。

起動指示の `置き場所` の行:

| 項目 | 中身 |
| --- | --- |
| 置き場所 | 長い出力と**報告の写し**を書くファイルの絶対パス（報告にはパスだけを載せる）。**worker は最後の応答の前に、長い出力の有無に依らず `## 作業の報告` の節をこのファイルの末尾へ同じ中身で書き、書き終えた後に空のファイル `<置き場所>.done` を作る（`: > "<置き場所>.done"`）。** supervisor は「無し」にせず、worker ごとに新しいパスで空のファイルを作ってから渡す |
...
sequenceDiagram
    ...
    W->>W: 背景の待ちの完了で再開
    W->>F: 報告の写しを末尾へ書き、<置き場所>.done を作る
    W-->>S: 2 回目の通知（読み直さない）
    F-->>S: 写しの待ちの完了通知（.done が現れた）
    S->>F: 最後の報告の節から末尾までを読む
    S->>S: 次の段へ進む
```
...
| AC11 | `claude -p --output-format stream-json` で、supervisor 役のサブエージェントが worker 役を起動する 2 段を動かす。worker 役は規則 5 に反して、背景で 30 秒の待ち（`sleep 30`）を残したまま応答を終えるよう指示して再現する。worker 役が報告の写しを書き終えた後に `<置き場所>.done` を作ること、supervisor の写しの待ちが `.done` の出現で終わること、supervisor の最後の応答に worker の報告が畳まれていること、supervisor の結果が 1 回で `## 作業の報告` の中身を含むことを確かめる。記録を実装の Pull Request に残す |
