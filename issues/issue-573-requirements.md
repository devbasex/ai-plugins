...
- [ ] AC8: 読める宣言を指す symlink がある状態で `init` を実行すると、終了コード 0 で「既にあります」を出す
  （現状は「symlink です」で 1）
- [ ] AC9: `init --force` は、JSON として壊れた宣言・`version` が未対応（`99`）・空のファイル・読み取り権限の無いファイルでは、
  宣言を作り直して終了コード 0 で終わり、直後の `check` は 0 を返す。宣言の位置がディレクトリの形は #628 が扱うため
  含めない（現状は 1 で終わる）。symlink の宣言と symlink の `.ndf` は、これまでどおり 1 で断る
...
- `init --force` の書き込みの経路。宣言の位置がディレクトリのときに 1 で終わり一時ファイルが残る事象は #628 が扱い、AC9 もこの形を含めない
