...
- [ ] AC34: **測定の部品（`transcript_agents.py` と `skill-stats`）がネットワークを使わない。** 読むのはローカルの記録だけで、統計の送信先を持たない（#159 の送信はこの課題の範囲外）。
  振り返りの記録（AC33）は、既存の `retrospective` と同じく GitHub のコメントとして投稿される。**投稿してよいのは AC30 が許す値**（数値・層・持ち場・作業の種類・モデル名・終わり方）**だけ**で、プロンプト・本文・パスは投稿しない
...
| 運用・保守性 | 上限の中断と再開の回数が測定に残る。報告なしで続けさせた回数が president の報告に持ち場ごとに残る。見回りが尽きたことに人が気づけるよう、president は止まる前に現在地を 1 回出す |
