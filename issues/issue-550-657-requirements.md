...
- [ ] AC34: **測定の部品（`transcript_agents.py` と `skill-stats`）がネットワークを使わない。** 読むのはローカルの記録だけで、統計の送信先を持たない（#159 の送信はこの課題の範囲外）。
  振り返りの記録（AC33）は、既存の `retrospective` と同じく GitHub のコメントとして投稿される。**投稿してよいのは AC30 が許す値**（数値・層・持ち場・作業の種類・モデル名・終わり方）**と、そこから導いた印（`束ねる候補` / `割る候補` / `worker を使いすぎ`）と判断の理由の文**だけで、プロンプト・応答の本文・パスは投稿しない
