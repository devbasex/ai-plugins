...
**構造改善（`cross-refactoring`）を飛ばした。** 工程表では `standard` の必須工程である。

| 理由 | 中身 |
| --- | --- |
| 変更が削除である | 新しい構造を持ち込まない。消した実体の跡に提案する対象が無い |
| 収束ループが成立しない | 母集合に codex が入るが、この環境ではモデルを引けず 404 になる（#461）。提案の 3 者のうち 1 者が毎ラウンド結果を残さない |

**実装レビュー（`cross-review`）は通す。** レビュワーは輪番で選ばれるため、codex が当たらない
ラウンドで回せる。
