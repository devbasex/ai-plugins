...
リポジトリごとの差は宣言ファイル（`.ndf/worktree.json`）が持ち、宣言が無いリポジトリでは
これらの仕組みは何もせずに終わる。
...
| ローカル環境 | 人が画面を触って動作を確かめるためのサービス一式。主ディレクトリの 1 セットを共有することを既定とする |
| テスト環境 | 1 つの作業ツリーに対応して立ち上げる、自動テストの実行に必要なサービス一式。番号で識別され、他の作業ツリーのものとポート・データ保存先・ネットワークを共有しない |
...
- 作業ツリーごとのローカル動作検証の支援（追跡されない設定と依存物の持ち込み、相乗りと分離の使い分け）
- 作業ツリーごとのテスト環境の分離（環境名・スロット・ポートの採番、データ保存先の分離）

含まない:

- レビュー用の作業ツリーの置き場所の変更。既定は非永続領域のまま据え置く（環境変数による切り替えは既に存在する）
- 主ディレクトリの編集を拒否する強制。誘導と検知に留める
- `compose` 以外の実行系での分離。`localenv.kind` が `compose` 以外のリポジトリは未対応として扱う
...
| 層 | 担当 | 働くタイミング |
| --- | --- | --- |
| 誘導（土台） | 新設する Skill | 開発の依頼を受けた時点 |
| 誘導（主） | tool 実行前の hook（Claude Code / Codex CLI） | 主ディレクトリの保護対象パスを編集しようとした時点 |
| 誘導（主） | プロンプト送信時の hook（Kiro CLI） | 主ディレクトリで作業している時点 |
...
plugins/ndf/scripts/worktree-testenv.sh          テスト環境の採番・起動・公開・後片付け
```
...
- **対象ファイル:** `plugins/ndf/skills/worktree/references/test-execution.md`、`plugins/ndf/scripts/worktree-testenv.sh`
- **変更内容:** 環境名・スロット・ポートを作業ツリーから決定的に採番し、台帳へ記録する。基準イメージのタグをスキーマ定義の内容から計算する。起動・停止・公開・後片付けを 1 つの入口にまとめる。外部公開はマスク済みデータを載せたテスト環境だけに許す
...
- **詳細設計:** [`04`](04-component-and-data.md) 台帳、[`05`](05-interface-contract.md) テスト環境の操作、[`06`](06-decisions-and-tests.md) 決定 7・10
...
| 外部公開を伴う検証で顧客データが外部へ出る | 公開はマスク済みデータを載せたテスト環境に限り、公開コマンドが照合して他を拒否する（受け入れ条件 36）。マスク済みデータが整うまで公開を伴う検証は行わない |
