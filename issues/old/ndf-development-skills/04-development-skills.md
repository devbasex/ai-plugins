...
Apache-2.0 の告知保持は、頒布物の受領者に告知が届くことを求める。リポジトリ直下に置くだけでは、プラグインとして導入した利用者の手元に届かない。手順は [07-tasks.md](07-tasks.md) Task 1-5 に置く。

> **実装時の判断（v6.1.0）**: 告知そのものは**持たないことにした**。上流の文章・コードを一切転用しておらず、MIT も Apache-2.0 も告知の義務が発生しないためである。義務のない告知を配布物へ同梱すると、受領者には転用があるかのように読め、同期の仕組みだけが保守対象として残る。記録は `upstream-skills.lock.yaml` に残し、転用が生じた時点で告知と同期経路（`build-runtime-plugins.sh` / Kiro の `install.sh`）を同時に用意する。
