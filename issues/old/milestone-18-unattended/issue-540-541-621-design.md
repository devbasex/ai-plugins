...
「長い待ちに入ったら次の設計に着手する」を下限に足す形は採らない。工程を supervisor へ出す運転（G3）では、
supervisor の中の待ちは進行側から見えず、契機として観測できない。依存と本数だけで着手を決めれば、待ちを
知らなくても次の設計が起動される。
...
| 1 本（担当 = supervisor 1 つ）の見込み | 2048 MiB | 要求の実測「このホストの値」の、5〜6 本で落ちた後（2026-09-13 07:50）の `memory.peak` 7.4GiB（#621）。1 本あたり約 1.2GiB に、テストとコンテナの山を足した |
...
**1 本は担当 1 つ（作業ツリー 1 つ、G3 の supervisor 1 つ）である。** その中で動く worker は既定 1 つで、この見込みに含めて数える。
...
| 担当（supervisor と、その中の worker） | 進行側と同じプロセスのサブエージェント | 実行計画を書き換えない。報告で状態を返す。本数には supervisor だけを数える |
...
| G3（#550 #657） | `stage-windows.md` が並行の本数を `parallel-work.md` から引く。president が着手の判定で `issue-plan-strategy` を起動する | G2 は `stage-windows.md`・`context-window.md`・`retrospective`・`skill-stats` を編集しない。**G2 の担当 1 本 = G3 の supervisor 1 つ（作業ツリー 1 つ）で、worker はその 1 本の中に含めて数える**（前提 7、G3 の決定 26）。supervisor の報告を受け取った時点が、実行計画の見直しの契機になる |
| G4（#554） | なし | — |
