...
**2 つ目は `ai-plugins` の `f56c90d9` で実際に出る形である**（AC78）。
...
検査は同じに動く（AC48）。**そのため、書き始めるのは配布のタグを打った後でよい** ── 定義ファイルを新設する
...
対象の有無によらず「確かめられなかった」である（AC3 と AC63 の順序）。
...
| 捕捉した値が版数の形でない | その行を読み飛ばさず、終了コード 2。**形は semver に従う** ── 数字 3 つと、任意の接尾辞（`.` で割った各要素が空でなく、英数字と `-` だけで、数として読める要素に先頭の 0 が無い）|
...
**取れないときは終了コード 2 で止める。** 判定できないことを通過にしない（AC63）。
