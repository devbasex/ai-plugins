...
**`expose` を落として空になった `testenv` は、節ごと反映しない。** 空の節を重ねると、`testenv` を
宣言していないリポジトリで `worktree-testenv.sh` の起動判定（`(.testenv | type) == "object"`）が
off から on へ変わり、追跡されないファイルだけで仕組みの有無が動く。報告（決定 12 の `ignored`）は
従来どおり `testenv.expose` を並べる。
