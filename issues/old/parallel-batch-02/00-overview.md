...
残っていたのは完了の定義のうち「版の配布後にリリース後テストを実施し、結果を記録する」
の 1 項目だけだった。受け入れ条件 15 件と振り返りは先に満たされており、この 1 項目も
この Pull Request で実施して記録した。
...
```console
$ gh auth status
  ✓ Logged in to github.com account <利用者> (GH_TOKEN)
  - Token scopes: 'gist', 'read:org', 'repo', 'workflow'
```

`<利用者>` は実際のアカウント名を置き換えたものである。判断の根拠になるのはスコープの
一覧であり、どのアカウントで認証していたかは要らない。
...
PR #194 のベースは `main` ではなく `feature/issue-188-release-step`（担当 B）であり、B が
マージされた後にベースを `main` へ切り替える。
...
担当 D はこの表に入れていない。担当 B の枝から派生させるため、A / B と同時には動かないためで
...
Skill であり、B の枝の外には存在しない。担当 D を B から派生させるのは、この重なりを
順序で避けるためである。
...
- **担当 D は担当 B から派生させる。** 工程表の同じ行を触るため、B の実装が入った枝を
  分岐元にする。PR #194 のベースは `feature/issue-188-release-step` である。B がマージ
  された後にベースを `main` へ切り替え、このバッチの中でまとめてマージする（図の
  `B --> D --> M` はこの順序を指す）。着手の条件（`project` スコープ）が整ったのは
  指示書を書いた後である（前掲の経緯を参照）
