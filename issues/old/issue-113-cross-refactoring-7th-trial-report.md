...
| 提案（重複排除後） | 26 |
| 採用 | 19 |
| Pull Request に残った改善項目（ラウンド 1〜3） | 10 |
| 取り消し | 5 |
| ラウンド 4 の未取り込み | 4 |

採用 19 件の内訳が下 3 行である。ラウンド 4 の 4 件は適用まで進んだが、中断のため
進行側の取り込みに至っていない。
...
```console
$ git diff --stat 462b6e5..678baeb -- lib/devbase/volume lib/devbase/snapshot \
      tests/volume tests/snapshot
（出力なし）
```

計画に記した実測（`6702c16` で 1376 passed / 38.6 秒）は、計画を書いた時点の `main` で
取った値である。そこから試行までに `main` が 5 コミット進んだため、着手前のテストは
上表の `462b6e5` での値（1474 passed / 1 skipped / 33.9 秒）を試行時点の実測とする。
...
| ---: | --- | --- | ---: | ---: | ---: | ---: | ---: | ---: |
| 1 | codex | 17:59:49 | 30 分 20 秒 | 231 秒 | 544 秒 | 246 秒 | 570 秒 | 163 秒 |
| 2 | kiro | 18:30:09 | 34 分 06 秒 | 163 秒 | 861 秒 | 312 秒 | 360 秒 | 286 秒 |
| 3 | claude | 19:04:15 | 23 分 14 秒 | 283 秒 | 626 秒 | 237 秒 | 240 秒 | — |
| 4 | codex | 19:27:29 | 25 分 44 秒 | 263 秒 | 482 秒 | 223 秒 | 225 秒 | 297 秒 |

提案とレビューは複数の担当を並列で起動するため、最も遅い 1 者の所要がそのまま待ち時間に
なる。レビュー欄は再レビューを含む待ち時間の合計で、実行ログの経過表示（15 秒刻み）から
取った。各欄の合計はラウンドの所要に収まる。

状態ファイルの `durations` は、各担当が結果ファイルへ自己申告した `elapsed_seconds` の
集計であり、待ち時間とは別の値である。ラウンド 1 のレビューは 1,935 秒（gemini 415 秒 +
kiro 1,520 秒）と記録されているが、ラウンドの所要 1,820 秒を超えている。本レポートの
所要はすべて、プロセスの起動から終了までを測った値を使う。
...
所要秒は集計出力の値で、各担当が結果ファイルへ自己申告した `elapsed_seconds` の合計である。
プロセスの起動から終了までを測った待ち時間とは一致しない（ラウンド 1 の kiro のレビューは
申告 1,520 秒に対し、待ち時間は 255 秒と 315 秒の 2 回だった）。担当同士の比較には使えるが、
経過時間の節では待ち時間の側を使っている。

| 実装担当 | 担当したラウンド | 適用（状態 `done`） | Pull Request に残った件数 | 取り消し | 予算超過率 | 所要秒 |
| --- | --- | ---: | ---: | ---: | ---: | ---: |
| claude / default | 3 | 4 | 4 | 1 | 0.20 | 567 |
| codex / default | 1・4 | 0 | 3 | 2 | 0.00 | 1916 |
| kiro / claude-sonnet-5 | 2 | 3 | 3 | 2 | 0.40 | 1380 |

「適用」は集計出力の同名の列で、状態が `done`（レビュー 2 者の承認まで到達）の件数を数える。
codex の 0 は、担当した 2 ラウンドがどちらも承認まで到達しなかったことを表す。ラウンド 1 の
3 件は取り消しを免れて Pull Request に残り、ラウンド 4 の 4 件は中断のため取り込まれていない。
「Pull Request に残った件数」の合計 10 件が、結果の表の同名の行と一致する。

```console
$ python3 -c "import json;from collections import Counter;\
s=json.load(open('.cross_refactoring/cross-refactoring-rf121-state.json'));\
print(Counter(i['status'] for i in s['items']))"
Counter({'reviewing': 7, 'done': 7, 'abandoned': 5})
```
