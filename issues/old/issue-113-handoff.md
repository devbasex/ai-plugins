...
このメモは **設計と事前調査の段階**で書いたものである。実装は
`feature/cross-refactoring-skill` で行い、作業単位 1〜13 が完了した（下の「進捗」を参照）。
引き継ぎ時点で決まっていたこと・決まっていなかったことの記録として残す。
...
### 実装（`feature/cross-refactoring-skill`）

- [x] 作業単位 1〜12（状態管理 / 作業ディレクトリと Skill 配置 / 参照ファイル /
      提案 / 適用 / レビュー / 見送り / 収束判定 / 監視の汎用化 / SKILL.md と docs /
      テスト / 配布物同期）
- [x] 作業単位 13（共通層 `cross-review/scripts/lib/` の切り出し）
- [ ] 作業単位 14〜16（`cross-review` への展開）— 本 Skill が一周してから着手する

### 実装で設計から変えたところ

| 変更 | 理由 |
| --- | --- |
| 除外設定を `.git/worktrees/<name>/info/exclude` から**配置先の `.gitignore`** へ | 現行の git は作業ディレクトリごとの `info/exclude` を読まず共通側を見る（実測）。共通側へ書くと対象リポジトリ本体を書き換えることになる |
| gemini にも Skill を配置する（`.gemini/skills/`） | 配置しないと語彙を読めず、提案が全件 `unknown` へ降格して採用 0 件になる |
| gemini へ `--include-directories <tmp>` を渡す | 結果ファイルの置き場所が作業ディレクトリの外にあるため、渡さないと書き込みが拒否される |
| `monitor.py --state-file` は作らなかった | 監視スクリプトは状態ファイルを読まない。読み手のいない引数を足さない |
| レビュー結果の差し戻しに上限（1 回）を設けた | 形式を満たせないランタイムで再レビューが無限に続く |
...
| **Step 0.5 で NDF Skill を worktree へプロビジョニングする** | 参加ランタイムに NDF が入っている保証がない。ランタイム標準の配置先（`.claude/skills` / `.agents/skills` / `.kiro/skills`）へコピーし、既存があれば上書きしない。配置は worktree 内に閉じ、利用者のホームとリポジトリ本体は触らない |
| **プロンプトに Skill の明示パスを必ず書く** | **kiro は配置しただけでは SKILL.md 本文を読まない**（実測）。claude / codex は自動発動するが、プロンプトは 3 ランタイム共通なので常に明示する |
...
8. **Skill プロビジョニング（Step 0.5）の追加と、発動の実測**
   利用者の指摘で「参加ランタイムに NDF Skill が入っているかを事前チェックし、
   無ければ入れるフェーズ」を §11 として追加した。あわせて 3 ランタイムで
   Skill 呼び出しを実測し、**kiro だけ自動発動しない**ことが分かったため、
   プロンプトへの明示パス記載を必須要件にした（検証記録 §8）。

## 6. 落とし穴（実装前に必ず読む）

- **kiro は Skill を置いても本文を読まない。** `description` の語彙だけで「それらしい」
  応答を返すので、**発動しているように見えて中核手順を飛ばす**（実測では現状固定テストを
  1 本も書かなかった）。プロンプトに `.kiro/skills/<name>/SKILL.md` の明示パスを必ず書く
- **`codex exec` に argv でプロンプトを渡すとハングする。** stdin が開いている限り
  `Reading additional input from stdin...` で待ち続ける（600 秒でも終わらない）。
  cross-review 同様 **stdin から渡す**。argv 形式なら `< /dev/null` が必須
- **Skill を読ませても手順を守るとは限らない。** 実測では現状固定テストの追加数が
  claude 17 本 / codex 1 メソッド / kiro 0 本と揃わなかった。`merge-apply` の機械検証で
  結果側から担保する
- **claude の適用フェーズは 1 起動 $1.42 の実測**（29 ターン / 218 秒）。
  「単純なタスクで $0.26」の 5 倍以上で、上限値の既定は保守的に置く
