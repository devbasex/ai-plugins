...
| [#542](https://github.com/devbasex/ai-plugins/issues/542) / [#786](https://github.com/devbasex/ai-plugins/issues/786) | cross-review のラウンドを減らす。既定の母集合から agy を外し、出し切りを求め、既存コメントの控えをラウンドごとに取り直し、設計 Pull Request に観点を渡す（マイルストーン 26「17 トークン消費の削減」。前のラウンドからの変更の節は #934 へ移した）。確定仕様は [cross-review-round-inputs.md](../../docs/specifications/cross-review-round-inputs.md) / [cross-review-participants-and-seats.md](../../docs/specifications/cross-review-participants-and-seats.md) / [cross-refactoring-participants.md](../../docs/specifications/cross-refactoring-participants.md) | [milestone-26-cross-review-rounds/](milestone-26-cross-review-rounds/issue-542-786-requirements.md)（要求・設計・決定・計画の 4 本） |
| [#928](https://github.com/devbasex/ai-plugins/issues/928) / [#936](https://github.com/devbasex/ai-plugins/issues/936) | 中継の導入を明示の `/ndf:install-wrapper` にし、`/ndf:restart` を足し、切れ目の `/exit` が質問の答えにならない守りを入れ、2 つ目以降の区間へ起動の方針の引数を引き継ぐ（マイルストーン 26「17 トークン消費の削減」。送り込みの実装は #931 へ分けた）。確定仕様は [ndf-relay-install-and-restart.md](../../docs/specifications/ndf-relay-install-and-restart.md) / [ndf-relay-segment-restart.md](../../docs/specifications/ndf-relay-segment-restart.md) | [milestone-26-relay-install-restart/](milestone-26-relay-install-restart/issue-928-requirements.md)（要求・設計・決定・送り込みの実測・計画の 5 本） |
