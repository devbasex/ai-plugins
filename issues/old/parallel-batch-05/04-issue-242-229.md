...
タスクは受け入れ条件に紐づける。**先に落ちるテストを書き、通す最小の記述を入れる。**

| # | 何を作るか | 対象 | 満たす受け入れ条件 |
| --- | --- | --- | --- |
| 1 | 判断表とリポジトリの解決の 3 段 | `out-of-scope/references/issue-target.md`（新設） | 10 / 14 / 15 / 17 |
| 2 | 「起票先を決める」の段と、`--repo` の引き渡し | `out-of-scope/SKILL.md` | 13 / 14 / 16 |
| 3 | 記録先をコメントへ移す | `retrospective/SKILL.md` | 1 / 2 / 3 / 4 / 5 / 8 |
| 4 | 落とし先の表の直後の参照 1 行 | `retrospective/SKILL.md` | 11 / 12 |
| 5 | 既存の記録の範囲と以降の記録先 | `AGENTS.md` のドキュメント表の 1 行 | 6 / 7 |

検査は束ごとに置く。`out-of-scope/tests/` が 10 / 13 / 14 / 15 / 16 を、
`retrospective/tests/` が 1 / 2 / 3 / 4 / 5 / 8 / 11 / 12 を見る。残る 9 / 18 は
リポジトリ全体の検査で確かめる。
