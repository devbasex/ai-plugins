...
| 最終ゲートの修正 1 回分（決定 26） | 5.5 分 | 同上 |

使える時間は 51 − 13 = **38 分**である。項目 1 件の見積りは配分テーブルから引く（初期値）。
...
順位の順に積むと、テストを足す項目が半分なら 13 件前後が入る。#917 は 71 分で 25 項目（テスト 10・改善 15）だった。
...
| `final_fix` | 同上。最終ゲートの修正 1 回分（決定 26）。検証の直しの終わり（`budget.fix_end`）からも差し引き、最終ゲートのために残す |
...
2. 修正の残り時間が `fix` に足りなければ、落ちた項目を取り消す（回数の上限は持たない。決定 23）。修正の残り時間は `started_at + budget_minutes − danger_whole_test − final_whole_test − final_fix − 今` で測る（控えの `fix` を差し引かない終わりから測る。`final_fix` は最終ゲートのために残す（決定 26）。`T` から測ると、控えた修正 1 回分が使われない）（項目の単位。隣接する変更は今と同じく全件の取り消しへ退避）
