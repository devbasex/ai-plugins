...
| 最終ゲートの全体のテスト（工程の 1 つとして起動したときだけ） | 1 分 | 同上 |
...
| `judge` | `{kind: "jev" \| "runtime", reason}` | Jev を使ったか。使わなかった理由（`no_key` / `private_repo` / `disabled` / `probe_failed`） |
...
| `final_whole_test` | 工程の 1 つとして起動し、`--ci-check` が無いときだけ `baseline_test.seconds`。それ以外は 0 |
...
2. 落ちた項目があり、Jev が使えるなら失敗を分類する。`flaky` なら 1 度だけ走らせ直す
3. 落ちた項目のうち、`fix_count` が上限に達したもの、または残り時間が `fix` に足りないものを取り消す（項目の単位。隣接する変更は今と同じく全件の取り消しへ退避）
4. 残りの落ちた項目があれば `VERIFY=fix` を返す（駆動が修正を 1 回起動する）
5. 落ちた項目が無くなったら危険の印を判定する。印が 1 つでもあり、`whole_test.ran` が偽なら全体のテストを 1 度走らせる
6. 全体のテストが落ちたら、印を持つ項目を新しい順に取り消す。**走らせ直さない**（確かめるのは最終ゲートである）
7. `VERIFY=done` を返す
...
| D3 | 触った本番のファイルが、`--scope` の外から参照されている | 拡張子を除いたファイル名を `git grep -l` で `--scope` の外に探す |
| D4 | 限ったテストのファイルのどれも、触った本番のファイルの名前を含まない | 同上を限ったテストのファイルの中で探す |
...
| --- | --- | --- |
| 単独 | `cross-review`（想定最大時間の外） | 関係しない |
| 工程の 1 つ・`--ci-check` あり | 継続的統合の結果 | 関係しない |
| 工程の 1 つ・`--ci-check` なし | 全体のテスト（想定最大時間の内。控えに入れる） | **検証の中で走って通り、その後に HEAD が生成物の同期のコミットしか進んでいなければ、走らせずに通す** |
...
| セキュリティ | Jev は公開リポジトリに限る。送るのは提案の要約・差分の統計・定義の行・テストの出力の末尾だけで、ファイルの本文を送らない。鍵は環境変数からだけ読む |
...
| #930 との順序 | #930 が `setup.py` / `report.py` / `prompts/apply.md` を触る | 実装は #930 のマージの後に作業ツリーを作る |
