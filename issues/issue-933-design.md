...
| `commands/propose.py` | 新しく作る（`apply.py` の `merge-proposals` から移す） | `merge-proposals`: 鍵（`path` + `symbol` + `smell`）が同じ提案の機械的な統合・語彙としきい値の検査・候補の切り出し。切り出しは `path` + `symbol` の組の単位で行い、組を上位から 30 組まで取り、取った組の提案はすべて渡す（下の「候補の切り出し」） |
