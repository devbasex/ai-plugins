...
ブロックの読み取りは、行頭の ```` ```ndf-next ```` から次の行頭の ```` ``` ```` までとする。
...
| AC3 | `test_relay.py`: tmux の問い合わせを差し替え、ブロック 1 つの標準入力で印が書かれ、6 つのキーを持つこと |
| AC4 | 同: `TMUX` 無し・環境変数無し・pid が死んでいる・ブロック 0・ブロック 2・壊れた JSON の 6 通りで、印が無く出力が空で終了コード 0 |
