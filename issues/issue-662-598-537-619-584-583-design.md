...
| cross-refactoring の測定（`refactor_lib/measure.py`） | 新設 | 状態と監視の記録から工程ごとの所要と群の試行回数を組み立てる | P1 |
...
| D-C | cross-refactoring の `SKILL.md` と `docs/` の `monitor.py` の呼び出し 8 か所の**引数**と、`launch-cli.sh` の `PRINT_TIMEOUT` の行 | D-A（P2）が変える。D-C はこれらの行の上限の値を書かない。骨組みのそれ以外の行は D-C |
