...
| 対象 | 期待値が変わったとき |
| --- | --- |
| 既存のテスト | **振る舞いの変更である**（してはいけない） |
| 現状固定テスト | **役目を終えた置き換えでありうる**（印と理由が要る） |

**置き換えには印を残す。** どのテストが現状固定であるかは、`refactoring` の
`characterization-tests.md` が定める印で分かる。**印の無いテストを現状固定として扱わない。**
...
| 16（`legacy-refactor` との書き分け） | `pre-refactoring.md` の表。`references/workflow-modes.md` からの参照 |
