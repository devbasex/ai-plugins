...
**機械で判定できない差分は、AI エージェントへ渡して判定させる。** `cross-refactoring` は
既に 4 つの CLI を動かす枠組みを持つ。**その枠組みを使わずに人へ回すのは、レビューの前に
解けるものを後ろへ送ることになる。**

| 段 | 誰が判定するか | 何を見るか |
| --- | --- | --- |
| 1 | 機械（`verify.py`） | 接頭辞を伏せた `assert` 行の集合が一致するか |
| 2 | **AI エージェント** | 集合で判定できない差分について、**同じ入力に対する期待出力が変わったか** |
| 3 | 人（Pull Request のレビュー） | AI が「判定できない」と答えたものだけ |

**2 の起動は、検証（Step 5）の中で行う。** レビュー（Step 7）より前である。判定の対象は
段 1 が拾えなかった差分に限るため、掛ける費用は差分の量で決まる。

**AI に渡すのは差分と、判定の基準である。** 「期待する振る舞いを変えたか / 読み込みの経路を
変えただけか」の 2 択で答えさせ、**判定できないときは「判定できない」と答えさせる**。答えを
3 択にしないと、迷ったものが片方へ倒れる。

**3 まで残るものは少ない。** それでも段を残すのは、AI が判定できないと答えたときに止まる先が
要るためである。**通ったものとして扱わない。**
...
| 11（検証が種類を見る） | `verify.py` のテスト。期待値が変わった差分を落とすこと。**機械で判定できない差分を AI エージェントへ渡すこと** |
...
| AI エージェントへ渡す形 | 差分をそのまま渡すか、`assert` の周辺だけを切り出すか。**渡す量が増えるほど費用が上がる**ため、実装時に測って決める |
| AI の判定の当たり | 期待出力が変わったかを 3 択で答えさせるが、**当たりの実測は無い**。最初の数ラウンドで人の判定と突き合わせる |
