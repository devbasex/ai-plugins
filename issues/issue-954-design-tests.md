...
| 受け入れ条件 | 何で確かめるか | 階層 |
| --- | --- | --- |
| AC1 | `test_supervisor_agents.py`（設計の「supervisor の定義の作り方」の 3）。2 つの定義の frontmatter を読み、`name`・`experimental.cacheTtl` が片方だけにあること・frontmatter を除いた本文がバイト単位で等しいこと・`tools` / `disallowedTools` が無いこと・`plugin.json` の `agents` に載ることを照らす。`claude plugin validate .` の終了コード。定義の数を書いた文書の更新はレビューで見る（文言のテストは書かない） | 単体・コマンド |
| AC2・AC4・AC4b | 文書の変更。文言を照合するテストは書かない（`AGENTS.md`）。設計 Pull Request と実装の `cross-review` が見る。AC4b の落とし方は AC3 の実機の 1 回で U5 と一緒に確かめる | レビュー・手動 |
...
| --- | --- | --- | --- | --- | --- |
| 1 | `ndf:supervisor` | `cross-review` | 1.5P | — | 止める |
| 2 | `ndf:supervisor` | `ndf:cross-review` | 1.5P | — | 止める |
| 3 | `ndf:supervisor` | `cross-refactoring` | 3P | — | 止める |
| 4 | `ndf:supervisor` | `ndf:cross-refactoring` | 3P | — | 止める |
| 5 | `ndf:supervisor` | `cross-review` | 1.4P | — | 通す |
| 6 | `ndf:supervisor` | `pr` | 3P | — | 通す |
| 7 | `ndf:supervisor` | `cross-review` | 1.5P | 1 の直後の同じ起動 | 止める（1 度通しを持たない） |
...
| 13 | `ndf:supervisor` | `cross-review` | 1.4P | 親の記録（`transcript_path`）の最後の文脈は自身の P の 10 倍 | 通す（自身の記録を読むこと） |
| 14 | `ndf:supervisor` | `cross-review` | 1.5P | 自身の記録が 300 行を超え、最初の呼び出しが末尾 200 行の外にある。末尾 200 行の中の最も古い呼び出しの文脈は 1.4P | 止める（P を先頭から読むこと） |
...
| U5 | 知らない `subagent_type` で `Agent` を呼んだときの振る舞い | 失敗すると見て、conductor が一覧を見てから起動し、失敗したら省いてやり直す形にした（「supervisor の定義の作り方」の 4）。失敗の形（エラーで返るか、`general-purpose` へ黙って落ちるか）は見ていない | 実装（AC3 の実機の 1 回で、定義を入れない HOME から `ndf:supervisor` を指定して起動する） |
