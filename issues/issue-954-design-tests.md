...
| 10 | `general-purpose` | `cross-review` | 3P | 親の記録の `Agent` の `subagent_type` も無い（`general-purpose`） | 通す |
| 10b | `general-purpose` | `cross-review` | 3P | 親の記録の `Agent` の `subagent_type` が `ndf:supervisor` | 止める（親の記録から引くこと） |
| 10c | `general-purpose` | `cross-review` | 3P | 親の記録の `Agent` の `subagent_type` が `ndf:supervisor-waits` | 通す |
