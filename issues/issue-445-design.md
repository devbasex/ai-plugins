...
| 3 | 文字・数字・結合文字・`-`・`_` 以外を落とす | `` `$SCRIPTS` を決める `` → `scripts を決める` |
...
コードブロックの中の行は、既存の除外と同じ扱いで読まない。引用（`>` で始まる行）は見出しの
抽出では除かない。引用の中に見出しは現れないためである。
