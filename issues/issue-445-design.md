...
graph TD
    ...
    C -->|空でない| S{既存の除外<br/>should_skip に当たるか}
    S -->|当たる<br/>外部の URL・絶対パスなど| Z
    S -->|当たらない| H{相手の文書は<br/>在るか}
    H -->|無い| Y[見出しの照合を飛ばす<br/>欠落は既存の照合だけが積む]
...
**`#` の左が空でない参照は、相手の文書を探す前に既存の `should_skip` を通す。** 外部の URL
（`https://…#節`）や絶対パス、`{` を含む書式は既存の照合でも対象外である。この除外を通さずに
文書名として扱うと、URL やパスを手元の文書として探しに行き、在るはずの無い文書を照合することになる。

**ファイルの欠落を積むのは、既存の `target_path` の経路だけである。** 見出しの照合の経路は、
相手の文書が無ければ何も積まずに照合を飛ばす。両方の経路が積むと、1 つの欠落に 2 行が出る
（決定 3）。
...
| 2 | 文字・数字・結合文字・空白・`-`・`_` 以外を落とす | `` `$SCRIPTS` を決める `` → `scripts を決める` |
| 3 | 空白を `-` にする | `design notes` → `design-notes` |
| 4 | 同じ名前が既にあれば `-1`、`-2` を付ける | 2 つ目の `## 手順` → `手順-1` |

**`slugify` の入力は見出しの Markdown そのままで、囲みの記号を落とす段を前に置かない。**
インラインコードの `` ` `` も山括弧も段 2 で落ちるため、`` `<worktree-base>` の解決順 `` は
`worktree-base-の解決順` になり、GitHub が作る名前と一致する。検査の対象となる文書には、
インラインコードに山括弧を含む見出しが 3 件ある（`cross-review/docs/02-fix-and-rotation.md` に 2 件、
`cross-review/docs/04-contracts.md` に 1 件）。山括弧を含む部分を先に取り除く段を置くと、
この 3 件は中の文字ごと落ちる。後からこれらの見出しを指す参照が加わったとき、正しい参照を
失敗として積むことになる。
...
始まる文があり、コードブロックの外に 6 行ある（実測）。空白を求めないと、この 6 行が見出しの
名前を作る。

| 形 | このリポジトリでの件数 | 扱い |
| --- | ---: | --- |
| `#` の直後に空白が続く行 | 多数 | 読む |
| 行頭が `#` で直後が空白でない行 | 6 | 読まない |
| `===` の下線 | 0 | 読まない |
...
連番（段 4）もその分ずれる。この 3 行を指す参照は 0 件であるため、除いて困る箇所は無い。
...
**欠落の 1 行は既存の `target_path` の経路が積む。** `target_path` は `#` の左だけを取り出して
存在を照合するため、`相手.md#名前` の形でもこれまでどおり欠落を報告する。見出しの照合の経路は
欠落を積まず、相手の文書が無いと分かった時点で照合を飛ばす。
...
`--root` にそのディレクトリを渡して検査を別プロセスで実行し、終了コードと出力を見る。
既存の `test_doc_line_limit.py` と同じ形である（スクリプト名が `-` を含み、そのままでは
import できない）。
...
| インラインコードの山括弧を含む見出しが解決できる | `` ### `<worktree-base>` の解決順 `` を置き、`#worktree-base-の解決順` を指す |
...
| HTML のタグを持つ見出し | 検査の対象となる文書には 0 件（実測）。GitHub はタグを落として中の文字だけを残すため、そうした見出しが加わったときは名前が食い違う |

## #543 への申し送り

#543 は同じ検査の走査範囲へ `issues/` を足す。**この変更には取り込まない**が、足した時点で
この照合も `issues/` に掛かるため、重なる点を実測して残す（2026-09-13 の `develop`）。

| 観点 | 実測 | #543 で何が起きるか |
| --- | --- | --- |
| `issues/` の中の見出しへの参照 | 0 件（`](…#…)` の形は 2 件あり、どちらも外部の URL） | 見出しの照合による新たな失敗は出ない。落ちるのはファイルの欠落だけである（#543 は起票時に 31 件と数えた） |
| 決定 2（対象外の文書は見出しを読まない） | `issues/` は現在対象外 | 走査へ足すと `issues/` を指す参照も見出しの照合に入る。決定 2 の境界は `iter_markdown_files` の母集合をそのまま使うため、コードの変更は要らない |
| 行頭が `#` で直後が空白でない行 | `issues/` を足すと 6 → 39 行 | 見出しを空白付きの ATX に限る規則が、そのまま効く |
| 引用の中の見出しの形 | `issues/` を足すと 3 → 12 行 | 引用を除く既存の除外で見出しから外れる |
| インラインコードに山括弧を含む見出し | `issues/` を足すと 3 → 8 件 | 段 2 で山括弧が落ちるため、GitHub と同じ名前になる |

**#543 が `issues/old/` を除外する案を採るとき、決定 2 により `issues/old/` を指す参照は
見出しを照合しない。** ファイルの存在の照合だけが残る。
