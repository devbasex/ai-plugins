...
- 版の更新（配布の工程が決める）
- 誘導の hook が拒否しても Serena へ切り替わらない問題の作り直し（実機の確認 AC26 で判明。#971 として起票した）
- `.serena/serena_config.yml` を追跡している導入先で、Serena が起動のたびに書き直す問題（変更前と同じ挙動。`SERENA_HOME` は決定 16 で保つ。起票しない。Skill の手順 4 で追跡から外す手段を示す）
