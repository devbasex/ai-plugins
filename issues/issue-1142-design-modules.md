...
| `relay_lib/mark.py` | Stop hook と PreToolUse / PostToolUse hook とカットポイントの告知の本体: `FENCE_RE`・`next_blocks`・`running_tasks`・`background_running`・`hold_reason`・`hold_once`・`asked_after`・`cmd_mark`・`cmd_stop`・`DENY_REASON`・`cmd_question`・`notice_lines`・`cmd_notice` | 約 250（実測 228） |
...
| `worktree-branch.sh` | 作業ツリーの一覧と追従先（`wt_dev_worktrees` `wt_follow_target` `wt_follow_enabled`）、ブランチ（`wt_default_branch` `wt_branch_exists` `_wt_declaration_string` `wt_declaration_branch` `wt_base_branch` `wt_production_branch`）、`wt_dirty_paths`。計 10 関数 | 約 195 |
...
| `instructions_lib/findings.py` | 即時読み込み・読み込みの量・指示の数の判定: `import_findings`・`stale_allowance_findings`・`read_size`・`budget_findings`・`count_instructions`・`count_findings`・`SENTENCE_END` | 約 175（定義 161） |
| `instructions_lib/versions.py` | 出た版の段落の判定: `version_findings`・`_inline_pending_findings`・`paragraph_starts`・`released_versions`・`_read_changelog_lines`・`_read_tag_lines`・`_valid_suffix`・`version_key`・`base_triple`・`VERSION_RE`・`VERSION_AT_START`・`LEAD_RE` | 約 190（定義 179） |
...
| `pathkinds.py` | テストと本番コードの判定: `is_test_path`・`_has_shebang`・`_is_code_path`・`production_code_changes`・`TEST_PATH_MARKERS`・`TEST_NAME_MARKERS`・`CODE_EXTENSIONS` | 約 80（定義 67） |
...
| `worktree.py` | 取り消しと作業ツリーの掃除: `revert_item_commits`・`reset_hard`・`revert_range`・`replay_commits`・`_order_newest_first`・`_worktree_changes`・`_control_prefix`・`_dirty_paths`・`_discard_worktree_changes`・`discard_impl_leftovers`・`_require_clean_worktree` | 約 240（定義 224） |
