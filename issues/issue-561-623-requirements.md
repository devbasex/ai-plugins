...
設計の工程で決めた。承認は設計 Pull Request のマージで得る。

| 項目 | 決めた場所 |
| --- | --- |
| まとまりの課題を閉じる時点（C1） | 設計文書の決定 7 |
| squash / rebase のブランチ（A7） | 設計文書の決定 2 |
| 無視されたファイル（A8） | 設計文書の決定 5 |
| Pull Request の head と対応付かないリモートブランチ（A6） | 設計文書の決定 4 |
