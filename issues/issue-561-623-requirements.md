...
- [ ] A12: 無視されたファイルの退避が 1 件でも失敗したとき、同意の有無にかかわらず `git worktree remove`（`--force` を含む）を
      実行せず、退避済みのパスと失敗したパスが報告に載ることが `merged/SKILL.md` に書かれている
...
- [ ] C3: 終わりの工程が振り返りでないモード（`light`、リリース後テストだけを通る `operation`、どちらも通らない `operation`）でも、課題を閉じる手順へ辿り着く。
      そのモードの終わりの工程の Skill に、閉じる手順を呼ぶ記述がある。`release` が配布を飛ばしたまとまりも含む（C12）
...
- [ ] C12: 配布を飛ばしたまとまりでも、`段階: 配布なし` の記録が残り、閉じる条件を満たした課題が閉じる
- [ ] C13: 終わりの工程で盤面の `status` を `Done` にするのは「まとまりを閉じる」の手順だけで、`retrospective` の入口の進行の記録は `Done` を書かない
