...
「約 15 秒後に自動で新しい会話へ切り替わる。キー入力やスクロールをせずに、そのまま待つ」に固定する。**
...
この設計の後は、同じ場面の最後の応答が次のようになる。

````text
約 15 秒後に自動で新しい会話へ切り替わる。キー入力やスクロールをせずに、そのまま待つ
...
- 前提 1: 静まりの既定は 10.17.8 で 5 秒である（`relay.md` の「上限」）。10.17.7 の 15 秒には
  合わせない
- 前提 2: 切り替え（`/exit` → プラグインの更新 → 起動）は、#980 の記録で 4 秒だった。区間 2 の
  `end` が 11:33:10Z、区間 3 の `start` が 11:33:14Z である。更新に通信が要る回を見込んで 10 秒と置く
- 前提 3: conductor の区間の子は、中継が渡した環境をそのまま継ぐ。`NDF_RELAY_DIR` と、利用者が設定して
...
| `-inf` | 待たずに切り替える（`経過 < -inf` が偽） | 負の値と同じ。`relay`、N = 10 |
...
### 決定 5: 秒数は「静まり + 10 秒」の目安として示す

実際の切り替えは、`/goal` の目標の判定の記録を待つ・背景の処理を待つなどで延びる。正確な時刻は
出せないため「約」を付ける。切り替えの時間を中継の記録（`log.jsonl` の `end` → `start`）から毎回
集計する案は採らない。区間の子から見える記録は今の中継の分だけで、最初の切れ目では標本が無い。
...
`relay.py` に足すのは関数 1 つと定数 1 つで、型を定義・変更しない。
...
| AC3c | `NDF_RELAY_QUIET` が `nan` / `-inf` / `inf` | `relay` | `nan` と `-inf` は静まりを 0 として数える（N = 10）。`inf` は契約の表の `inf` の行（`outside` の行ではない） | 0 |
...
| AC3・AC3b・AC3c | `test_relay.py`: `NDF_RELAY_QUIET=abc` で秒数 15、`-3`・`nan`・`-inf` で秒数 10、`inf` で 1 行目 `relay`・2 行目が契約の表の `inf` の行と一致する |
...
| 切り替えの時間の 10 秒 | 実測は #980 の 1 件（4 秒）だけ。プラグインの更新が遅い環境では 10 秒を超えうる。リリース後テストで測る |
