...
「約 5 秒後に自動で新しい会話へ切り替わる。キー入力やスクロールをせずに、そのまま待つ」に固定する。**
...
`N = round(q)`（最も近い整数。切り上げない）。`q` は `quiet` が有限で正ならその値、負か `nan` なら 0 とする（`inf` は下の表で別に扱う）。
...
| --- | --- | --- |
| `nan` | 待たずに切り替える（`経過 < nan` が偽） | `relay`。静まりを 0 として数える（N = 0） |
| `-inf` | 待たずに切り替える（`経過 < -inf` が偽） | 負の値と同じ。`relay`、N = 0 |
...
| AC3b | `NDF_RELAY_QUIET` が負 | AC1 と同じ | 静まりを 0 として数える（中継本体は負の静まりを待たない。N = 0） | 0 |
| AC3c | `NDF_RELAY_QUIET` が `nan` / `-inf` / `inf` | `relay` | `nan` と `-inf` は静まりを 0 として数える（N = 0）。`inf` は契約の表の `inf` の行（`outside` の行ではない） | 0 |
...
| AC3・AC3b・AC3c | `test_relay.py`: `NDF_RELAY_QUIET=abc` で秒数 5、`-3`・`nan`・`-inf` で秒数 0、`inf` で 1 行目 `relay`・2 行目が契約の表の `inf` の行と一致する |
