...
`### セキュリティ要件` の手前まで）。行は移設の時点の `AGENTS.md`（478 行）を指す。
記述が失われていないことは、この表の各行に移動先があることで確かめる。
...
| `docs/plugin-development-guide.md` の「バージョン管理」（L113-174） | - | 章 6・章 7（重複は正本へ集め、跡はリンク） |
