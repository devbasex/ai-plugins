...
| 移した節を名前で指す参照（8 ファイル） | 正本の章を指すように張り替える（「参照を張り替える箇所」） |
...
```bash
git grep -n '版の付け方と\|利用者が過去の版へ戻る\|検査が突き合わせる 15' -- ':!issues/' ':!docs/development-history/'
```
...
| `scripts/tests/test_doc_staleness.py` / `doc_staleness_helpers.py` | 398 / 3, 66 | `AGENTS.md` の節 | 正本の雛形（「テスト設計」） |
...
**区間の外を見るテストは、正本の雛形の側で成り立たせる。** `test_versions_outside_the_section_do_not_fail`
はいま `AGENTS.md` の雛形の変更履歴（`v8.5.4` / `8.4.0`）を使う。検査 J が正本を読むように
なると、`AGENTS.md` の雛形に何を書いてもこのテストは通り、区間の境界を見なくなる。正本の雛形の
章 2 より後ろの章へ、囲んだ古い版数を置き、それで落ちないことを見る。
