...
- **変更内容:** 適用後の検証を次の順序へ変える。
  ...
  4. どちらかが失敗したときだけ、コミットを新しい順に戻しながら原因の項目を特定する。
     特定に使うのは**失敗した側と同じテスト**である。範囲テストが失敗したなら範囲テスト、
     全体テストが失敗したなら全体テストを、戻すたびに実行する。両方が失敗したときは
     全体テストで特定する（範囲テストの失敗は全体テストにも現れるため）
  5. 原因の項目を取り消したら、残りの項目を積み直した状態で、失敗した側のテストを
     もう一度実行する。通れば次へ進む。まだ失敗するなら手順 4 へ戻り、次の原因を
     特定する。取り消しても通らない場合と、項目を分離できない場合は、そのラウンドを
     全件取り消す
