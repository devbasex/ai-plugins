...
| `WF_MODE_ALIASES`（新設） | 旧いモード名を新しい名前へ読み替える表 |
| `WF_STAGE_MATRIX` | 工程 × モードの必須・条件付き・対象外 |
| `WF_STAGE_ALIASES`（新設） | 旧い工程名を新しい名前へ読み替える表 |
...
が失われる。**読むときだけ読み替えの表を当てる。**

```text
読み取り: 生の値 → 読み替えの表 → 現行の名前 → 判定
書き込み: 現行の名前のみ
```
...
**読み替えの表は共通層に 1 つだけ置く。** 控えを読む箇所は複数ある（実行証跡の報告・
Pull Request を作る時点の検査・進行の記録）。それぞれが自前で「`architecture` は `standard`
とみなす」と書くと、次の改名で直し漏れが出る。読む側は必ずこの表を通す。
...
**記録を現在の名前へ合わせるのではなく、現在の読み手が過去の名前を読めるようにする。**
...
| 本 | 主題 | 課題 | 主に触るもの |
| --- | --- | --- | --- |
| 1 | モードの一覧の再編 | #421 | `WF_MODES` / `WF_MODE_HEIGHT` / `WF_MODE_ALIASES` / 判定の基準の表 / `architecture` の 59 か所（周辺 Skill は語の置き換えのみ。決定 12） |
...
| 3 | ドキュメント再構成 | #391 | 工程表の行追加と改名 2 件（決定 13 を含む） / `PJ_STAGES` / `WF_STAGE_ALIASES` / `WF_PR_EXEMPT_STAGE` / 新 Skill / 配布一覧 / 確定仕様 `ndf-workflow-unit-and-gates.md` の工程名の参照（決定 13） |
