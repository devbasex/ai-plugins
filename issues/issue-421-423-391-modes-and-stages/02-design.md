...
| 軸 | 変更前 | 変更後 |
| --- | --- | --- |
| モード（列） | `light` / `standard` / `architecture` / `legacy-refactor` | `light` / `operation` / `legacy-refactor` / `standard`（**高さ順**） |
...
**列の位置からは導かない。** 上げる方向の判定にだけ使う。**列の並びは高さと同じ順だが**
（決定 10）、**高さはこの表が持つ**。位置から計算しない。
...
**移行は 1 度だけ通る道である。** 書き換えを終えた後は、突き合わせる名前が 1 通りしかない。

```mermaid
flowchart TD
    M[記録済みの旧い名前] -->|移行で 1 度だけ書き換える| R
    W[新しい書き込み] -->|現行の名前だけを書く| R
    R[控えと盤面と課題の本文の値] --> D[工程表と突き合わせる]
    D --> E[案内・報告]
```

**読み替えの分岐は無い**（決定 3）。定常時は現行の名前だけを突き合わせるため、値が工程表に
無ければ、名前の揺れではなく記録そのものの誤りとして扱う。
...
**結論**: `WF_MODES` を `light` / `operation` / `legacy-refactor` / `standard` の順にする。
工程表の列もこの順に並べる。

**理由**: **表は軽い順に読める。** 左から右へ、通る工程が増えていく形になる。並びに意味が
無いと、読む側は列の位置から何も読み取れず、毎回どれがどれかを確かめることになる。

**決定 2-b（v10.5.1）と矛盾しない。** そちらが定めたのは「**高さを列の位置から導かない**」
という実装の規約であり、並びを一致させてはいけないという意味ではない。高さは
`WF_MODE_HEIGHT` が別に持ち、列の位置からは計算しない。**一致しているのは読みやすさの
ためであって、導出の根拠ではない。**

**採らなかった案**: `architecture` を除いた位置をそのままにし、`operation` を末尾へ足す
（列の並びに意味が無くなる）。
...
| A4 / A9 | `SKILL.md` の表と `WF_MODES` を突き合わせる既存テスト（並びまで見る） |
| A5 | 移行の後に `git grep` と課題の走査で、旧いモード名が残っていないことを見る |
| A6 | `git grep` による検査（履歴と記録を除く。「実装を 4 本へ分ける」の対象外の表を除く） |
| A7 | `quality-gates` と `design` の記述をレビューが読む（A6 の `git grep` は語が消えたことまでしか見ない。旧 `architecture` の条件がそのまま引き継がれたかは読まないと分からない） |
| A8 | `git grep` で、旧い名前を新しい名前として読む表（`WF_MODE_ALIASES` / `WF_STAGE_ALIASES` にあたるもの）が実装に無いことを見る |
...
| C12 | 課題の走査（旧い名前を持つ `## 進行` の節が 0 件） |
| C13 | 既存テスト（`WF_PR_EXEMPT_STAGE` が工程表の行名に含まれることを見る） |
| C14 | 変更履歴と更新の案内の記述をレビューが読む |
| C15 | 追随が要ると数え直したリポジトリの課題一覧で、移行の課題が立っていることを見る |
