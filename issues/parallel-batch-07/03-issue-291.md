...
4. 上限の応答（HTTP 403 で `message` に `rate limit` を含む / HTTP 429）を模したとき、
   `gh pr comment` が待ち行列へ積まれ、終了コードが 0 になる。巻き直しの 3 種は積まず、
   回復を待って再実行する（待つあいだラウンドは進まないが、巻き直しは 8 ラウンドに 1 度である）
...
| 進行側の投稿のうち `gh pr comment`（1 種） | 扱う。待ち行列を通す |
| 巻き直しの 3 種（`gh pr close` / `gh pr create` / `gh pr reopen`） | 扱う。**待ち行列へは積まず、上限のときは回復を待って再実行する**（決定 3） |
...
└── pending/
    ...
    └── 0002-pr-comment-291.json
```

**この版で積むのは `pr-comment` だけである。** 受け皿は `review-post` / `review-reply` /
`thread-resolve` の種別も持つが、積む側を移すのは次の変更である（決定 2）。巻き直しの
`pr-close` / `pr-create` / `pr-reopen` は種別を持たない。
...
| 4 | `rate_limit` と `secondary` の 2 つで、待ち行列へ 1 件積まれ終了コードが 0 になる。巻き直しの 3 種は積まれず、回復を待って再実行される | `tests/test_state_queue.py` |
...
| 8 | 同じ内容を 2 度流し、2 度目が照会だけで終わることを見る（種別 4 つそれぞれ） | `tests/test_queue_idempotency.py` |
