...
**待ちはフックの 1 回の起動の中だけで生きる。** 保存しない。保存するのは通知の記録だけで、
待ちの鍵を ID として参照する。

**戻り先と関連 URL は待ちに入れない。** 不変条件を守るのに要らないためである。本文を組み立てる
時点で、戻り先の組み立てと関連 URL の抽出がそれぞれ作る。
...
| I2 | 待ち | 種類が「待ちでない」なら送らない（`NDF_SLACK_NOTIFY_DONE=true` のときだけ「完了」として送る） | 送らずに終わる |
| I3 | 待ち | 非対話の実行（`CLAUDE_CODE_ENTRYPOINT` が `sdk-` で始まる）と `stop_hook_active` の再帰では待ちを作らない | 判定の前に終わる |
| I4 | 待ち | 本文は種類の印（`【回答待ち】` / `【承認待ち】` / `【完了】`）で始まり、戻り先を 1 つ持つ | 戻り先が組み立てられなくても、ホスト名と cwd の行を必ず載せる |
| I5 | 待ち | 承認待ちで PR が見つかるなら、本文に PR の URL が載る | 本文から取れなければ現在のブランチの PR を補う。どちらも無ければ PR の行を省く |
| I6 | 通知の記録 | 通知の失敗（Slack・`gh`・`git`・transcript の読み込み）はフックを失敗させない | 終了コード 0 で終わり、`DEBUG_SLACK_NOTIFY=true` のときだけログへ書く |
...
| 戻り先 | 通知から当該セッションへ戻る手段。セッションの URL か、再開のコマンドとホスト名・cwd | 追加（`ndf-notification`） |
...
| 入口 | 標準入力と transcript の末尾を読んで判定へ渡す。判定・抑止を同期で済ませ、送信を切り離した子へ渡して戻る | `plugins/ndf/scripts/wait-notify.py`（新設） |
