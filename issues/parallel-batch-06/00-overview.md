...
持ったまま 2 箇所から使われる。** 担当 B が `wt_lock_acquire` へ印の確認を入れ、それが
`develop` へ載ってから、担当 C が起点を取り込んで切り出す。
...
| 2 | `set -C` を有効にした `: >"$dir/held"` | 持ち主を 1 つに決める。シェル自身の `open(O_CREAT|O_EXCL)` が関門になる |
...
| 担当 B ↔ 担当 C | `set -C` は呼び出し側のシェルの状態を変える。**元へ戻す責務がどちらにあるかは、共通ファイルの契約として担当 C が決める** |
