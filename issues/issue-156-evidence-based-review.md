...
- [ ] 各担当は、自分の指摘を出し終えるまで、同じラウンドの他の担当の投稿と結果を参照しない
      （参照してよいのは起動時に渡された既存コメントのスナップショットだけである）
- [ ] 指摘は自由文に加えて、根拠（`evidence`）と反証条件（`falsification`）を持つ
- [ ] 総評だけへ投稿した指摘も、インラインへ投稿した指摘と同じ形で記録される
- [ ] 総評だけへ投稿した指摘のうち位置を持つものが、新規性の判定と振動の検知の母集合へ入る
      （一致の判定・層の順序・閾値は変えない）
