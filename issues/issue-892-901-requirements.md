...
- [ ] AC10: worker の起動指示の `置き場所` の項目が、worker に `## 作業の報告` の写しをそのファイルの末尾へ書かせ、その後ろに最後の行として完了の目印 `<!-- 作業の報告の終わり -->` を 1 行書かせる。supervisor の写しの待ちはこの目印の行を待つ。`置き場所` を「無し」にしない。supervisor は worker を起動する前に、`置き場所` のファイルを worker ごとに新しいパスで空に作る（`: > <置き場所>`）
...
| 報告の写し | worker が `置き場所` のファイルの末尾へ書く `## 作業の報告` の節。最後の応答の報告と同じ中身で、最後の行に完了の目印 `<!-- 作業の報告の終わり -->` を置く |
