...
- [ ] AC10: worker の起動指示の `置き場所` の項目が、worker に `## 作業の報告` の写しをそのファイルの末尾へ書かせ、書き終えた後に完了の目印として空のファイル `<置き場所>.done` を作らせる（長い出力が無くても書かせる）。supervisor の写しの待ちはこのファイルの出現を待つ。`置き場所` を「無し」にしない。supervisor は worker を起動する前に、`置き場所` のファイルを worker ごとに新しいパスで空に作る（`: > <置き場所>`）
- [ ] AC11: supervisor → worker の 2 段を Claude Code で再現する。worker 役は規則 5 に反して、背景で 30 秒の待ちを残したまま応答を終えるよう指示して再現する。supervisor は worker の途中の通知で止まらず、worker の `## 作業の報告` を受け取ってから次の段へ進む。conductor が報告なしで続けさせた回数は 0 になる
...
| 報告の写し | worker が `置き場所` のファイルの末尾へ書く `## 作業の報告` の節。最後の応答の報告と同じ中身で、書き終えた後に完了の目印として空のファイル `<置き場所>.done` を作る |
