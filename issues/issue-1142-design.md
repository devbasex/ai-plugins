...
| I13 | （全体） | hook とラッパーの経路は、決まった環境の python から起動し、`deps.require()` を呼ばない（決定 20）。環境を用意するのは SessionStart とラッパーの複製の 2 か所だけで、ほかの hook は用意の有無だけを見る。hook ではないエントリポイントで外部パッケージを使うものは、外部パッケージの import より前に `deps.require()` を呼ぶ | 構造チェック（`hook-deps`）が落とす |
