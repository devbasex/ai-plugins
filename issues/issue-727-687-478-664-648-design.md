...
`--exclude` にホストがあれば弾く検査も、ホストを受け取るこの関数が持つ。Skill が持つのは、
...
| 使える者の解決（`assignment.resolve_participants`） | 母集合・ホスト・足す・外す・`only`・確認・`require_all` から `Participants` を返す。名前の矛盾（`--exclude` にホストを含む）と欠けを `AssignmentError` で返す | P6 |
