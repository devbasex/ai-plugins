...
- [ ] GitHub の読み書きが 1 つのライブラリを通り、REST にある操作は常に REST で行う。GraphQL を使うのは REST に無い操作（レビューのスレッドの解決状態の読み取りと resolve・Projects のボード・draft を ready にする）だけで、GraphQL が上限のときはその操作だけ回復の時刻まで待ってからやり直す（g）
