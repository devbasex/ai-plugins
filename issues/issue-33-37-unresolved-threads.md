...
- 継続的インテグレーションから Python の単体テストを実行する設定は入れない。範囲外として
  issue #182 へ由来を残した
...
- [x] 1. 引き継いだ指摘が記録されているとき、そのラウンドの判定が両者とも承認でも収束せず、修正の工程へ進む（自動テスト）
- [x] 2. 引き継いだ指摘が無いとき、収束の判定が現行と変わらない（既存テストと自動テスト）
- [x] 3. 引き継いだ指摘に対して修正の工程を 1 度通した後は、両者が承認したラウンドで収束する（自動テスト）
- [x] 4. 前のラウンドが修正必須の判定だったのに修正の記録が無い状態で次のラウンドを開始すると、終了コード 5 で失敗する（自動テスト）
- [x] 5. 前のラウンドで Resolve したと申告されたスレッドが GitHub 側で未解決のまま残っているとき、次のラウンドの開始が終了コード 5 で失敗する（自動テスト）
- [x] 6. 未解決の指摘を数える経路が取得の失敗と 0 件を区別する。取得できないときは判定を止めず、その旨を出力へ残す（自動テスト）
- [x] 7. 最終スイープの結果を検証する経路があり、未解決の指摘が 0 件なら終了コード 0、残っていれば終了コード 6 を返す（自動テスト）
- [x] 8. 0 件にできないとき、残った件数と理由が完了報告（`state.py report` の出力）に含まれる（自動テスト）
- [x] 9. 修正サブエージェントを起動するテンプレートと最終スイープのテンプレートの両方に、投稿数ではなく未解決の指摘を数え直す指示がある（文書の確認）
- [x] 10. 判定の流れが `docs/01-state-and-review.md` に書かれている（文書の確認）
- [x] 11. `uv run --with pytest pytest plugins/ndf/skills/cross-review/tests -q` が通り、既存の 148 件が引き続き通る
...
| 指摘 | 何が起きるか | 直し方 |
| --- | --- | --- |
| 再開のたびに `carried_over.fixed_in_round` を `null` へ戻す | 修正の工程を通した後でも、Resolve されない指摘（`deferred` / `rejected` と最終スイープ待ち）が残っているだけで再開のたびに 1 ラウンド増える。「増えるラウンドは最大 1 回」という不変条件が再開の回数だけ破れる | 前の記録に無い識別子を含むときだけ数え直す。含まなければ `fixed_in_round` をそのまま残す |
| 手順の `eval "$(スクリプト)"` | コマンド置換の終了コードは `eval` の終了コードにならない。出力が空なら 0 で通るため、`start-round` が後始末の未了で exit 5 を返しても止まらず、前のラウンドの `$ROUND` のまま Step 2 へ進んでループが止まらない | 変数で受け、終了コードを見てから `eval` する。`start-round` は exit 1 でループを抜け、それ以外はその場で止める |

2 件目は `init` と `rotate-pr.sh` の呼び出しにも同じ形があったため、手順の 3 ファイルで揃えた。手順の bash は実行されないため、`eval "$(` が行頭に現れないことを `tests/test_procedure_eval_exit_code.py` で検査する。
...
- [x] `uv run --with pytest pytest plugins/ndf/skills/cross-review/tests -q` が通る
- [x] `bash scripts/validate-runtime-plugins.sh` が終了コード 0 で終わる
- [x] `claude plugin validate` が通る
- [x] 配布物の同期を同じ Pull Request に含めた
