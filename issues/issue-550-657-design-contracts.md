...
| ある | `関門` | `提示物` を読み、`AskUserQuestion` で承認を求める。承認の後の動きは `関門` の値で決める。`設計 Pull Request のマージ` なら親がマージしてから `次の窓` を起動する。`本番の系へ届く操作` ならマージせずに `次の窓` を起動する（`operation` では設計の窓がこの値を返す）。親はモードや窓の名前から分岐しない |
...
**親が再開するのは、自分が起動した窓（深さ 1）だけである。** 孫の中断は、その孫を起動した窓が再開された後に、自分の窓にある孫の `agent_id` へ同じ手順 2〜3 を行う。親が孫を直接再開すると、再開された窓と孫が同じ作業や外部への書き込みを重ねる。
...
| `interrupted` | `--session <ID>` / `--format md\|json` / `--now <ISO 8601>`（テスト用） | `ending` が `rate_limit` で、**深さが 1 の窓だけ**（孫は返さない）。`resets_passed`（真偽）を足す | 0 |
...
2 つ目の表は**窓の名前とモデルの組**ごとの束ね。窓ごとにモデルを変えられる（決定 10）ため、同じ窓の名前でもモデルが違えば別の行にする。**応答数が 3 に満たない記録は外し、外した件数を表の下に 1 行出す。**

```text
| 窓 | モデル | 件数 | 固定費の中央値 | 実作業の中央値 | 実作業 < 固定費 | 最大充填の最大 | 印 |
```
...
| 印 | 条件 |
| --- | --- |
| `束ねる候補` | `実作業 < 固定費` の件数 > 件数の半分。**窓の名前が `設計` の窓には付けない**（設計の窓は、作るときはどのモードでも必ず関門を返す。`operation` では関門 2）。`取り込み` は関門 2 を返すかがリポジトリで変わり、記録はそれを持たないため、名前では除かない |
