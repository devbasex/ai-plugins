...
NDF v8.5.3 時点の状態と、次に触る人が知っておくことをまとめる。
...
| 他者の Pull Request | 5 回とも自分の Pull Request を対象にした。投稿の event を倒さない経路は未確認 |
...
| 事象 | 内容 |
| --- | --- |
| 差分予算の超過 | `long_method` の抽出は見積より膨らみやすい。5 回目のラウンド 1 では 2 件が予算超過で落ちた（277 行 / 240 行、113 行 / 100 行） |
| kiro の既定モデル | `auto` は実際に選ばれたモデルを取得できず、そのラウンドは集計から分離される。モデルを比べるなら `--model kiro=<name>` を指定する |
