...
### 配布そのものを目的とする変更を、次の配布の対象に含めない

`AGENTS.md` は main への直接コミットを禁じるため、版を上げる変更も Pull Request を通る。
その Pull Request をマージすると、上の「時期」の定めだけではそれ自体が次のまとまり
（1 件）に見え、また版を上げることになる。工程が終わらないため、終端条件を置く。

> **配布そのものを目的とする変更（版数と説明文書の記載だけを変える Pull Request）は、
> 次の配布の対象に含めない。**

`release` は「配布物に差分が無ければ飛ばす」開始条件を既に持っている。版を上げる
Pull Request の差分は、利用者へ届く振る舞いを何も変えていない。同じ判断に含める。

この終端条件は `plugins/ndf/skills/release/SKILL.md` の開始条件として書く。Skill 本体の
変更は担当 B の Pull Request（#192）で行う。
