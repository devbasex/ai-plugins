...
| 空白や制御文字を含むパスでも案内が通る | 空白を含むパスとタブを含むパスを未作成のまま 1 つずつ渡す。`HINT: mkdir -p ` に続く部分を `bash -c 'eval "set -- $1"; printf "%s\0" "$@"' _ <その部分>` で読み戻し、語が 1 つで渡したパスと一致することを見る |
...
**読み戻しに `shlex.split` を使わないのは、`$'...'` の形を解さないためである。** タブを含むパスの
出力 `$'/tmp/x/tab\tx'` を `['$/tmp/x/tab\\tx']` と読み、先頭に `$` が残ってタブも戻らない（実測）。
案内を読む側のシェルを bash と定めた決定 6 に揃え、bash で読み戻す。
