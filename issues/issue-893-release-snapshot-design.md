...
- 変更の判定は、段の前後の `git status --porcelain -uall` の差で行う。段の前から変わっていたパスは数えない
...
| `--min-version` | 前の記録の最後の版 | 表の最初の行。`--out` の中で最も新しい `.json` の `per_pr` の版のうち最大のもの |
...
書き出すファイル名は `<集計日>.md` / `<集計日>.json`。どちらかが既にあれば `<集計日>-2`・`-3` と進める。
...
| AC9 | 同: 同じ `--until` で 2 回作った `.json` が一致する |
