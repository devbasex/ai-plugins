...
- 変更の判定は、段の前後の控えの差で行う。控えは `git status --porcelain -uall` が挙げたパスごとの内容の要約（`git hash-object`。消えたパスは「無し」）である。段の後に、控えに無いパスが現れたか、要約が変わったパスを「段が変えたパス」とする。段の前から変わっていたパスも、段が中身を変えれば数える
...
**前の記録は、`--out` の `.json` のうち `meta.until` がこの打ち切りの時刻より前のもので最も新しいものである。** `meta.until` が無い記録（2026-09-23）は、どの打ち切りよりも前とみなす。同じ打ち切りの時刻で走らせ直しても、自分が前に書いた記録を前の記録に取らない。

書き出すファイル名は `<集計日>.md` / `<集計日>.json` である。`meta.released` と `meta.until` が同じ記録が既にあれば、そのファイルへ書き直す（走らせ直しは同じ名前になる）。別の記録が同じ名前を持っていれば `<集計日>-2`・`-3` と進める。

`.json` の形は `token-usage.py --by version,mode,model,cc,reviewers --format json` と同じである。`meta` にだけ `released` と `previous`（前の記録のファイル名）を足す。
...
`token-usage.py` を 2 回走らせると、記録の読み込みも 2 回（実測で各 35 秒）になる。`collect(...)` を切り出して読み込みを 1 回にする。ファイル名にハイフンがあるため、`importlib` でパスから読み込む。

### 決定 7: 同じ日の記録があるときは `-2` を付けて別に作る

`<集計日>` の形は PR #899 が決めた。同じ日に 2 度出すと衝突する（2026-09-24 には既に記録がある）。上書きは、コミット済みの記録を作業ツリーで消すことになる。
...
| AC2 | `test_release_steps.py`: 宣言なしで出力なし・0。段階が合う段だけを実行する。`{version}` の置き換え |
| AC3 | 同: `writes` の外にファイルを作る段で 1。段の前からあった変更は数えない。`stage` の不一致で実行しない |
...
| AC9 | 同: 同じ `--until` で 2 回作った `.json` が一致し、2 回目は同じ名前へ書く。前の記録に 1 回目の出力を取らない |
