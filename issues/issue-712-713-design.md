...
**2 つとも満たすものだけを候補にする。** 欠けたときの行き先を列で持ち、既存の 7 判定の
どれかへ倒す。形は同じ Skill の `no-work.md` の「2 つの必要条件」に合わせる。

| # | 条件 | 確かめ方 | 欠けたときの行き先 |
| --- | --- | --- | --- |
| 1 | 3 件以上が、同じ修正レイヤーを指す | 段 2A が控えた修正レイヤーを段 2B で並べ、同じ層を指す件数を数える | **既存の判定のまま。** 2 件以下のときも、層を名指しできないときも同じである |
...
**1 段上げても層を名指しできないなら、その課題は他と同じ層を指していない。** 条件 1 が
欠けるだけなので、判定は既存のまま置く。人へ返さない。

## 採る手

**5 つに限る。** **手は判定の条件ではない。** 親 issue に必ず書く項目であり、1 つに
決まらないときは当てはまるものを全部書く。決まらないことを理由に判定を止めない。
