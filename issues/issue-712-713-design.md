...
**結び付きの既定は GitHub のサブイシュー関係である。** 多数のケースは 1 対多で、この機能が
そのまま当てはまる。**2 つ目以降のクラスタだけ本文の 1 行で指す。** 子 1 件が持てる親は
1 件だけであり、複数のクラスタへ属する課題は関係だけでは扱えないためである。サブイシューの
API を持たないリポジトリでは、すべて本文の 1 行にする。
...
**既定はサブイシュー関係である。** 付けるときに渡すのは issue の番号ではなく、
データベースの ID である。
...
**2 つ目以降のクラスタと、サブイシューの API を持たないリポジトリでは、本文へ 1 行足す。**
