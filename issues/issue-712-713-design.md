...
| # | 条件 | この 6 件では |
| --- | --- | --- |
| 1 | 2 件以上が、同じ修正レイヤーを指す | 6 件とも「失敗の扱いを決める共通の層」。満たす |
...
| # | 条件 | 確かめ方 | 欠けたときの行き先 |
| --- | --- | --- | --- |
| 1 | **2 件以上**が、同じ修正レイヤーを指す | 段 2A が控えた修正レイヤーを段 2B で並べ、同じ層を指す件数を数える | 1 件なら**追記が要る**。その課題の本文へ修正レイヤーと採る手を書く |
...
**1 件でも、修正レイヤーが現象レイヤーと違うなら上位で直す。** 条件 1 が決めているのは
親 issue を作るかどうかであって、どこで直すかではない。1 件のときに親 issue を作ると、
同じ内容の課題が 2 件並ぶだけで指す先が増えない。代わりに、その課題の本文へ修正レイヤーと
採る手を書く（既存の判定の「追記が要る」）。
...
| **修正レイヤー** | **直すべき場所。その責務を持つべき場所までさかのぼる。段 2B がクラスタを確定する材料** |
...
flowchart TD
    ...
    B0 --> B["控えを並べる"]
    B --> C{"同じ修正レイヤーを<br/>指す課題が 2 件以上か"}
    C -->|1 件| D["判定 = 追記が要る<br/>その課題へ修正レイヤーと手を書く"]
...
| 箇所 | 現状 | 直す形 |
| --- | --- | --- |
| `no-work.md` の条件 2 の行き先 | 「重複」固定 | 寄せ先が同じ課題なら「重複」。同じ課題ではなく、同じ修正レイヤーを指すものが 2 件以上あって 2 条件を満たすなら「クラスタ化」。どちらでもないなら寄せ先が無く、条件は欠けていない |
| `no-work.md` の条件 2 の確かめ方 | 「段 2B の重複の突き合わせの結果を使う」 | 重複とクラスタ化の両方の突き合わせの結果を使う（重複が先、クラスタ化が後） |
| `SKILL.md` 段 2B の注記 | 起点が同じ 2 件を重複の枠で説明する | 個別の作業が残るものはクラスタ化へ倒す分岐を足す |
...
| 項目 | 条件 | 実現方式 |
| --- | --- | --- |
| 外部への書き込みの量 | 1 クラスタにつき起票 1 件と、子 issue 2 件以上の本文の更新が増える | 既存の「外部への書き込みの制限」の待ち方をそのまま使う。**処理するクラスタの数に上限を置かない** |
