...
**重複との境目は条件 2 である。** 直せば全部が消えるなら同じ課題（重複）、消えずに個別の
作業が残るなら別の課題（クラスタ化）である。

**件数が決めるのは親 issue を作るかどうかであって、どこで直すかではない。** 1 件でも
修正レイヤーが現象レイヤーと違うなら上位で直す。親 issue を作らないのは、指す先が増えずに
同じ内容の課題が 2 件並ぶためである。
...
| `issue-upkeep/references/grouping.md`（新設） | クラスタ化の条件 2 つ・修正レイヤーの決め方・採る手の 5 つ・親 issue に書く 4 つ・結び付け方 |
