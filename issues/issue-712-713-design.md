...
クラスタ化の条件 2 は「同じ課題ではない」ことを確かめるため、その結果を材料にする。

正本が決まった後に控えを並べると、6 件が同じ修正レイヤーを指していることが分かる。ここで
3 つの条件を確かめる。

| # | 条件 | この 6 件では |
| --- | --- | --- |
| 1 | 3 件以上が、同じ修正レイヤーを指す | 6 件とも「失敗の扱いを決める共通の層」。満たす |
| 2 | 修正レイヤーを直しても、各課題に個別の作業が残る | 層を作った後も、6 か所それぞれに当てはめる作業が残る。満たす |
| 3 | 修正レイヤーが 1 つに名指しでき、採る手が 1 つに決まる | 層がまだ無いため、手は新設。満たす |
...
**各所への当てはめは手の数に数えない。** 条件 3 が数えるのは修正レイヤーと手の数であり、
6 か所それぞれに残る作業は条件 2 が残ると定めたものである。当てはめ先がスクリプトと設定に
分かれても、直す層は 1 つのままである。

### 段 3 — 反映する

親 issue を 1 件つくり、子 issue 6 件へリンクを足す。**どちらも外部から見える場所への
...
| # | 条件 | 確かめ方 | 欠けたときの行き先 |
| --- | --- | --- | --- |
| 1 | 3 件以上が、同じ修正レイヤーを指す | 段 2A が控えた修正レイヤーを段 2B で並べ、同じ層を指す件数を数える | 2 件以下なら**既存の判定のまま**。層を名指しできないなら**要判断** |
| 2 | 修正レイヤーを直しても、各課題に個別の作業が残る | 課題ごとに、修正レイヤーを直した後に残る適用を 1 行で書けるか見る。先に済ませた重複の突き合わせの結果を材料にする | 残らないなら**重複** |
| 3 | 修正レイヤーが 1 つに名指しでき、採る手が 1 つに決まる | 下の 5 つから手を 1 つ選び、親 issue の完了条件を「その層への 1 つの変更」の形で書き下してみる | 層が 2 つ以上にまたがる、または手が 2 つ以上要るなら**要判断** |

**重複との境目は条件 2 である。** 原因を直せば全部が消えるなら、それは同じ課題（重複）で
ある。消えずに個別の作業が残るなら、原因は同じでも別の課題であり、クラスタ化に当たる。

**条件 1 が欠けたときの行き先は既存の判定のままである。** 重複はこの前の突き合わせで決まって
おり、2 件以下で重複でもないなら寄せ先が無い。行き先を「重複の突き合わせへ」と書くと、
条件 2 がその結果を待つ形になり、判定が循環する。

## 修正レイヤーの決め方

**現象レイヤーと修正レイヤーを分けて書く。** 子 issue が現れているのは呼び出し側であり、
直すべき場所はその上位にある。呼び出し側を 1 件ずつ直す形と、上位の層を 1 度直す形は、
分けて書かないと同じ記述になる。
...
**上位へ 1 段だけ上げる。** いくつも上げると直す場所が抽象的になり、着手できない。
1 段上げても責務が収まらないなら、そのクラスタは**要判断**へ倒す。

## 採る手

**5 つに限る。** 手が決まらないクラスタは着手できないため、**要判断**へ倒す。
...
**結び付きは本文の 1 行だけで持つ。** GitHub のサブイシュー機能（`sub_issues` の API）は
使わない。子 1 件が持てる親は 1 件だけであり、1 件が複数のクラスタへ属することを扱えない。
用語はこの機能の語彙から採るが、関係の実体は本文のリンクである。
...
## 採る手
（移動 / 統合 / 新設 / 向きの修正 / 分離 のいずれか 1 つ。手法の呼び名があれば添える）

## どの課題がこれを指すか

| 子 issue | 現象レイヤー | その課題で何が起きているか |
| --- | --- | --- |
| #<番号> | （現れている場所） | （1 行。再現手順の本体は子 issue に残す） |

## 直すと何が変わるか
（修正レイヤーを直した後、各課題に何が残るか）

## 完了条件
（修正レイヤーへの 1 つの変更として書く）
...
| **修正レイヤー** | **直すべき場所。現象レイヤーの 1 段上。段 2B がクラスタを確定する材料** |

## 処理の流れ

```mermaid
flowchart TD
    A["段 2A: 課題ごとに調べる<br/>現象レイヤーと修正レイヤーを控える"] --> B0["段 2B: 重複の突き合わせを行い<br/>正本を決める"]
    B0 --> B["控えを並べる"]
    B --> C{"同じ修正レイヤーを<br/>指す課題が 3 件以上か"}
    C -->|いいえ| D["既存の判定のまま"]
    C -->|はい| E{"修正レイヤーを直しても<br/>個別の作業が残るか"}
    E -->|残らない| D2["判定 = 重複"]
    E -->|残る| F{"修正レイヤーが 1 つで<br/>採る手が 1 つに決まるか"}
    F -->|決まらない| G["要判断へ倒す"]
    F -->|書ける| H["判定 = クラスタ化"]
    H --> I["親 issue の起票と子 issue の<br/>本文の更新を一括で提示し承認を得る"]
...
別のクラスタの親 issue を指していることがある。1 行が指す親 issue の修正レイヤーを読み、
このクラスタのものと一致するときだけ飛ばす。
...
| 箇所 | 現状 | 直す形 |
| --- | --- | --- |
| `no-work.md` の条件 2 の行き先 | 「重複」固定 | 寄せ先が同じ課題なら「重複」。同じ課題ではなく、同じ修正レイヤーを指すものが 3 件以上あって 3 条件を満たすなら「クラスタ化」。どちらでもないなら寄せ先が無く、条件は欠けていない |
...
| 中断からの再開 | 書き込みの制限で途中で止まっても、やり直しで親 issue が 2 件にならない | 子 issue の 1 行が指す親 issue の修正レイヤーを読み、このクラスタのものと一致すれば起票を飛ばす |
...
| AC35 | `test_grouping_separates_the_two_layers`（現象レイヤーと修正レイヤー、1 段だけ上げること） |
