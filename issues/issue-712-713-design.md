...
| # | 条件 | 確かめ方 | 欠けたときの行き先 |
| --- | --- | --- | --- |
| 1 | 3 件以上が、同じ修正レイヤーを指す | 段 2A が控えた修正レイヤーを段 2B で並べ、同じ層を指す件数を数える | 2 件以下なら**重複**の突き合わせへ。そこで重複と決まらなければ**既存の判定のまま**。層を名指しできないなら**要判断** |
| 2 | 修正レイヤーを直しても、各課題に個別の作業が残る | 課題ごとに、修正レイヤーを直した後に残る適用を 1 行で書けるか見る。段 2B の重複の突き合わせの結果と合わせて読む | 残らないなら**重複** |
...
```mermaid
flowchart TD
    A["段 2A: 課題ごとに調べる<br/>現象レイヤーと修正レイヤーを控える"] --> B["段 2B: 控えを並べる"]
...
決定が 14 件あり、設計文書が 500 行を超えるため分けた。
