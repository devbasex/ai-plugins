...
キュー（不足 i）はこの文書の対象と同じ基準だが、試行（`experimental/runner-trial.py`）の結果で決めるため、
別の決定にする。
...
| 9 | 版数の比較と一括の書き換え | `instructions_lib/versions.py`・`relay_lib/version_dir.version_key`・根の `token-usage.py:version_key`・`check-doc-staleness.py:base_of`・`release-steps.py` の `Editor`・`cmd_bump` ほか | 約 220 | semver（比較）・bump-my-version（15 箇所の書き換え。試行で決める） | `version_dir`（ラッパー） |
...
| 17 | `claude -p` の起動と結果の読み取り | `supervise_lib/claude.py` の `claude_cmd`・`call_claude`・`mvv-gate.py:ask`・`steps.last_json` | 約 100 | claude-agent-sdk（CLI をサブプロセスで起こす。サーバーは立てない。試行で決める） | なし |
...
| 20 | 同じ処理の重複 | remote の URL から owner/repo を取る処理が 5 か所・原子的な書き込みが 4 か所（`lib/jsonio.write_atomic` と重複）・中央値と分位点（`statistics` で足りる） | 約 80 | 1 か所へまとめる（URL は giturlparse を試す。書き込みは `lib/jsonio`。統計は標準ライブラリ） | 一部 |
...
| `lib/locks.py` | filelock | 7（Python の側だけ。シェルの `lock-common.sh` は決定 20 で扱う） |
| `lib/shparse.py` | tree-sitter-bash | 8（決定 20 の結果で置く） |
...
そこへ足す。mcp-serena の `project_yml.py` は別のプラグインで宣言の置き場が無いため、この課題の範囲の外に置き、
起票する。
...
## 決定 20: hook とラッパーの経路（I13）をどう扱うか — 利用者の判断が要る
...
自作が 3 本）で、PreToolUse の hook が Bash と Edit のたびに呼ぶ。
...
**案 A（I13 を保つ）**: hook とラッパーの経路は標準ライブラリとシェルのまま残し、重複だけを 1 か所へまとめる
（ヒアドキュメントの除去の 3 本を 1 本へ）。上乗せは 0 で、置き換えの対象は約 2,300 行減る。

**案 B（hook を 1 本の Python プロセスへまとめ、依存を持たせる）**: PreToolUse・Stop・Notification の hook を
lock 済みの uv の環境で動く 1 本の Python のエントリポイントへまとめ、シェルの字句解析を tree-sitter-bash へ、
`.env` と Slack と ロックをライブラリへ置き換える。1 回の上乗せは 2 回目以降で約 10〜20 ms 見込みで、今の
bash と jq の fork（1 回 30 ms）の一部が消えるため、差し引きは試行で測る。ラッパーのバージョンディレクトリは、
複製のときに同じ lock から uv の環境を作る（`uv sync --frozen` を複製先で打つ）。

**推奨は案 B を試行で確かめてから決めること**である。試行 T2（`experimental/`）で、今の hook のテストの
入力のすべてを tree-sitter-bash で解析して同じ判定になるか、hook 1 回の所要（冷えた状態と温まった状態）、
uv の無い環境で hook が止まらずにパススルーへ落ちるか（hook が止まると Tool の呼び出しが全部止まる）を測る。
成り立たなければ案 A にする。**どちらにするかは利用者が決める**（承認ゲート 1）。
...
| ステージ | 移行ステップ | 触るファイル |
| --- | --- | --- |
| 0 | T2 試行（決定 20 の案 B・claude-agent-sdk・bump-my-version） | `experimental/` だけ |
...
| 2 | D3 外部 CLI と記録 | C3 と同じファイル（`wait-notify.py` は決定 20 で扱う） |
| 2 | D4 cross-refactoring | C4 と同じファイル |
| 2 | D7 リリースと文書 | C7 と同じファイルと `glossary.py`・`doc-lint.py`・`spec-copy.py`・`pr-steps.py` |
| 2 | D8 根の scripts と playwright-kit | 根の `scripts/`（`check-*.py`・`build-runtime-plugins.sh`・`validate-runtime-plugins.sh`・`token-usage*.py`）・playwright-kit の `config.py` |
| 3 | D5 hook（案 B のときだけ） | `scripts/worktree-*.sh`・`token-guard.sh`・`lib/worktree-*.sh`・`lib/token_guard_sleep.py`・`workflow-common.sh`・`wait-notify.py`・`hooks/*.json` |
| 3 | D6 ラッパー（案 B のときだけ） | `relay_lib/`（`proc`・`common`・`record`・`terminal`・`version_dir`・`mark`） |
...
- 上の 20 種類のうち、決定 20 の結果で対象にしたものが包みの上に置き換わり、自作の関数が消えている
- 構造チェックの I14 が、包みの外で包みの受け持つ部品を使うモジュールを落とす
- 置き換えないものの一覧（この文書）に載っていない汎用の自作が、新しく足されていない
- hook 1 回の所要が、移行の前と比べて悪くなっていない（案 B のとき。T2 と同じ測り方）
