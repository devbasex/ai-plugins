...
flowchart TD
    ...
    Skip --> SkipOK{"通る ?"}
    SkipOK -->|いいえ| SkipFix["修正ラウンド<br/>上限に達したら失敗として報告"]
    SkipFix --> SkipOK
    SkipOK -->|はい| Report["報告"]
    CR --> Report
```
...
**適用ラウンドだけは親の層が 2 つある**ため、全体の最悪は両方の回数の合計に上限を掛ける。
...
| **適用ラウンド = 1 コミット** | **適用ラウンド。適用ラウンドどうしは重ならないので、群の間は常に分離できる（群の中は分離しない）** | **採る** |
...
**依頼の「継続的統合のテストで代替してよい」は Step 7 に掛かる。** 群ごとに push して
継続的統合を待つ形は、**未検証の変更が公開されない**という方針（前提 3）と両立しない。
**Step 5 は手元のテストを必須とする。**
...
**Pull Request へ出すすべての文章に入れる**（検証の結果・修正の要約・進行の報告）。
...
**2 回目に出るのは 1 回目の挙げ漏らしだけである。** 2 回目で新しい提案が出なければ収束する。
**上限は歯止めであって、回数の指定ではない。**
...
| 判定 | 手元の全体のテストと継続的統合の**どちらか一方が通れば通過**。**どちらの結果も得られなければ通過させない**（fail-closed） |
...
**理由**: **取り消しの単位と一致する。** 適用ラウンドどうしは重ならないので、コミットを
戻せば**群の間は**常に分離できる。**群の中は分離しない**（「群の中の道連れ」の節）。
...
| B2 | 新しいテスト（**`--ci-check` があれば `check-runs` の結果で全体のテストを代替し、どちらの結果も得られなければ通過させない**） |
