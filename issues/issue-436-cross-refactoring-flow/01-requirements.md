...
```mermaid
flowchart TD
    C6["6. テスト整備を前へ"] --> C2["2. Step 5 をテストへ"]
    C2 --> C3["3. Step 7 の省略"]
    C8["8. 提案と適用のラウンドを分ける"] --> C7["7. 取り消しの単位"]
    C8 --> C4["4. ラウンド上限 3"]
    C7 --> C1["1. コミット粒度"]
    C5["5. 提案の時点で防ぐ"] --> C7
    C9["9. 外へ出す文章"]
...
- [ ] B4. テストの整備がラウンドより前の工程として分かれている（または分けない理由がある）
- [ ] B5. Step 7 を省略する条件と、その見分け方が定まっている
- [ ] B6. テストで見つからない誤りを拾う工程が示されている

### C. 提案（変更 5）

- [ ] C1. 提案プロンプトが、テストを足す先が対象範囲の中にあることを確かめさせる
- [ ] C2. 提案プロンプトが採用上限を伝える
...
| 提案・適用・検証・報告の各プロンプト | 提案の語彙（兆候・手法の一覧） |
| ラウンドと上限の定義 | 輪番の決め方（`lib/assignment.py`） |
| 改修計画の書式・置き場所・参照 | 改修計画を残す方針そのもの |
...
| 確認してから行う | ラウンドの定義の変更、上限の既定値、Step 7 の省略の条件 |
