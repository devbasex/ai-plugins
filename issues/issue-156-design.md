...
| 3 | 重複の統合・反証・実行検証・証拠ベース集約 | 収束の判定そのものを変える |
...
| 項目 | 何を書くか | 無いときの扱い |
| --- | --- | --- |
| `evidence` | 根拠。対象のコードと到達経路 | `has_evidence` が偽になり、集約で「根拠なし」として扱う |
...
**`has_evidence` は取り込みのときに付ける派生の項目である。** レビュワーは書かない。
`cmd_read_result` が `evidence` と `falsification` の**両方**が空でないときだけ真にする。
片方だけで真にすると、反証条件の無い指摘が「根拠あり」として集約へ渡る。「指摘の取り込み」の
...
`_finding_keys` の 1 箇所だけで（`state.py:2510-2554`）、読んだ内容は状態ファイルへ残らない。
...
### 収束の判定への影響（Pull Request 2）

**`comments[]` を広げると、その時点で収束の判定の入力が変わる。** `_finding_keys` が返す
並びは終了基準の 3 層のうち 2 つが使う（`docs/05-pool-and-convergence.md:40-44`）。どちらも
`payload.json` の `comments[]` を母集合にしており、Pull Request 3 を待たずに値が動く。

| 層 | 読む経路 | 何に効くか |
| --- | --- | --- |
| 新規性 | `cmd_judge` → `_new_finding_count` → `_finding_keys`（`state.py:2390`） | 新規が 0 件なら収束する |
| 振動 | `cmd_check_oscillation` → `_finding_keys`（`state.py:2628`） | 重複率 0.5 以上で中断する |

**母集合が増えるのは、位置を埋めるからである。** `_finding_keys` は `path` と `line` の
両方を持つ要素だけを鍵にする（`state.py:2549`）。総評だけへ投稿した指摘にも位置を埋める
という上の決めごとによって、これまで数えられなかった指摘が両方の層へ入る。

**向きは 2 つの層で違う。** 新規性は、前のラウンドで見えていなかった指摘が新規として
数えられるため、**ラウンドが増える方向**へ動く。振動は、増えた分が前のラウンドと
一致するかで決まるため、重複率は上下どちらへも動きうる。

**この変化を受け入れる。** 見えていない指摘を 0 件として収束するほうが害が大きい。#156 が
実測した「レビュー body だけの指摘が進行側から見えない」（PR #157 の round 4）は、この
母集合の欠落そのものである。

**Pull Request 3 との境界は、母集合と式の間にある。** Pull Request 2 が動かすのは母集合
だけで、一致の判定（位置・近傍 3 行・本文）も、層の順序も、振動の 0.5 も、収束の上限も
触らない。**式を変えるのは Pull Request 3 である**（根拠と反証条件を読んで指摘を選り分ける
証拠ベースの集約）。**母集合を広げる変更と、広げた母集合をどう判定するかを変える変更を、
同じ Pull Request に載せない。** 載せると、ラウンド数が動いたときにどちらが原因かを
切り分けられない。
...
| 総評だけの指摘が新規性の判定に入る | 前のラウンドに無い `posted_to: body` の指摘を 1 件足したラウンドで、`cmd_judge` の `NEW_FINDINGS` がその 1 件を数えること |
| 総評だけの指摘が振動の検知に入る | 同じ `posted_to: body` の指摘を 2 ラウンド続けて出したとき、`cmd_check_oscillation` が重複として数えること |
| 位置を持たない指摘は判定へ入らない | `path` か `line` が欠けた要素を含む `payload.json` で、両方の層の件数が変わらないこと |
...
| 重複の統合と `origin_runtimes` | Pull Request 3 で決める。統合するかは根拠を突き合わせて決まるため、集約と同じ Pull Request に置く。Pull Request 2 の `review_findings[]` は担当ごとの指摘を `agent` を添えて並べるだけで、**統合しない** |
| 実行検証の対象 | リポジトリが持つコマンドをどう宣言させるかは Pull Request 3 で決める |
| 効果の測定の指標 | Pull Request 4 で決める。比較のための Pull Request 群も未定 |
| 振動の検知への影響 | 母集合が広がることは Pull Request 2 のテストで固定する。実際の重複率とラウンド数がどれだけ動くかは、記録が貯まるまで測れない。**測るのは Pull Request 3 の後** |
