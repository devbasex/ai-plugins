...
- **出典ごとに有限の待ちを置く**（引数 `--refresh-timeout` > 宣言の `refresh_timeout_seconds` > 既定 10 秒）。**待ちは 1 件あたりの総経過時間である** ── 単調に進む時計で期限を持ち、接続と読み取りの合間に残りを計り直す。**少しずつ返し続ける相手でも指定の秒数の付近で終わる**（接続と読み取りの無通信時間だけを見る形にしない）。**待ちを越えたら取得の失敗として扱い**、
