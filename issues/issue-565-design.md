...
| 要素 | 責務 |
| --- | --- |
| `scripts/lib/workflow-common.sh` の `wf_split` | 引用の外の制御演算子と行末で空の語（区切り）を出す（F1） |
...
| `a27ffb35`（9/12） | 2.1.269 | bypass / `/goal` | `"配布"; echo "exit=$?"` ほか 3 件 | 積まれない |
| `9e188b9b`（9/13） | 2.1.270 | bypass / `/goal` | `"要求と受け入れ条件"; echo "exit=$?"` | 積まれない |

**積まれなかった 5 件は、すべて閉じ引用符の直後に `;` が密着している。** 積まれた記録は
すべて空白を挟んで `2>&1` を続けている。
