...
| 引用の外で改行をまたぐ | 語の終わり。区切りは **2 行目以降の行頭**で出す。**直前の行が `\` で終わっていないときに限る**。本文の最後の改行（here-string が足すものを含む）では出さない |
| 引用の外で、行末の直前が `\`（バックスラッシュ + 改行） | 継続。`\` を捨て、語の終わりとし、区切りを出さない |
...
| 1 行目 `gh pr \`、2 行目 `merge 268` | `gh␀` `pr␀` `merge␀` `268␀` |
...
| AC12 | テスト一式の実行 | 通る |
| AC13 | 行末の `\` で継続した 3 つの本文（`gh pr merge \⏎  268 --merge` / `gh pr \⏎merge 268` / `bash plugins/ndf/scripts/projects-sync.sh \⏎  565 stage "設計"`）を parametrize | 1 つ目は通る。2・3 つ目は落ちる（マージと見なさない / 値が `\ 565 stage` に壊れる） |
