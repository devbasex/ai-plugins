...
`architecture` で独立したファイルにするのは、**設計として確定した内容と、実装の段階で入る計画の
更新を、別の差分として読めるようにする**ためである。仕様と同じファイルに書くと、進捗の記入や
タスクの追加が設計と同じファイルの差分に載る。`standard` で同じファイルを許すのは、設計の分量が
小さく、混ざっても読み分けられるためである。
...
| 触る領域 | 受け入れ条件と対象範囲から読む（永続データ・API・画面の有無） | 仕様を先に書く。設計の時点では実装の差分がないため、差分からは判断しない |

### 出すもの

| 出力 | 形 | 次に使う工程 |
| --- | --- | --- |
| 設計文書 | `issues/<課題名>/NN-design.md`。`standard` では仕様ファイルの設計の節でもよい | `implementation-plan` がタスクへ分解する |
...
**参照を 2 本に分けても、設計文書の節は「入出力の契約」1 つである。** 参照を分けるのは読ませる
分量を絞るためで、成果物の節の粒度とは別の判断による。API と画面の両方を触る変更では、1 つの節に
両方を書く。
...
flowchart TD
    ...
    I --> J[マージと後片付け]
    J --> K[実装用の作業ツリーを<br/>新しく作る]
    K --> H
```

**設計 Pull Request をマージした後、実装は新しい作業ツリーで行う。** `merged` が設計のブランチと
作業ツリーを消すため、そのまま実装を続けられない。`worktree` を実装用のブランチ名で呼び直す。
...
設計レビューは既存の 3 Skill を順に呼ぶ工程であり、新しい Skill を作らない。実装の Pull Request と
違うのは次の 2 点である。
...
マージした時点で課題が閉じ、実装の工程が残っていることが盤面と課題の一覧から見えなくなる。
課題を指すときは番号だけを書く。
...
### 決定 10: 設計 Pull Request で課題を閉じない

本文に課題を自動で閉じる語を書かない。設計 Pull Request は実装を含まない段階でマージするため、
閉じると残りの工程が追えなくなる。課題を閉じるのは実装の Pull Request である。

### 決定 11: 設計レビューに新しい Skill を作らない
