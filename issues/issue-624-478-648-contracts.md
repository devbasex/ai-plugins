...
**P4 の契約（`_classify_finding` の順 4 の条件と、`classification` の値）は [issue-732-624-706-design.md](issue-732-624-706-design.md) の「データ構造」「入出力の契約」へ移した。** 新しい設計は区分 `unrefuted` と項目 `unrefuted_reason` を足し、順 4 から根拠の条件を外す。印を外す契約（決定 5）はそのまま引き継がれた。以下の P4 の行は 2026-09-15 時点の記録として残す。
