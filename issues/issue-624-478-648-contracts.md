...
**P5 の契約は [issue-727-687-478-664-648-contracts.md](issue-727-687-478-664-648-contracts.md) が置き換える**
（親 #727 の設計、2026-09-19）。6 項目は 1 つの `participants` に畳まれ、`review_assign` は `review_seats` に、
`check_auth` は `probe_auth` に替わる。この文書の P5 の節は記録として残し、書き換えない。
