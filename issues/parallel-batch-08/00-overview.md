...
flowchart LR
    ...
    A --> E[E: #318 / #321<br/>SKILL.md の書き直し]
    C --> D[D: #331<br/>新しい Skill と配布物]
    C --> E
    B[B: #371<br/>cross-review だけ]
```

**図は依存を表し、下の表は着手の順序を表す。** B は他のどの担当とも同じ行を触らないため、
矢印を持たない。E は A と C が書き換えた `SKILL.md` の上に載るが、D とは触る箇所が
重ならない（D は配布物と新しい Skill だけを触る）。**逐次で進めるため、実際の着手は
D の後に E になる。**
...
| 5 | E | `SKILL.md` 全体を 300 行以内へ書き直す。**A と C の変更を取り込んでから行う**（D とは触る箇所が重ならないため、D を待つのは逐次で進めることによる） |
