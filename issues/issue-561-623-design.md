...
| F4 | まとまりの課題を、まとまりの終わりの工程で閉じる条件（本番への配布・リリース後テストの合格）を満たしたものだけ閉じ、閉じられなければ止まる | 進行側 |
...
| `plugins/ndf/skills/release/SKILL.md` | 後にリリース後テストも振り返りも続かない変更で、課題の手入れの前にまとまりを閉じる（「蓄積した課題を手入れする」の条件が「振り返りを通らない変更」からこれに変わる）。まとまりの範囲の確認を開始条件に持つ。**配布の記録を置く:** 配布の Pull Request の本文に `段階:`・`版:`・`まとまり:` の 3 行（出力物の行の形のまま）を置き、Pull Request を作らない形ではまとまりの最後にマージした Pull Request へ出力物のブロックをコメントで投稿する | F4 | 「開始条件」、手順 3 の配布の Pull Request の本文、「出力物」（置き場所の文を足す）、「蓄積した課題を手入れする」 |
...
| # | 条件 | 満たさないとき |
| --- | --- | --- |
| 1 | **本番への配布まで済んでいる。** `release` の出力物の `段階:` の値が `本番` で始まる（検証の段階が無く、最初の配布が本番だった場合を含む）。同じ実行なら出力物を、別の実行なら下の「配布の記録」を読む | まとまりの課題すべてを OPEN のまま残し、報告に「本番への配布の後に閉じる」と載せる。`段階:` の行が無いときは `開いたまま（配布の記録が読めない）` にする（推測で閉じない） |
| 2 | **リリース後テストを通る経路では、その課題の受け入れ条件がすべて `合格`**（`release-verification` の判定） | `不合格`・`保留` の条件を持つ課題を OPEN のまま残し、判定と理由を報告に載せる。読み方と読めないときの扱いは下の「配布の記録」の読み方の表にある |

**配布の記録は Pull Request に置く。** `release` の出力物は会話に出るだけで、別の実行からは読めない。
**書く側の形を 1 つに決め、読む側はその形だけを読む。** 別の配布の記録とは版で対応づける。

| 記録 | 書く形 | 置く側 | 置き場所 |
| --- | --- | --- | --- |
| 段階 | `段階: <検証 / 本番>（本番なら承認を得た時点、止めたなら理由）` | `release` | 配布の Pull Request の本文（手順 3）。配布の形が Pull Request を作らないときは、まとまりの最後にマージした Pull Request へ出力物のブロックをコメントで投稿する |
| 版 | `版: <前の版> → <新しい版>（<根拠>）` | `release` | 同上 |
...
| 読むもの | 読み方 | 読めないとき |
| --- | --- | --- |
| 条件 1 | 最後の `段階:` の値が `本番` で始まる（`case "$stage" in 本番*)`。値は `本番（…）` の形のため等値では比べない） | 行が無い: `開いたまま（配布の記録が読めない）` |
| 本番の版 | `段階:` が `本番` で始まる最後の `release` の記録で、その後の `版:` の `→` の後、`（` の前 | 同上 |
...
case "$stage" in 本番*) ;; *) echo "開いたまま（本番への配布の前、または配布の記録が読めない）" ;; esac
printf '%s\n' "$record" | awk '                 # 本番の版に対応するブロックを 1 つだけ出す
  /^段階: /              { prod = /^段階: 本番/ }
  ...
  END { for (i = n; i >= 1; i--) if (ver != "" && bver[i] == ver) { printf "%s", blk[i]; exit } }'
```
...
| `開いたまま（理由）` | 閉じる条件に当たらない（本番への配布の前 / リリース後テストが `不合格`・`保留` / 条件と課題の対応が読めない / 本番の版のリリース後テストの記録が無い / 配布の記録が読めない） | 番号と理由（条件の判定と、いつ閉じるか） |
