...
| `plugins/ndf/skills/progress-tracking/SKILL.md` | まとまりの課題の集め方、単位ごとの記録の担い手、**まとまりを閉じる唯一の手順**を持つ | F4 F5 | 冒頭の「この Skill は順序を持たない」の段落（例外として終わりの記録を足す）、「呼び方」の `status` の段落（終わりの工程で `Done` を書くのは「まとまりを閉じる」だけ、と書き換える）、新設「工程の単位と記録する課題」、新設「まとまりを閉じる」 |
| `plugins/ndf/skills/release/SKILL.md` | 後にリリース後テストも振り返りも続かない変更で、課題の手入れの前にまとまりを閉じる（「蓄積した課題を手入れする」の条件が「振り返りを通らない変更」からこれに変わる）。まとまりの範囲の確認を開始条件に持つ。**配布の記録を置く:** 配布の Pull Request の本文に、見出し `## 配布の記録` と直後の `段階:`・`版:`・`まとまり:` の行（出力物の行の形のまま）のブロックを置き、Pull Request を作らない形ではまとまりの最後にマージした Pull Request へ同じブロックをコメントで投稿する。**配布を飛ばしたときも `段階: 配布なし（<理由>）` と `まとまり:` の行のブロックを記録する**（`版:` の行は書かない。置き場所は Pull Request を作らない形と同じ）。`status Done` を書かない | F4 | 「開始条件」、手順 3 の配布の Pull Request の本文、「出力物」（置き場所の文を足す）と直後の飛ばしたときの段落（記録の形を足す）、「蓄積した課題を手入れする」 |
| `plugins/ndf/skills/release-verification/SKILL.md` | 後に振り返りが続かない変更で、まとまりを閉じてから `issue-upkeep` を呼ぶ。出力物の「## リリース後テスト」のブロックを、配布の記録を置いた Pull Request へコメントで投稿する。**出力物の表へ先頭の列 `課題` を足す**。`status Done` を書かない | F4 | 手順 4「合否を判定する」の「合否によらず、記録を残して `retrospective` へ進む」を、振り返りを通る変更に限る条件へ直す。「出力物」の置き場所の文（配布の記録の Pull Request へ変える）と表（先頭に `課題` の列を足す）。末尾の進行の記録の近くに節を 1 つ新設 |
| `plugins/ndf/skills/retrospective/SKILL.md` | 振り返りの後、課題の手入れの前にまとまりを閉じる | F4 | 進行の記録の 1 行から「あわせて盤面の `status` を `Done` にする」を外し、まとまりを閉じるへ一本化する（入口で Done を書くと `Auto-close issue` が先に閉じる） |
...
| リモートブランチ | マージ済み Pull Request の head で、同じリポジトリにあり、先端が `headRefOid` と一致する | — | 起点・本番のチャネル・fork の head、先端が `headRefOid` と違う、対応する Pull Request が見つからない（マージの後に積まれたコミットは Restore branch で戻らず、消した後に戻す手段が無い。決定 4） |
...
#    別の実行なら、下の「配布の記録」のコード例の bundle_prs（最後の「## 配布の記録」ブロックの `まとまり:` の行）を使う
...
| A3 A4 A6 A7 | 「`merged` の止まる条件」の表と本文の突き合わせ（レビュー）。A6 は同意を求める一覧（「止まったときの一覧」）にリモートブランチが載らないことも確かめる。A7 は要求文書の実測 2 行目の再現を実装の検証に含める |
...
| C11（本番の版のブロック） | 「配布の記録」のコード例。本文に `## 配布の記録` と `段階: 本番（…）` と `版: 10.14.0 → 10.15.0（…）`、コメントに対象の版 `10.15.0-dev.1` と `10.15.0` の 2 つのブロックを持つ入力で `10.15.0` のブロックだけを出し、ブロックの順を逆にしても同じ、`10.15.0` のブロックが無ければ出力が空、`段階: 検証（…）` なら条件 1 を満たさず版を読まない（設計の時点で mawk 1.3.4 で実測済み） |
| C11（手動確認） | リリース後テスト。このマイルストーンが検証への配布だけの時点で終わりの工程を通したとき、課題が OPEN のままで、報告に `開いたまま` と理由が載る |
| C12 | 「配布の記録」のコード例。コメントに `## 配布の記録`・`段階: 配布なし（配布物に差分が無い）`・`まとまり: PR #717 / #718` だけを持つ入力で条件 1 を満たし（`開いたまま` を出さない）、ブロックの出力が空。その記録の前に 1 つ・後に 2 つのブロックを持つ入力で後の 2 つ目だけを出し、前にしか無ければ空。古い `段階: 本番` の記録とその版のブロックの後に新しい `配布なし` の記録を置くと出力が空（C11 の 4 例も同じコード例で変わらないことと合わせ、設計の時点で mawk 1.3.4 で実測済み） |
| C13 | `grep -n 'status "Done"' plugins/ndf/skills/*/SKILL.md` の結果が `progress-tracking` だけになり、`retrospective` の進行の記録の行に `Done` が無い |
...
| D2（まとまりの行の取り出し） | 「配布の記録」のコード例の `bundle_prs`。本文に古い `## 配布の記録`（`まとまり: PR #700 / #701`）、その後に別の `## ` 見出しと裸の `まとまり: PR #999`、新しい `## 配布の記録`（`まとまり: PR #717 / #718 / #720`）を持つ入力で 717 718 720 だけを出し、ブロックが無ければ出力が空（パイプの終了コードはどちらも 0。設計の時点で mawk 1.3.4 で実測済み。`gh pr view 719` / `739` の本文とコメントの取得は終了コード 0） |
