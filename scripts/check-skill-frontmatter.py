...
使い方:
    ...
    python3 scripts/check-skill-frontmatter.py --skills-dir plugins/ndf/skills
...
#
# **値は実質の制約から逆算する。** 初期一覧の予算のうち最も厳しいのは Codex で、そこへ
# 達する手前で気づける位置に目安を置く。2026-09-01 の実測は次のとおりだった。
#
#   frontmatter 合計          12,570 文字
#   Codex の初期一覧           6,961 文字 / 予算 15,177 文字（46%）
#   Claude Code の初期一覧     6,393 文字 / 予算 27,900 文字（23%）
#   Kiro の初期一覧            7,143 文字 / 予算 27,900 文字（26%）
#
# frontmatter と Codex の一覧の比は 12,570 / 6,961 = 1.81 で、Codex が予算へ達するときの
# frontmatter は約 27,400 文字にあたる。その 75%（予算に 4 分の 1 の余裕を残す位置）を
# 目安とする。27,400 × 0.75 ≒ 20,000。
#
# **起動直後のコンテキストにも余裕がある。** 同じ日に `claude -p "/context"` で測ると、
# 1,000,000 トークンに対して全体 33.9k（3%）、うち Skill の一覧は 7.4k（0.7%。他の
# プラグインと組み込みを含む全量）だった。frontmatter がこの目安まで増えても、Claude Code
# の一覧は 2.3k から 3.6k トークンへ増えるだけで、コンテキスト全体では 0.1% の増加にとどまる。
#
# 引き上げは**実測を取り直してから**行う（`--calibrate` と `claude -p "/context"`）。
# 据え置いたまま Skill を足すと警告が出続け、目安として働かなくなる。
FRONTMATTER_TOTAL_MAX = 20_000
