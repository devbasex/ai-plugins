"""定義ファイルの `description` の版数チェックが、接尾辞の付いた版を読めることを固定する。

開発版の版数には接尾辞（`-dev.<連番>` / `-rc.<連番>`）が付く（`docs/versioning-and-distribution.md`
の「版の付け方と開発版の配布」）。数字 3 つの直後に閉じ括弧を求める書式では
`(v9.7.0-dev.1)` に一致せず、開発版を出すたびにこのチェックで止まっていた（#254）。
