...
**配布のチャネルは 2 つある。** `main` が開発版、`stable` ブランチが正式版です。ここに書く
手順は版数を書き換えるところまでで、**利用者へ届くのは `stable` を進めた時点**です。進める
判断と承認は `/ndf:release` が扱います。
...
**通常の取得経路で確かめたい場合は、開発版のチャネル（`main`）へ出す。** 版数へ
`-dev.<連番>` を付けて公開し、検証に参加する利用者が利用者と同じ手順で取得する。常用する
利用者は `stable` を見ているため、この公開では届きません（AGENTS.md の「版の付け方と
開発版の配布」）。
