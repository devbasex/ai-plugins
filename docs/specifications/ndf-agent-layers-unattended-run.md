...
**フェーズは 設計 / 実装 / 検査 / 取り込み / 仕上げ / 配布 の 6 つである。** `context-window.md` のカットポイント
4 点で工程表を切ると supervisor の 5 つになり、これに supervise.py の配布の計画が通す配布を足す。関門 2 つはそれぞれ設計と取り込みの終わりに当たる。
...
| フェーズ | supervisor 1 つが通す工程の束。`設計` / `実装` / `検査` / `取り込み` / `仕上げ`、計画が通す `配布` |
...
（どちらも起きなかったとき。承認ではなく、関門に数えない）。どれで起きても conductor は同じ
...
| 層 | 規則の要点 |
| --- | --- |
| supervisor | 関門に達したら承認を求めずに `結果: 関門` で返す / 関門以外の確認は提示して進める / 工程に入った時点で `progress-tracking` を呼び、記録は 1 回の Bash 実行に 1 件 / 待ちで応答を終えない / 外部へ書く前に既に書いたものを確かめる / 範囲外は `out-of-scope` / 委譲してよい作業は worker へ出し、委譲しない 5 つは自分で行う / worker の報告をそのまま渡さない / 末尾に `## フェーズの報告` / モードを上げるべきなら `結果: 止まった`、置き直した到達点に達したら `結果: 完了`・`次のフェーズ: 無し` / 収束ループの Skill を hook に止められたら、やり直さずに `結果: スイッチポイント`・`次の工程: <その工程>` で返す |
...
| ある | `スイッチポイント` | 同じフェーズを `次の工程` から通す supervisor として起動し直す（`subagent_type` は上の選び方）。関門を問わない。同じ `次の工程` の `スイッチポイント` が続けて 2 回返ったら `止まった` と同じに扱う |
...
ため関門は保たれるが、掛からないと supervisor の進行の記録が通過記録に積まれない。その場合は
