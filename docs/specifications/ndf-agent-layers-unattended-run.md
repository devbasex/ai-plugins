...
**担当が利用上限で落ちたことをオーケストレーターが検知できず、再開を手で指示していた（#657）。** 実測では、
...
確実に働く。**本番の系へ届く操作は、承認を受け取った次の supervisor が行う。** リリースは `release` の
...
（引継ぎの 1 行の先頭に `/goal ` を付けるか）だけを分ける。
...
| 取り込み | 確定仕様化 / 実装 Pull Request のマージ / 後片付け / リリース（検証まで） | ミッション | 本番の系へ届く操作（要るときだけ） |
...
中身が少なくても作る。`operation` では設計のフェーズが本番の系へ届く実行の前でゲート 2 を返し、
...
| 承認ゲートの数・承認の形が変わらない | `test_workflow_guard.py` が無改変で通る（`test_approval_gates.py` は #885 で削除） |
