...
**記録から**見分け（通知は使わない）、リセット時刻を過ぎてから直下だけを再開する手順を置いた。この文書は、
...
（`AskUserQuestion` を持つ）のは conductor だけで、supervisor は承認ゲートに達したら承認を求めずに
`結果: 関門` で返す。worker は葉であり、別のサブエージェントを起動しない。

**フェーズは 設計 / 実装 / 検査 / 取り込み / 仕上げ / リリース の 6 つである。** `context-window.md` のカットポイント
4 点で工程表を切ると supervisor の 5 つになり、これに supervise.py のリリースプランが通すリリースを足す。承認ゲート 2 つはそれぞれ設計と取り込みの終わりに当たる。

**報告は 2 層で、worker の報告を conductor へ転送しない。** worker は 5 項目の作業の報告を
...
大きさを測ることを求めた。当初の 2 層（オーケストレーターと担当）は 3 層へ見直され、責務・報告の経路・測定・
中断と再開のすべてを設計し直した。

**担当が利用上限で落ちたことをオーケストレーターが検知できないと、再開を手で指示することになる。** 実測では、
下の層が 429 で落ちると上の層には `failed` の通知が届き、記録の合成応答が `apiErrorStatus` と
...
**supervisor の単位は、カットポイントで区切ったフェーズにする。** 工程表の行は 18 あり、
...
supervisor は委譲しない 5 つ（モード判定・承認ゲートの判断・収束の判定・設計の決定と理由の記録・
受け入れ条件の書き換え）を自分で行う。

**報告を 2 層に分け、worker の報告を conductor へ転送しない。** worker を 5 つ使っても conductor
...
確実に働く。**本番系へ届く操作は、承認を受け取った次の supervisor が行う。** リリースは `release` の
手順そのもので 1 つのコマンドに収まらず、conductor が起動すると手順が conductor の context window
に載る。承認は起動の指示の `承認` の項目で渡す。

**承認ゲート以外の Skill の確認は、提示して進める。** supervisor も worker も `AskUserQuestion` を持たない
...
**レートリミット中断は記録で見分け、通知は使わない。** 通知の本文は人が読む文言で、書式を約束していない。
...
- 承認ゲートは 2 つで増えない。承認を取れるのは conductor だけである
...
| 検査 | リファクタリング / コードレビュー / 完了判定 / Pull Request（Draft を外す） | Pull Request | なし |
| 取り込み | 確定仕様化 / 実装 Pull Request のマージ / 後片付け / リリース（検証まで） | ミッション | 本番系へ届く操作（要るときだけ） |
| 仕上げ | リリース（本番）/ リリース後テスト / 振り返り | ミッション | なし |

フェーズの名前はモードで変えない。モードは各フェーズがどの工程を含むかと承認ゲートを返すかだけを変え、
作らないフェーズ（`light` の設計と仕上げなど）の工程は次のフェーズの先頭へ入る。承認ゲートを返すフェーズは、
中身が少なくても作る。`operation` では設計のフェーズが本番系へ届く実行の前でゲート 2 を返し、
...
conductor → supervisor の `prompt` は 8 項目（フェーズ・課題・モード・作業場所・前のフェーズレポート・
承認・到達点・承認資料の置き場所）と守る規則 12 個を持つ。supervisor → worker は 4 項目（作業・
入力・返す形・置き場所）と守る規則 4 個を持つ。`model` は省く（上の層と同じ）。

conductor → supervisor の `subagent_type` は、その supervisor が最初に通す工程で選ぶ。収束ループ
（リファクタリング・コードレビュー・ドキュメントレビュー）から始めるなら `ndf:supervisor-waits`（キャッシュの
...
| 層 | 規則の要点 |
| --- | --- |
| supervisor | 承認ゲートに達したら承認を求めずに `結果: 関門` で返す / 承認ゲート以外の確認は提示して進める / 工程に入った時点で `progress-tracking` を呼び、記録は 1 回の Bash 実行に 1 件 / 待ちで応答を終えない / 外部へ書く前に既に書いたものを確かめる / 範囲外は `out-of-scope` / 委譲してよい作業は worker へ出し、委譲しない 5 つは自分で行う / worker の報告をそのまま渡さない / 末尾に `## フェーズの報告` / モードを上げるべきなら `結果: 止まった`、置き直した到達点に達したら `結果: 完了`・`次のフェーズ: 無し` / 収束ループの Skill を hook に止められたら、やり直さずに `結果: スイッチポイント`・`次の工程: <その工程>` で返す |
...
| ある | `スイッチポイント` | 同じフェーズを `次の工程` から通す supervisor として起動し直す（`subagent_type` は上の選び方）。承認ゲートを問わない。同じ `次の工程` の `スイッチポイント` が続けて 2 回返ったら `止まった` と同じに扱う |

**スイッチポイントは hook が決める。** `token-guard.sh` は、寿命 5 分の supervisor（入力の `agent_type` が
...
| 観点 | 確かめ方 |
| --- | --- |
| 3 層の責務の表、フェーズ 5 つと承認ゲートの列、報告の 11 項目 / 5 項目、続けさせる回数 3、モデルの基準 2 つ、委譲しない 5 つ、並行の単位が supervisor、止まるときにフェーズの一覧を出す 3 つの時点 | 文書を読んで確かめる |
| 中断の契機 4 つ、落ちた層ごとの 3 通り、解除を待つ手段 3 つ、点検の手順、`--max-sleep` を付けない規約 | 同上 |
| 「メイン」の定義が supervisor を指す | 同上 |
| `context-window.md` にモデルに依る目安とリポジトリに依る固定費の区別があり、規約の文書に固定費の実測値が無い | 同上 |
| 3 つの規約の文書に「窓」「親」が無い | 同上 |
| `/ndf:development-workflow` を呼べば 3 層へ出す、承認ゲート以外は提示して進める | 同上 |
| 承認ゲートの数・承認の形が変わらない | `test_workflow_guard.py` が無改変で通る |
| 実機での無人の通過（人の入力の数、conductor の報告のフェーズの一覧と記録の `SendMessage` の数の一致） | リリース後テスト（`light` の課題 1 件を `/goal` で通し、次のミッションで `standard` を含む複数の課題を承認ゲート 2 つと振り返りまで通す） |
...
**進捗記録は 1 回の Bash 実行に 1 件である。** #487 が直れば起動の指示の 1 行を消す。
