...
| 自走の入口（`/goal` の有無によらず 3 層へ出す。#996） | `plugins/ndf/skills/development-workflow/SKILL.md` の「自走で工程を通す」 |
...
**conductor は人間と話し、supervisor は持ち場を通し、worker は 1 つの作業を行う。** 人間へ問える
（`AskUserQuestion` を持つ）のは conductor だけで、supervisor は関門に達したら承認を求めずに
`結果: 関門` で返す。worker は葉であり、別のサブエージェントを起動しない。

**持ち場は 設計 / 実装 / 検査 / 取り込み / 仕上げ の 5 つである。** `context-window.md` の切れ目
4 点で工程表を切ると 5 つになり、関門 2 つはそれぞれ設計と取り込みの終わりに当たる。

**報告は 2 段で、worker の報告を conductor へ転送しない。** worker は 5 項目の作業の報告を
supervisor へ返し、supervisor はそれを畳んで 10 項目の持ち場の報告を conductor へ返す。
conductor が 1 つの持ち場について読むのは 1 件である。
...
場面）。設計の持ち場でモードを上げるべきだと分かったら、supervisor は `結果: 止まった` で返し、
conductor が判定をやり直す。
...
diff を載せない」の対象は `state.py` の骨組みを回している層で、3 層では設計と検査の持ち場の
...
- 3 層へ出すかは `/goal` の有無で決めない。対話で呼んでも 3 層へ出す（#996 で改めた。以前は `/goal` の引数のときだけ）
...
| 取り込み | 確定仕様化 / 実装 Pull Request のマージ / 後片付け / 配布（検証まで） | まとまり | 本番の系へ届く操作（要るときだけ） |
| 仕上げ | 配布（本番）/ リリース後テスト / 振り返り | まとまり | なし |

持ち場の名前はモードで変えない。モードは各持ち場がどの工程を含むかと関門を返すかだけを変え、
作らない持ち場（`light` の設計と仕上げなど）の工程は次の持ち場の先頭へ入る。関門を返す持ち場は、
中身が少なくても作る。`operation` では設計の持ち場が本番の系へ届く実行の前で関門 2 を返し、
実装の持ち場が承認を受け取って実行する。
...
| supervisor | `spawnDepth` が 1 で、先頭語が持ち場の語彙にある | `<持ち場>: <課題番号を空白区切り>`（例 `設計: #550 #657`） |
...
conductor → supervisor の `prompt` は 8 項目（持ち場・課題・モード・作業場所・前の持ち場の報告・
承認・到達点・提示物の置き場所）と守る規則 10 個を持つ。supervisor → worker は 4 項目（作業・
入力・返す形・置き場所）と守る規則 4 個を持つ。`subagent_type` と `model` は省く（上の層と同じ）。

規則の要点は次のとおりで、文言は `agent-layers.md` が持つ。

| 層 | 規則の要点 |
| --- | --- |
| supervisor | 関門に達したら承認を求めずに `結果: 関門` で返す / 関門以外の確認は提示して進める / 工程に入った時点で `progress-tracking` を呼び、記録は 1 回の Bash 実行に 1 件 / 待ちで応答を終えない / 外部へ書く前に既に書いたものを確かめる / 範囲外は `out-of-scope` / 委譲してよい作業は worker へ出し、委譲しない 5 つは自分で行う / worker の報告をそのまま渡さない / 末尾に `## 持ち場の報告` / モードを上げるべきなら `結果: 止まった`、置き直した到達点に達したら `結果: 完了`・`次の持ち場: 無し` |
...
| `## 持ち場の報告`（supervisor → conductor） | 持ち場 / 課題 / 結果（`完了` / `関門` / `止まった`）/ 関門（`設計 Pull Request のマージ` / `本番の系へ届く操作`）/ 次の持ち場 / Pull Request / 最後に記録した工程 / 使った worker / 提示物 / 理由 |
...
| ある | `完了` | `次の持ち場` を起動する。`無し` なら到達の報告 |
| ある | `関門` | `提示物` を読み、`AskUserQuestion` で承認を求める。`設計 Pull Request のマージ` なら conductor がマージしてから、`本番の系へ届く操作` ならマージせずに `次の持ち場` を起動する。モードや持ち場の名前からは分岐しない |
| ある | `止まった` | `理由` を添えて利用者へ報告し、終える |

supervisor が worker の報告を見るときも同じ規則を使う。`結果: 判断が要る` は supervisor が判断する。

**conductor は、止まるときに `## 持ち場の一覧` を 1 回出す。** 出す時点は `AskUserQuestion` を出す前・
失敗を報告する前・背景の待ちを起動して応答を終える前の 3 つである。列は 持ち場 / 課題 / 結果 /
使った worker / 報告なしで続けさせた回数 / 上限の中断から再開した回数 で、続けさせた回数は記録から
見分けられないため conductor が `SendMessage` を送るたびに数えて持つ。同じ持ち場を工程の頭から
起動し直したときは別の行にする。
...
同じ持ち場の名前で起動し直し、起動の指示に旧 supervisor の `agent_id` を渡す。まだ過ぎていない
