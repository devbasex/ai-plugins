...
新書式の 3 Skill を一時的なローカルマーケットプレイス経由で導入し、セッションの Skill 一覧に
載ることと、自然文の依頼が該当 Skill に解決することを確認した。

- 一覧の `description` を引用させたところ、新書式の文面がそのまま返った（初期一覧に載っている）
- 「マージ済みのブランチを整理してください。」に対して `merged` を選択した

ただしこの測定では導入済みの旧書式（v6.0.0）も同時に一覧へ載っていた。**旧書式を外した単独条件での
測定は、`codex exec` が所定時間内に応答せず未完**である。新旧のどちらが選ばれたかの分離はできて
いないため、新書式が一覧に載り解決先になりうることまでが確認範囲である。

### Kiro（kiro-cli 2.16.1）

**測定不能。** この環境の `kiro-cli` が未認証で、`chat --no-interactive` が認証フローへ入る。
`description` 一致による自動発動自体は v5.0.0 の検証で確認済み（「実機検証の記録」）であり、
書式変更は同じ 1 個の文字列を短くするだけで判定経路を変えない。
...
Codex と Kiro で単独条件の実測ができていない。全 Skill へ広げたあと、いずれかのランタイムで
起動しない Skill が見つかった場合は、その Skill の**用途文**を見直す（旧書式へ戻す道は
残していない。`Triggers:` は廃止し、残っていると検査が失敗する）。実測では、旧書式で宣言
していたトリガ語が届かず新書式で届いた例があり、戻すことが解決になるとは限らない。
