...
オーケストレーターが、ミッションを渡されたら最初の supervisor を起動する前に**実行計画**を書き、工程の終わりごとに
読み直して着手できる行を起動する形にした。起動してよい本数はホストのメモリから測り、触る箇所の
変更重複は 3 つの区分で判定して並行するか順にするかを決める。マイルストーンへ課題を入れる時点では
**課題グループ**（触る場所の見込みと依存）を説明へ書き、実行計画がそれをバンドルの初期値として読む。この文書は、
決めたことの理由と、実行計画・課題グループ・測定のコマンドの契約を残す。

**手順は各 Skill の `references/` が正である。** 実行計画の雛形・見直しの手順の文言・必須ルールの文言を
ここへ書き写さない。
...
**競合は後からマージする側が解き、解いた後の head でレビューを収束させる。** 必須ルール 5 は「同じ
...
| 用語 | 意味 |
| --- | --- |
| オーケストレーター | 課題をバンドルに分け、supervisor を起動し、承認を仲介する実行主体。3 層では conductor |
| 本数 | 同時に動いている supervisor の数。worktree 1 つで 1 本の Pull Request を進める。CLI・テスト・コンテナのどれかを起動しうる supervisor を数え、その中の worker は 1 本に含めて別に数えない |
| 実行計画 | ミッション課題を、バンドル・工程ごとの依存・触る箇所・着手できる時点で並べた表。オーケストレーターが持つ |
| バンドル | 1 本の設計 Pull Request で決める課題の集合 |
...
ホストで 5〜6 本を並行したとき、OOM Killer が claude 本体を 2 回落として全 supervisor が止まり、3 本では
落ちなかった（#621）。落ちた後に初めてメモリを見たため、値の読み方（`MemAvailable` と cgroup の
違い、スワップ）を毎回人が判断していた。2 回目の落ち方では後片付けの漏れで残ったコンテナが 238 個
あった。**変更重複を見分けたのは設計の後だった。** マイルストーン 04 では #643 → #646 を逐次にしたが、
...
入ったら次の設計に着手する」を必須ルールに足す形は採らない。工程を supervisor へ出す運転では supervisor の
中の待ちはオーケストレーターから見えず、依存と本数だけで着手を決めれば待ちを知らなくても次の設計が起動される。

**競合は後からマージする側が解く。** 先にマージした側は解く時点で既に閉じている。`cross-review` は
Pull Request の head を読むため、新しい手順は要らない。並行する課題グループごとにレビューの観点で分担を決める
形は、分担を決めても同じ節を書き換えれば競合が起きるため採らない。

**変更重複は節の単位で 3 つに分け、書き換えの変更重複だけ実装を順にする。** 節は設計の時点で分かる最も
細かい単位である。行の単位は実装の後にしか分からず、ファイルの単位では別の関数を触る Pull Request の対まで順になる。
「足すだけ」を並行へ入れるのは、git の競合が起きても解き方が両方を残すことに決まっているためである。
設計文書はバンドルごとに別のファイルへ書くため、設計の工程どうしはどの区分でも並行する。
...
**コンテナを起動するチェックを持つ supervisor は同時に 1 本にする。** コンテナは supervisor の cgroup の外で動くため、
supervisor の使用量として測れず、1 本の見込みの外側で空きを食う。後片付けの漏れは `trap EXIT` で直してあり
（#597）、再び起きても空きメモリの低下として測定に現れる。

**マイルストーンの型を宣言せず、再編成せず、説明に課題グループを書く。** 型を宣言しても組み替えは起きず、
再編成すると連番が表す着手の順序と既存の説明の意味が動く。課題グループの触る場所は手順 2A が既に控える修正
レイヤーから写し、依存は「依存する課題の番号」から写す。新しく調べる項目を増やさない。**課題グループは見込み
である。** 確定の値は実行計画が持ち、マイルストーンの説明へ書き戻さない。**課題グループの表が無い既存の
...
- 課題グループはマイルストーンの中の単位で、マイルストーンの名前・連番・着手の順序の規則を変えない
- 必須ルール 1〜3 と承認ゲートの節、`development-workflow/SKILL.md` は変わっていない
...
**見直しの契機は 5 つである。** supervisor の報告を受け取った / Pull Request がマージされた / supervisor が止まった /
...
の表を置く。課題グループは 1 から始まる連番で、課題は同じ修正レイヤーを持つ課題の番号、触る場所の見込みは修正
レイヤーの名前（Skill の名前・スクリプトのパス・文書のパス）、依存は `なし` または課題グループの番号である。

| 時点 | 行うこと |
| --- | --- |
| 課題をマイルストーンへ入れる | 修正レイヤーが既存の課題グループと同じならその課題グループへ番号を足す。違えば課題グループを 1 つ足す。表が無ければ見出しと表を作る |
| 課題を別のマイルストーンへ移す | 元の表から番号を消す。0 件になった課題グループは行を消し、番号は詰めない |
| 課題が閉じた | 何もしない。実行計画が状態を持つ |

実行計画は表の課題グループをバンドルの初期値として読む。表に無い課題は本文を読んでバンドルを決め、どの課題グループにも足さない。
...
| 必須ルール 4・5・6 の文言、必須ルール 6 の理由に「メモリ」と「21GiB」、コンテナを起動する supervisor は 1 本、変更重複の目安の 3 区分と列 | 文書を読んで確かめる（照合していたテストは #885 で削除） |
| `execution-plan.md` に置き場所・コミットしないこと・行の列・確度と状態の値・契機 5 つ・閉じる手順・課題グループを読むこと・`capacity` を起動の前に実行することがあり、初期値の数値（2048・25%）が無い | 文書を読んで確かめる（照合していたテストは #885 で削除） |
...
| 閉じた実行計画のコメントと振り返りが読める | リリース後テスト（次に複数のバンドルを持つミッションを進めたとき） |
...
- [ndf-workflow-unit-and-gates.md](ndf-workflow-unit-and-gates.md) — 並行開発の必須ルール
