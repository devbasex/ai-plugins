...
**GitHub と git へ書くのは、レビューを回す側だけである。** レビューの担当は指摘ファイルと
...
| 担当に待ち行列へ積ませる形を採らない | 待ち行列は回す側の worktree にある。置き場所を担当へ開くと、巻き直しで捨てる範囲が変わる |
...
| 関数 | 入力 | 出力 |
| --- | --- | --- |
| `review_posts` | 指摘ファイルと結果ファイルのパス、リポジトリ、PR、ラウンド、席、頭のコミット、自分の PR か、開始時刻 | 待ち行列へ積む項目の列 |
