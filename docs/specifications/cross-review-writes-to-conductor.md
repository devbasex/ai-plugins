# cross-review: PR に出ている指摘が記録に残らず、同じ論点が 2 つのスレッドに分かれる → GitHub と git へ書くのはオーケストレーターだけになり、途中で止まっても同じものを二度書かない

## 目的

**GitHub と git へ書くのは、オーケストレーターだけである。** レビューの担当は指摘ファイルと
結果ファイルを書いて終わる。修正担当はコミットまでで止まる。投稿・返信・スレッドの決着・
...
担当・スロット・起動結果の語は
[起動結果](cross-review-launch-outcome.md#用語)と
[利用可能な参加者だけで始める収束ループ](cross-review-participants-and-seats.md#用語)と同じ意味で使う。

| 業務用語 | 識別子 | 何を指すか |
| --- | --- | --- |
| オーケストレーター | `state.py` と、それを呼ぶ `SKILL.md` のスケルトン | 担当を起動し、結果を取り込み、判定する側 |
| 担当 | `launch-reviewer.sh` が起動する CLI | 1 ラウンドで 1 スロットぶんのレビューを行う |
| 修正担当 | `/ndf:fix` を実行するサブエージェント | 指摘を直してコミットする |
| 結果ファイル | `<席>-review-pr<番号>-result.json` | 担当の判定の要約 |
| 指摘ファイル | `<席>-review-pr<番号>-round<R>-payload.json` | 担当が出した指摘の全件と総評 |
| 修正の結果ファイル | `fix-pr<番号>-result.json` / `sweep-pr<番号>-result.json` | 修正担当が書く戻り値 |
| 投稿キュー | `plugins/ndf/scripts/lib/post_queue.py` | 送る前に積み、上限で送れなければ残す仕組み |
| 投稿を組み立てる層 | `plugins/ndf/scripts/lib/result_posts.py` | 結果ファイルから投稿を組み立て、送るライブラリ |
| 指摘の取り込み | `state.py read-result` | 指摘ファイルを読んでレビューを投稿し、記録する |
| 修正の取り込み | `state.py merge-fix` | 修正を送り、返信・決着・まとめを投稿し、記録する |
| 単独の修正の口 | `result_posts.py fix` | 修正を単独で行うときと最終スイープで呼ぶ部分命令 |
| 重複投稿 | — | 送ろうとした投稿と同じものとして、すでに PR にある投稿 |
...
| 扱う | 扱わない |
| --- | --- |
| レビューの投稿（総評とインライン） | PR の巻き直しの締め・作り直し・再開（もとからオーケストレーターが行い、投稿キューに積まない） |
...
ファイルを書いていた。この 2 つの間で担当が止まると、投稿は残り、記録は残らない。オーケストレーターは
...
2 つのスレッドを見て、修正担当は両方へ返信した。

**投稿キューは担当の投稿を通していなかった。** 上限で拒まれた投稿を残して後から流す仕組みは
オーケストレーターにあった。担当が自分で送るため、上限に当たった投稿はその場で失われた。

**送信の報告を確かめる手段が無かった。** 修正担当はブランチ名だけを指定して送信していた。
worktree が切り離された頭で作られていると、この指定は現在の頭を送らないまま終了コード 0 で
終わる。取り込む側はその報告をそのまま記録した。

**根本原因は、書き込みの担い手が記録の担い手と別であることにある（#730）。**
書き込みをオーケストレーターへ集めると、この 3 つが同じ場所で解ける。

## 決定と理由

| 決定 | 理由 |
| --- | --- |
| GitHub と git への書き込みをオーケストレーターだけが行う | 書き込みと記録が同じ手順で続けば、片方だけが残る状態を作れない |
| 担当の投稿の後にオーケストレーターが実物を照合して記録を補う形を採らない | 照合は問い合わせを増やす。問い合わせが上限で失敗すると同じ食い違いが残る |
| 担当に投稿キューへ積ませる形を採らない | 投稿キューはオーケストレーターの worktree にある。置き場所を担当へ開くと、巻き直しで捨てる範囲が変わる |
...
| 送信に成功した項目は記録より先に投稿キューから消える形を変えない | 取り込みをやり直せば同じ状態になる。流し方を変えると、巻き直しのコメントの契約まで変わる |
...
| 開始時刻かレビューの時刻を読めないときは、番号とスロット名だけで照合する | 二重に送る側へ倒さない |
...
| 修正の書き込みをライブラリの 1 か所にまとめる | cross-review から呼ぶ経路と単独で使う経路で、担い手と実装が分かれない |
| 単独の口は新しい入口のスクリプトにせず、ライブラリの部分命令にする | 手順に書く行が 1 本で済み、実装が 1 つになる |
...
| 条件 | 破れたときの扱い |
| --- | --- |
| 担当へ渡すプロンプトは、投稿の手順（投稿の呼び出し・判定の格下げ・インラインの組み立て）を持たない | プロンプトのチェックが落ちる |
| 担当が指摘ファイルを書かずに止まった実行では、PR のレビューは増えない | — |
| 同じラウンド・同じスロットのレビューは、この実行の中で 1 件しか PR に出ない | 重複投稿があれば送らず、重複投稿の参照を記録する |
| 取り込みの標準出力に、総評とインラインの本文は出ない | — |
| 記録に入るレビューの参照は、送信の応答か重複投稿から取る | — |
...
| 投稿を組み立てる層（`result_posts.py`） | 指摘ファイルと結果ファイルから投稿を組み立てる。退避・格下げ・送信・照合を行う |
| 投稿キュー（`post_queue.py`） | 送る前に重複投稿を照合する。拒まれ方が位置によるものかを見分けて返す |
| スケルトン（`SKILL.md`） | 起動し直しを繰り返しの先頭へ戻す。最終スイープの書き込みを単独の修正の口で行う |
...
| 関数 | 入力 | 出力 |
| --- | --- | --- |
| `review_posts` | 指摘ファイルと結果ファイルのパス、リポジトリ、PR、ラウンド、スロット名、頭のコミット、自分の PR か、開始時刻 | 投稿キューへ積む項目の列 |
| `post_review` | 投稿キューと `review_posts` の入力、投稿者 | 送った結果（参照・インライン数・総評へ移した数・残した数・指摘数・失敗・送った形・本来の判定） |
| `fix_posts` | 修正の結果ファイルのパス、リポジトリ、PR、ラウンド | 返信 → 決着 → まとめの順の項目の列 |
| `post_fix` | 投稿キューと `fix_posts` の入力、投稿者 | まとめの参照・返信数・決着数・残した数・失敗 |
| `push_fix` | worktree、送り先のブランチ、報告されたコミット | 成否・送ったか・載っているか・説明 |

### 担当が書くもの

担当は 2 つのファイルを書く。**どちらも一時の名前（末尾 `.tmp`）で書き終えてから、指摘ファイル →
結果ファイルの順に正式の名前へ改名する。** 結果ファイルが正式の名前で現れたことが、2 つとも
書き終えたマーカーになる。

| 状態 | 取り込みの扱い |
| --- | --- |
| 結果ファイルが正式の名前である | 指摘ファイルを読んで投稿する |
| 指摘ファイルだけが正式の名前で、結果ファイルが無い | 結果なし。投稿は 0 件 |
...
| 指摘ファイル | 総評（`summary`）、指摘の全件（`comments[]`） | 各指摘の送れた先（`posted_to` の `inline` / `body`） |
...
1. 投稿キューの残りを流す。残りは前の取り込みが上限で送れなかった投稿で、その担当の記録は
   積んだ時点で書いてある
2. 指摘ファイルと結果ファイルから、この担当のレビューを 1 件組み立てて積む
3. 流す
4. 位置を解決できずに拒まれたら、インラインをすべて総評へ移して積み直し、もう 1 度流す
5. 送れたら、指摘ファイルへ送れた先を書き戻す
6. 送信の応答を記録へ書き、指摘を取り込む

```mermaid
sequenceDiagram
    participant M as オーケストレーター
    participant A as 担当
    participant G as GitHub
    M->>A: 起動（投稿の手順を持たないプロンプト）
    A->>A: 指摘ファイルと結果ファイルを書いて改名する
    A-->>M: 終了
    M->>M: 指摘ファイルを読み、レビューを投稿キューへ積む
    M->>G: 重複投稿を照合し、無ければ送る
    G-->>M: レビューの参照
    M->>M: 参照と件数を記録し、指摘を取り込む
```

**組み立てるレビューの本文の先頭行は、ラウンド・スロット名・本来の判定を持つ。**
...
投稿キューは送る前に、同じものが PR に先にあるかを照合する。**重複投稿があれば送らず、重複投稿を
送信の応答の代わりに返す。** 呼び出し側は、送った場合と同じ経路で参照を記録する。

| 種別 | 照合の要点 |
| --- | --- |
| レビュー | 投稿者と、先頭行のスロット名までの前方一致。判定の語を含めない。ラウンドの開始時刻より後のレビューに限る |
...
**投稿者は、どのスロットでも同じ 1 つのアカウントである。** 作業環境の `gh` は 1 アカウントを
持ち、スロットごとに認証を切り替える口は配布物に無い。そのためスロットの区別は先頭行が担う。
...
修正担当はコミットまでを行い、修正の結果ファイルを書く。取り込みは次の順に進む。

1. 現在の頭を送り先のブランチへ送る（`git push origin HEAD:<ブランチ名>`）。認証で落ちた
   ときは、ライブラリの退避の値で 1 度だけやり直す
...
| 引数を省いたとき | 決め方 |
| --- | --- |
| リポジトリと送り先のブランチ | worktree の中で `gh` に問い合わせる |
| worktree | いまいるディレクトリ |
...
送信に成功した項目は、記録より先に投稿キューから消える。**立て直しは、投稿キューに項目が残る
ことに頼らない。**

| 止まった場所 | 投稿キューの項目 | 立て直し |
| --- | --- | --- |
| 担当が指摘ファイルか結果ファイルを書く前 | 無い | 結果なし。起動し直しても投稿は重ならない |
| 取り込みが送る前（上限で送れない） | 残る | 判定の終了コード 8 の枝が流し直し、参照を記録へ書き戻す |
| 取り込みが送った後・記録の前 | 残らない | 取り込みをもう一度呼ぶ。照合が重複投稿を見つけ、重複投稿の参照を記録する |
...
graph TD
    ...
    E --> F{重複投稿があるか}
    K --> F
    F -->|ある| G[送らずに<br/>重複投稿の参照を記録する]
    F -->|いない| H[送って記録する]
```

**送れた後に指摘ファイルへ送れた先を書けなければ、取り込みは止まる。** 指摘ファイルは原子的に書くため、
書きかけは残らない。やり直すと、照合が重複投稿を見つけて同じ参照を記録する。

### 起動し直しの経路

スケルトンのレビューの手順は「起動 → 監視 → 取り込み → 根拠の検証 → 反証 → 判定」の繰り返しで
ある。**判定が結果なし（終了コード 7）を返したとき、名前の出た担当だけを入れて繰り返しの
先頭へ戻る。** 起動し直しは同じラウンドで 1 度だけである。

| 判定の終了コード | 扱い |
| --- | --- |
| 8（投稿キューに残りがある） | 流してから判定し直す。**7 より先に見る** |
| 7（結果なし）で、まだ起動し直していない | 名前の出た担当で繰り返しの先頭へ戻る |
| それ以外 | 繰り返しを抜ける |

**経路が 1 本であるため、起動し直した担当の指摘も根拠の検証と反証を通る。**
起動し直せない起動結果（利用上限）の扱いは[起動結果](cross-review-launch-outcome.md)が持つ。
...
| 鍵 | 何が入るか |
| --- | --- |
| `review_url` | 送信の応答が返した参照。重複投稿が見つかったときは重複投稿の参照 |
| `queued` | 上限で送れず投稿キューに残っているとき真。流した直後に参照を書き戻して偽にする |
...
**投稿キューの置き場所は変えない。** 状態ファイルと同じ一時ディレクトリの `pending/` にあり、
巻き直しのときは worktree ごと捨てられる。単独の修正の口は、一時ディレクトリを環境変数
`CROSS_REVIEW_TMP_DIR` から、無ければ worktree の `.cross_review/` から決める。
...
| `INLINE` / `BODY` / `QUEUED` | 送れたインライン数・総評へ入れた数・投稿キューに残した数 |
...
送れない・指摘ファイルを書けない・修正が送り先に載らないときは、終了コード 1 で止まる。上限で送れなかったときは
止まらず、投稿キューに残して判定の 8 の枝へ渡す。

## テスト観点

| 観点 | 確かめ方 |
| --- | --- |
| 担当のプロンプトに投稿の手順が無く、2 つのファイルを一時の名前で書いて指摘ファイル → 結果ファイルの順に改名させること | `plugins/ndf/skills/cross-review/tests/test_launch_reviewer_prompt_context.py` |
| 取り込みがレビューを送って応答を記録し、標準出力に本文を出さないこと | 同 `tests/test_read_result_posts.py` |
| 指摘ファイルだけの状態と、何も書かなかった担当で、投稿が 0 件であること | 同上 |
...
| 回し直した実行で、前の実行の同じラウンド・同じスロットのレビューを重複投稿と取り違えないこと | `plugins/ndf/scripts/tests/test_result_posts.py` |
| 指摘ファイルへ送れた先を書けないとき、指摘ファイルを壊さず取り込みを止めること | 同上と `plugins/ndf/scripts/tests/test_statefile_emit.py` |
...
| スケルトンの起動し直しが繰り返しの先頭へ戻り、8 の枝を 7 より先に見ること | `plugins/ndf/skills/cross-review/tests/test_skill_layout.py` |
| 文書が投稿の担い手と種別ごとの契約を書いていること | 文書を読んで確かめる（照合していたテストは #885 で削除） |

## 関連リンク

- [issue #730](https://github.com/devbasex/ai-plugins/issues/730) — GitHub と git への書き込みをオーケストレーターへ移す
...
- [起動結果](cross-review-launch-outcome.md) — 結果なしの理由とリトライ可否
- [証拠ベースのレビューと効果の測定](cross-review-evidence-based.md) — 指摘の構造化・指摘区分・収束の判定
- [利用可能な参加者だけで始める収束ループ](cross-review-participants-and-seats.md) — スロット名と決め方
