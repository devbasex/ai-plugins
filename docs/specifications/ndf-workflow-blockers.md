...
**繰り返しの中で使う値は、繰り返しの中で得られる。** 提案・レビューの参加者プール
（`RUNTIMES` / `RUNTIMES_CSV`）は `refactor.py init` と `start-round` の両方が返す。

**スケルトンが参照する変数の出所は機械で見る。** `scripts/check-skill-shell-vars.py` が、
手順書の bash が参照する変数を、その行より前のコマンドが返しているかでチェックする。

**`--scope` のゲートは、名前で当たらないときだけ実体を見る。** 走査は配下 1 階層に限り、
返すのは**当たった置き場所そのもの**である。

**push の退避は、失敗してから 1 度だけ行う。** 既定の経路は変えない。退避の値はライブラリ
...
2 つの副コマンドが違う参加者プールを返すことはない。**足すだけで、既存の値と失敗の形は
変えない。**

**常に成り立つ条件**: スケルトンが繰り返しの中で参照する変数は、その繰り返しの中で実行する
...
| 副コマンドが返す値 | `cmd_<名前>` が呼ぶ `emit(...)` のキーワード。ヘルパー経由も 1 階層だけたどる |
| スケルトンの外から渡る値 | `--external` が宣言するもの |

**シェルの構文解析器は使わない。** スケルトンは代入・`for`・コマンド置換に限られており、
この範囲は後ろ向きの状態だけで読める。
...
**親のままにすると、通した直後に同じゲートの別の判定が拒む。** 後段は `--baseline-test` が
限定した探索の起点で始まるかを見る。起点が `.../tests` のとき、親のパスはその起点で
始まらない。

**双方向の包含（親が起点を含む場合も通す）は許さない。** ゲートが見たいのは「足したテストが
実行されるか」であり、実行されるのは起点の配下だけである。

**走査は配下 1 階層に限る。** 深く潜ると、無関係な階層のテストを根拠にしてゲートが素通りする。
...
| `pr` / `fix` の `SKILL.md` | 同じ `git` コマンドを案内する（ライブラリは読み込まない） |

**`pr` と `fix` がライブラリを読み込まないのは、任意のリポジトリで動く手順書だからである。**
読み込みの経路を増やすと、NDF を導入していない環境で手順が成立しない。

**ライブラリに、呼び出し側の無い関数を置かない。** 退避の分岐がシェルと Python の 2 か所に
分かれると、片方だけが更新される。

## 運用

**この 2 つの直しはリリースした版から効く。** 手元のリポジトリを直しても、利用者が起動するのは
リリース済みの Skill である。`cross-refactoring` を開発中のリポジトリで回すときは、リリース済みの
版が動いていることを踏まえて参加者プールを状態ファイルから読む。

## テスト観点

| 観点 | 何で確かめるか |
| --- | --- |
| `start-round` が参加者プールを返し、既存の値を変えないこと | `<crf>/tests/test_start_round_emits_runtimes.py` |
| スケルトンの変数の出所が突き合わせられること | `scripts/tests/test_skill_shell_vars.py` |
| 現行のスケルトンがチェックを通ること | 同上（実物を読む 2 件） |
| 実体を持つ親がゲートを通り、返す値が配下の側になること | `<crf>/tests/test_scope_gate.py` |
...
| 手順書の記載がライブラリの値と一致すること | `scripts/tests/test_push_fallback_docs.py` |
| ライブラリが値だけを持つこと | 同上 |
