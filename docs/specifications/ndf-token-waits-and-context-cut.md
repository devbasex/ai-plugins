...
読み直す呼び出し（フォアグラウンドの `sleep` の待ちと、変わらないファイルの読み直し）と、コンテキスト量が上限を
...
| 何を読むか | 正本 |
| --- | --- |
| 待ちの費用、許す待ち方と禁じる待ち方、待つ相手ごとの手、中間通知を受けたときの層ごとの手とコピーの待ちのコマンド、hook の止め方、4 ランタイムの扱い | `plugins/ndf/skills/development-workflow/references/waiting.md` |
...
| conductor が再開コマンドを出す時点 | `plugins/ndf/skills/development-workflow/SKILL.md`（「工程は 1 つの context window で通し切らなくてよい」の段落） |
| supervisor と worker が待ち方に従う規則、worker の起動指示の `置き場所`（報告コピーと完了マーカー） | `plugins/ndf/skills/development-workflow/references/agent-layers.md` |
...
フォアグラウンドの `sleep` を本体の会話でしか止めず、サブエージェントの中の `sleep 12 && echo` は 12 秒
...
| フォアグラウンド Bash | `run_in_background` を付けずに実行する Bash。終わるまで呼び出しが返らない |
| コンテキスト量 | 1 回の API 呼び出しで読んだトークン数。`input_tokens + cache_read_input_tokens + cache_creation_input_tokens` |
| 工程 Skill | `context-window.md` の 4 つのカットポイントの直後に始まる工程を持つ Skill と、入口の `development-workflow` / `issue-plan-strategy`（下の「工程 Skill の一覧」） |
| 中間通知 | 背景の処理を残したまま応答を終えたサブエージェントについて、親へ届く 1 回目の通知。注記に「background work of its own still running」「may be interim」と出る |
| 報告コピー | worker が起動指示の `置き場所` のファイルの末尾へ書く `## 作業の報告` の節。最後の応答の報告と同じ中身 |
| 完了マーカー | worker が報告コピーを書き終えた後に作る空のファイル `<置き場所>.done` |
| 再開コマンド | 新しい会話の最初に打てば、その工程から再開できるコマンド 1 行。`/ndf:development-workflow #<課題> [#<課題> ...]`。情報文字列 `ndf-next` の囲みのコードブロック 1 つで出す |

## 構成要素

| 要素 | 責務 |
| --- | --- |
| `plugins/ndf/scripts/token-guard.sh` | PreToolUse の入口。`tool_name` で 3 つの判定（`Bash` → sleep / `Read` → 連続 Read / `Skill`・`Agent`・`Task` → コンテキスト量）へ振り分け、拒否か通過を返す。排他は `scripts/lib/lock-common.sh` を読み込んで使う |
...
| `external-ai/references/cli-codex.md`・`cli-agy.md`・`qa-security-scan/03-report-template.md`・`release/references/completion-check.md` | フォアグラウンドの待ちのループの直前に「Claude Code では、このループを `run_in_background: true` で実行して完了通知を待つ」の 1 行を置き、`waiting.md` を指す |
...
graph TB
  ...
  subgraph ST["状態"]
    RS["連続 Read の記録と案内のマーカー<br/>guards/"]
...
**フォアグラウンド Bash で、コマンドの位置の `sleep` が次のどちらかに当たると拒否する。**
...
全プロジェクトの記録（6,713 本、Bash 74,223 件）で、`sleep <数>` を含むフォアグラウンド Bash は次のとおり
だった。

| 形 | 件数 | 費用（input 換算） |
| --- | ---: | ---: |
| フォアグラウンド・ループの中 | 1,543 | 67.3M |
| フォアグラウンド・ループなし | 743 | 9.9M |
...
**conductor が工程へ入る起動で、会話のコンテキスト量が上限（既定 200,000）を超えていると、1 度拒否
して再開コマンドを示す。** 工程へ入る起動は経路によって違うツールに現れるため、両方を見る。
...
- **サブエージェントの中の起動は見ない。** 入力に `agent_id` が付くか、`transcript_path` が
  ...
  途中で途切れる
- **工程でない Skill と、先頭語が作業種別（`調査` など）の Agent は見ない。** 工程の途中で
  起動されるためカットポイントにならない
- **拒否の後、次に工程へ入る起動が同じ鍵なら 1 度だけ通し、マーカーを消す。** 間に他のツールや
  工程でない Skill・Agent が挟まってもマーカーは残る。次の起動が別の鍵なら、上限を超えていればマーカーを
...
- **コンテキスト量を読めない（記録が無い・`usage` が無い）ときは通す**
...
**コンテキスト量は `transcript_path` の末尾 200 行の、最後の assistant 行の `message.usage` から読む。**
...
**conductor は `context-window.md` の 4 つのカットポイントと、コンテキスト量の hook が拒否したときにこの 1 行を
...
**supervisor は応答を終える前に、完了マーカーの出現を待つ Bash を自分の背景の処理として起動する。**
自分で起動した背景の Bash の完了通知なら supervisor は再開する（上の実測）。worker の終わりを
自分の背景の処理の終わりへ写すことで、supervisor が自分で起こされる手段を持つ。

| 層 | 常に成り立つ条件 |
| --- | --- |
| supervisor | worker を起動する前に、`置き場所` のファイルを worker ごとに新しいパスで空に作り、完了マーカーを消す。`置き場所` を「無し」にしない |
| worker | 最後の応答の前に、長い出力の有無に依らず報告コピーを `置き場所` の末尾へ書き、その後に完了マーカーを作る。`Monitor` で待つときも同じ |
| worker | 背景の処理を残したまま応答を終えない（規則 5）。コピーとマーカーはこの規則を緩めず、守れなかった worker を待つための備えである |
...
| 決定 | 理由 |
| --- | --- |
| 待つのは別ファイルの完了マーカーの出現で、`置き場所` の中身は待たない | `置き場所` は長い出力と共用のため、見出しや固定の行を待つと、書きかけのコピーや長い出力に同じ行が含まれたときにも反応する。別ファイルの存在は中身に左右されず、書き終えた後にだけ現れる |
| 起動の前に `置き場所` を新しいパスで空にし、マーカーを消す | 前の worker の報告やマーカーが残ったパスを渡すと、コピーの待ちが即座に終わり、今の worker の報告を待たない |
| 中間通知を受けても worker へ `SendMessage` を送らない | worker は背景の待ちが終わるまで報告を出せず、送っても同じく中間通知が返る |
| サブエージェントの出力ファイル（`tasks/*.output`）を背景で見張らない | `waiting.md` が読むことを禁じており、パスの形も Claude Code が約束していない |
| コピーの待ちに上限 3600 秒を置き、上限の後の扱いは既存の点検と `SendMessage` の規則を使う | worker が報告を書かずに落ちるとマーカーは現れず、上限が無いと永久に待つ。3600 秒は初期値で、worker の所要時間の実測で見直す。上限で起きても点検が 1 回挟まるだけで作業は失われない |
...
**配布物の文書にあるフォアグラウンドの待ちのループは、ループを書き換えずに案内の 1 行を足す。** 拒否の
...
| `context-<session_id>.json` | コンテキスト量の案内のマーカー。`{"key": "<鍵>"}`。鍵は Skill なら `skill\t<名前>\t<args>`、Agent なら `agent\t<description>` |
...
- **記録とマーカーの読み・判定・書き込みは session ごとのロックの中で行う。** 同じ session の hook が
  並列に走ると、置き換えだけでは `count` の更新やマーカーが失われる。ロックは `lock-common.sh` の
...
| `NDF_CONTEXT_GUARD` | `1` | `0` でコンテキスト量の判定を止める |
| `NDF_CONTEXT_LIMIT` | `200000` | コンテキスト量の上限（トークン） |
...
| `tool_input.skill` / `tool_input.args` | コンテキスト量（対話の経路） | 通す |
| `tool_input.description` | コンテキスト量（3 層の経路） | 通す |
| `session_id` | 連続 Read の記録・案内のマーカー | 通す |
| `transcript_path` | コンテキスト量 | 通す |
| `agent_id` | コンテキスト量（付いていれば見ない） | conductor とみなす |
...
| コンテキスト量 | コンテキスト量と上限、次のコマンドを `ndf-next` のブロック 1 つで示すこと（中身は再開コマンド、`/goal` で始めたセッションなら先頭に `/goal `）、続けるなら同じ起動をもう一度行うこと。ラッパーの直接の子では、止め続けること・動いている supervisor の報告を待ってから引継ぎ文書を更新してブロックを出すこと。規約 `context-window.md`。`NDF_CONTEXT_GUARD=0` と `NDF_CONTEXT_LIMIT` |

### 4 ランタイム

**hook は Claude Code にだけ登録し、他の 3 ランタイムは規約で守る。** 拒否の理由が案内する
代わりの手段（`Monitor` / `run_in_background` の通知）は Claude Code にしか無く、コンテキスト量も
...
- **止める:** 環境変数で判定ごとに止める。hook そのものを外すなら `hooks/claude.json` の登録を
  1 つ外す。データの移行は無い
- **続ける:** コンテキスト量の拒否の後、利用者がこのまま続けると決めたら、同じ起動をもう一度行う。
  ラッパーの下ではこの手は無く、会話を切る
...
- フォアグラウンドの `sleep` の待ち（`sleep 30 && tail`・`while` / `until` の本体の `sleep`・`bash -c` / `sh -c` /
...
- コンテキスト量が上限を超えた conductor の工程 Skill とフェーズの Agent を拒否し、再開コマンドに課題番号を
  示すこと。番号が無ければ `<課題番号>` のまま示すこと
- サブエージェントの中の起動・工程でない Skill・作業種別の Agent を通すこと
...
- `waiting.md` が 1 か所にあって `agent-layers.md` の supervisor と worker の規則から参照され、
  Claude Code 向けにフォアグラウンドの `sleep` のループを勧める例が無いこと。`context-window.md` に #827 の
  実測値・上限の値・戻す手順があり、`SKILL.md` に再開コマンドの規約があること。README に
  4 ランタイムの表があること
...
- [#901](https://github.com/devbasex/ai-plugins/issues/901) — supervisor が worker の中間通知で止まる（実装は [PR #910](https://github.com/devbasex/ai-plugins/pull/910)）
- [#895](https://github.com/devbasex/ai-plugins/issues/895) — 再開コマンドを `ndf-next` のブロックにし、ラッパーの下ではコンテキスト量の拒否を止め続ける（[ndf-relay-segment-restart.md](ndf-relay-segment-restart.md)）
