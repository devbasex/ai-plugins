...
| 設計レビューに新しい Skill を作らない | `pr` → `cross-review` → `merged` と手順が同じであり、作ると同じ内容が 2 箇所に増える |
...
`design` は永続データを持たない。盤面（GitHub Projects）へ記録する値は `設計` と
`設計レビュー` で、`.ndf/projects.json` を持つリポジトリでだけ働く。**盤面の単一選択へこの
2 つの値を足すのは、盤面を持つリポジトリ側の操作である。**
