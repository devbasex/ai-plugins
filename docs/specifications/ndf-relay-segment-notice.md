...
| カットポイントの再起動を関門に数えないこと | 関門の前に会話を切らない規則（`context-window.md` のまま） |
...
| カットポイントの再起動は関門に数えず、ブロックの前に確認を挟まない | 状態は課題の本文と Pull Request に残り取り消せる。ラッパーの下で `AskUserQuestion` を出すと質問の合図が切り替えを止め、人の入力が 1 回増える。ラッパーの外でも、確認の答えと貼り付けで入力が 2 度になる |
...
- カットポイントの再起動は関門に数えない。引継ぎ文書を更新したら、承認・確認（`AskUserQuestion` を含む）を挟まずに告知とブロックを出して応答を終える
- ラッパーの外でも挟まない
