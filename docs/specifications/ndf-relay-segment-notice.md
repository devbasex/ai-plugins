# カットポイントの告知（`relay.py notice`）と、カットポイントで承認を挟まない規則
...
固定する。N は静止の秒数（`NDF_RELAY_QUIET`、既定 5）である。文面と秒数は `relay.py notice` が出し、
...
この文書は、告知の入出力の契約と、カットポイントで承認を挟まない規則と、それぞれをそう決めた理由を残す。

| 何を読むか | 正本 |
| --- | --- |
| 告知の書き方・カットポイントで承認を挟まないこと（conductor の規則） | `plugins/ndf/skills/development-workflow/references/context-window.md` の「新しい会話で戻す」 |
| `notice` の呼び方・カットポイントの再起動が関門でないこと | `plugins/ndf/skills/development-workflow/SKILL.md`（ラッパーの段落と「人手の承認を求める関門」） |
| `/ndf:restart` の判定と示し方 | `plugins/ndf/skills/restart/SKILL.md` |
| 文面と秒数 | `plugins/ndf/scripts/relay.py` の `notice_lines` |

## 概要

**例: 区間 2 の conductor が文脈量の上限で止められ、区間を切る（静止が既定の 5 秒のとき）。**
最後の応答は次のようになる。
...
| 5 | 利用者 | 何もせずに待つ |
| 6 | ラッパー | 合図を拾い、静止を待って次の区間へ切り替える（ラッパーの本体は変えていない） |

## 対象範囲

| 含む | 含まない |
| --- | --- |
| ラッパーの下・外で、カットポイントの告知に何を書くか（conductor の 4 つのカットポイント・文脈量の hook に止められたとき・`/ndf:restart`） | 静止の数え方（端末のどの入力でも数え直す） |
| 告知の文面と秒数を出す `relay.py notice` | ラッパーが待っている間に端末へ 1 行を出すこと |
| カットポイントの再起動が関門ではないこと | 関門の前に会話を切らない規則（`context-window.md` のまま） |
...
| LLM に残すのは「いまがカットポイントか」と「再開用のコマンドの中身」の 2 つ | どちらも報告の `結果`・`/goal` の目標・引継ぎ文書の名指しに依る判断である。告知とブロックをまとめて出すスクリプトは作らない（コマンドを引数で渡すと写すだけになり、減る判断が無い） |
| カットポイントの再起動は関門ではなく、ブロックの前に確認を挟まない | 状態は課題の本文と Pull Request に残り取り消せる。ラッパーの下で `AskUserQuestion` を出すと質問の合図が切り替えを止め、人の入力が 1 回増える。ラッパーの外でも、確認の答えと貼り付けで入力が 2 度になる |
| `/goal` の文面の「承認を求める」は関門 2 つだけを指す | カットポイントの再起動の前に「再起動してよいか」を尋ねたのは、`/goal` の文面をこのカットポイントにも当てたためである |
| N は静止の秒数そのもので、切り替えの時間を足さない | 静止自体が人の入力と重ならないための余白で、そこへ `/exit` → 更新 → 起動の時間を足すと余白に余白を重ねる。伝えたいのは「待っている間は操作しない」ことである。背景の処理などで延びるため「約」を付け、表示は切り上げず最も近い整数にする |
| ラッパーの本体は、待っている間に端末へ 1 行を出さない | 子の claude の TUI が画面を持っている間に書くと、再描画で消えるか入力欄と重なる。ラッパーが出すのは今と同じく `ndf-relay:` の 1 行だけで、告知の 2 行目はその 1 行を案内する |
| 静止は端末のどの入力でも数え直すまま残す | 入力の種類を見分けて一部を無視すると、人が打ち始めた入力へ `/exit` を重ねる経路ができる。告知で「操作せずに待つ」と伝えるほうが壊れる経路を作らない |
| 1 行目は `is-child` の判定と常に一致させ、自動で切り替わるかは 2 行目で表す | 1 行目を切り替えの可否で変えると、`token-guard.sh` がラッパーの子を外と読み、上限を超えた起動を 1 度通してしまう |

フェーズをどこで切るかは変えない。カットポイントが増えても、増えたカットポイントの告知はこの規則に従う（#954 と接する点）。
...
- ラッパーの下（1 行目が `relay`）の告知に、手で入力させる文（「次の 1 行を使って」「貼り付けて」）を書かない。例外は 2 行目そのものが貼り付けの手順を示すとき（`NDF_RELAY_QUIET=inf`）
...
| 文脈量の hook（`token-guard.sh`） | `NDF_RELAY_DIR` があり上限を超えたときだけ、以前の `is-child` の代わりに `notice` を 1 回起動する（起動の数は変わらない）。1 行目が `relay` ならラッパーの下として止め続け（`NDF_RELAY_QUIET=inf` でも 1 度の通しをしない）、拒否文に 2 行目をそのまま埋め込み、「承認や確認を挟まずに出して終える」と書く。ラッパーの外の拒否文は変えない。conductor は拒否文の 1 文を実行し直さずに写してよい |
...
- カットポイントの再起動は関門ではない。引継ぎ文書を更新したら、承認・確認（`AskUserQuestion` を含む）を挟まずに告知とブロックを出して応答を終える
- ラッパーの外でも挟まない
- `/goal` の文面が「承認を求める」と書いていても、指すのは関門 2 つ（設計 Pull Request のマージ・本番の系へ届く操作）だけである
- 関門の報告（`結果: 関門`）を受けたカットポイントでは、関門の承認と取り込みの後にブロックを出す（関門の前に会話を切らない）
...
| 実機での確認 | ラッパーの下で `/ndf:restart` を打ち、告知の 1 行に秒数が出て、何も押さずに切り替わるのを見る（リリース後テスト） |
