...
| 1 日の起動回数の上限は 20、静まりは 5 秒を初期値にし、環境変数で変える | 1 日に 2〜3 のまとまりを進めても 20 には届かず、空回りの検出を抜けた暴走は 1 日で止まる。静まりは切り替えの待ちにそのまま足されるため短くする（15 秒では Stop から次の区間の起動まで約 34 秒かかった）。`/exit` を早く打ちすぎないことは秒数ではなく、目標の判定の記録・質問の印・利用者の入力の待ちが受け持つ |
