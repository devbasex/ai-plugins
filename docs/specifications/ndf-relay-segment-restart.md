...
| `/goal` の目標の判定を待たない（#994） | 未達の判定は止めを拒んで応答を再開させ、「印の後に応答が再開した」で印が無効になる。判定を待つ処理はこれと重なり、`/goal clear` の行（`met: true` と `sentinel: true` の両方を持つ）を新しい目標と読んで切り替えを止めた |
...
"goal_status"` に書かれる。**中継はこの行を読まない**（#994）。未達の判定は止めを拒んで応答を
