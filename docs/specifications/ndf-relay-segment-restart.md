...
| 2 つ目以降のセッションへ、`run` の引数のうち起動の方針を表すものだけを引き継ぐ（#936） | 何も引き継がないと、devbase の `alias claude` が足す `--dangerously-skip-permissions` が 2 つ目のセッションで落ちる。`--resume` や `-c`・最初のプロンプトを引き継ぐと捨てた会話へ戻るので、会話ごと・セッションごとのものは値ごと落とす（[導入の仕様](ndf-relay-install-and-restart.md)の「起動の方針の引数の引継ぎ」） |
