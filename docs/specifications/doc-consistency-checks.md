# 説明文書のチェック — リンクの照合と版数の例

説明文書の食い違いを継続的統合で落とす 2 本のチェックのうち、見出しへの参照の照合・`issues/` の
走査・インラインコードの扱い（`scripts/check-markdown-links.py`）と、版と配布の正本の
「版の付け方と開発版の配布」章のチェック J（`scripts/check-doc-staleness.py`）について、確定した
振る舞いと決定の理由を残す。
...
- **リンクのチェックは、ファイルの存在に加えて見出しへの参照を照合する。** `#見出し` と
  `相手.md#見出し` の 2 形が、存在しない見出しを指していれば `missing heading anchor` で落ちる
- **`issues/`（`issues/old/` を含む）はリンクのチェックの走査に入る。** 設計文書と計画は分割や
  移設のたびに参照が壊れうるため、`docs/` へ移る前の期間もチェックする
- **インラインコードの中の記法はリンクとして読まない。** 記法を説明する例で誤って落ちない
- **文書が指す参照の実在は、このチェックが失敗として出す（pytest では見ない）。** `.md` を読む pytest は
  文言の照合と区別が付かないため置かない（[#885 の分類の規則](cross-refactoring-round-tests-and-assess.md#リポジトリに文言固定テストを置かない885)）
- **チェック J は正本（`docs/versioning-and-distribution.md`）の章を読む。** 章に並ぶ版数の基底を
...
**入力は見出しの Markdown そのままで、囲みの記号を先に落とす手順は置かない。** インライン
コードの `` ` `` も山括弧も手順 2 で落ちるため、`` `<worktree-base>` の解決順 `` は GitHub と同じ
...
**走査の対象外の文書は見出しを読まない。** 照合するために対象外の文書を読むと、チェックの母集合が
実質的に広がる。見出しの名前のキャッシュは実行 1 回のあいだだけ辞書で持ち、同じ文書を 2 度読まない。
...
**母集合はファイルシステムから集める形のまま変えない。** メインディレクトリの `issues/` には未追跡の
...
深さを 3 に固定して `## ` の章を閉じると、章の中の `### ` 小見出しで章が途切れる。

**正本がディレクトリへ分かれた場合も、チェック J が読むのは章 2 を持つ 1 ファイルである。** 分けた
ままチェックを動かし忘れても、読む先が無いか版数を読み取れないこととして落ち、素通りしない。
...
| 決定 | 理由 |
| --- | --- |
| 次に出す版を指す例をチェックで止め、手で直す箇所の表には足さない | 版数を一括で置換すると、次の版を指す例が現行版を指す形へ崩れる。v10.10.0 は開発版の行が正式版と同じ値のままリリースされた。人の読み直しに頼る限り、置換のたびに同じ機会が生じる |
| 例を固定の値（`9.9.9` / `X.Y.Z`）にしない | 基底の規則で落ちるか版数を読み取れず、チェック J の撤去を伴う。章の古い版数を見つける既存の機能まで失う |
...
| 語が見つからない・重なるときは失敗にする | 記載を消してもチェックは通らない、という既存の方針に揃える。黙って通すと表の行を言い換えるだけで規則が外れる |
...
- [issue #499](https://github.com/devbasex/ai-plugins/issues/499) / [PR #594](https://github.com/devbasex/ai-plugins/pull/594)（実装） — チェック J の読む先を正本へ移す
...
- [issue #885](https://github.com/devbasex/ai-plugins/issues/885) / [PR #918](https://github.com/devbasex/ai-plugins/pull/918)（実装） — 参照の実在を見ていた pytest をこのチェックへ寄せる
