...
worktree ごとのテスト環境で、**ロックを握られているかの判定**と、**割り当ての台帳へ書けなかった
ときの扱い**を決めた経緯と結論を残す。
...
| 停止の対象へ戻したロックのディレクトリを `reap` が取り除かない | 取り除きは `token` を照合するゲート（`_ndf_lock_discard`）を通す必要がある。ゲートを通らずに消すと「別の担当が取り直したロックを壊す」（#297 で直した事象）が起きる。残したディレクトリは次の `test` の取得が捨てるため、結果は変わらない |
...
`expose` の先着 1 本のゲートは以前から戻り値を受けているため、この表に入らない。
...
台帳は共通の git ディレクトリ配下に置く。worktree の中に置くと、削除した時点で割り当ての記録が
...
- [worktree 運用の宣言とエントリポイントの判定](ndf-worktree-declaration-and-entry-points.md)
