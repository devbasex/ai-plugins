...
進む。**リファクタリング計画の後で LLM が動くのは作業の CLI（テストの追加・実装・直し・最終ゲートの修正）
...
| 時点 | 所要 | 経過 |
| --- | ---: | ---: |
| `init`（着手前の全体のテスト 1 回とラウンドテスト 1 回） | 約 1.5 分 | 1.5 分 |
| 提案（3 者が並行。律速は codex） | 約 4.5 分 | 6 分 |
| リファクタリング計画（実装担当 claude が 1 回） | 約 3 分 | 9 分 |

残り 51 分からバッファ（全体のテスト 1 分 × 2、修正 5.5 分 × 2）の 13 分を引いた 38 分に、項目を
...
| 実装担当 | `implementer` | リファクタリング計画・テスト追加・実装・修正・最終ゲートの修正を通して担う 1 ランタイム |
| 候補 | `candidates[]` | 提案を鍵（`path` + `symbol` + `smell`）でまとめたもの。リファクタリング計画へ渡す |
| 改善項目 | `items[]` | リファクタリング計画が採った候補。`I-001` の形の ID を持つ。1 改善項目 = 1 コミット（テストを足す項目は 2 コミット） |
...
| 候補の切り出し、リファクタリング計画の取り込み、見積り・バッファ・件数の選び方・締め切り | 検証・危険フラグ・取り消し・最終ゲート（[検証と最終ゲートの確定仕様](cross-refactoring-verify-and-final-gate.md)） |
...
| 17 | 提案は観点を並べて観点ごとに探させ、件数の上限を示さない。リファクタリング計画へ渡す候補は `path` + `symbol` の組で上位 30 組・組の中の 3 件まで | 一覧が無いと目についた兆候に偏る（#917 は `long_method` が 10 件）。上限を示すと出す前に絞られる。組で切れば統合で組の数が減らない |
...
| 26 | 最終ゲートの修正 1 回分（`final_fix`）をバッファに足し、1 回目の最終ゲートの修正は時計で打ち切らない | 検証の直しがバッファを使い切ると、最終ゲートが落ちたときに 1 度も直せずに終わる。全体のテストが落ちたまま残るのが最も悪い結末である |
...
| 最終ゲートの修正の 1 回目の長さの下限 | バッファの `final_fix` を秒へ直した値 | `final_fix_seconds` |
...
| リファクタリング計画の取り込みが順位・見積り・テスト・対象を持ち、リファクタリング計画を読めなくても止まらず、叩き直しで作り直さず、実行時の値をすべて書き出すこと | 同 `tests/test_merge_plan.py` |
...
| `start-phase` が開始を 1 度だけ書き、最終ゲートの修正の 1 回目にバッファの長さを渡すこと | 同 `test_init.py` |
