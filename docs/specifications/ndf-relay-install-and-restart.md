...
**ラッパーの本体の契約（パススルー・アイドル・上限・空回り・シグナルファイル・記録）は
...
| 順 | 誰が | 何をする |
| ---: | --- | --- |
| 1 | 10.17.4〜10.17.6 の利用者 | 次の版へ上げて claude を起動する。SessionStart hook の `relay.py startup` が、10.17.4〜10.17.6 が置いた旧いコピー（`~/.local/share/ndf/relay.py`）を今の版で置き直し、`rc-added` に載った `~/.bashrc` に囲みが残っているのを読んで、「10.17.4〜10.17.6 が自動で足したもの…」の 1 行を 1 度だけ出す。シェルの設定は書かない |
| 2 | 同じ利用者 | そのまま `claude` と打つ。囲みの alias が、置き直された旧いコピーのラッパーを起こす |
| 3 | 新しい利用者 | claude の中で `/ndf:install-wrapper` を打つ。コピーを `~/.claude/ndf/relay.py`、`claude` の関数を `~/.claude/ndf/shellrc` に置き、`~/.bashrc`（macOS の bash では `~/.bash_profile`）をバックアップしてから、`shellrc` を読む 1 行の囲みを足す。次に開いたシェルから効く |
| 4 | どちらの利用者も | ラッパーの下で `/ndf:restart` を打つ。claude が再開用のコマンドを `ndf-next` のブロックで出して応答を終え、ラッパーがアイドル（5 秒）の後に `/exit` → 更新 → 起動を行う |
| 5 | 外したい利用者 | `/ndf:install-wrapper uninstall`。`~/.bashrc`・`~/.bash_profile`・`~/.zshrc` の囲みを外し（バックアップの後）、`shellrc`・コピー・旧いコピーを消す |
...
| 旧いコピー | `${XDG_DATA_HOME:-~/.local/share}/ndf/relay.py`。10.17.4〜10.17.6 が置いたコピーで、10.17.4〜10.17.6 の囲みの alias が指す |
...
| 質問のシグナルファイル | 作業ディレクトリの `question`。質問が表示されているあいだ在る |
| 送り込み | ラッパーが子の擬似端末へ、`/exit` 以外の入力を書くこと。実装は #931 |
| 起動の方針の引数 | 最初のセッションの `claude` の引数のうち、会話ごと・セッションごとに変わらないもの（`--dangerously-skip-permissions`・`--model` など） |
...
| 扱う | 扱わない |
| --- | --- |
| SessionStart hook から導入を外し、在るコピーの置き直しと自動の囲みの知らせにすること | 送り込みの実装（#931。実測と不変条件はこの文書） |
...
| 2 つ目以降のセッションへの起動の方針の引数の引継ぎ | アイドルの秒数・上限・空回り・パススルー・`mark` のシグナルファイルの判定の変更 |
...
| 旧いコピーと状態の親は動かさない | 旧いコピーを動かすと 10.17.4〜10.17.6 の囲みの alias が壊れる。状態の親の記録はこのコンテナの `~/.bashrc` のパスを持つので、シェルの設定と同じく作り直しで消える場所に置けば食い違わない |
...
| devbase 以外（変数が無い）では `install` がシェルの設定へ囲みを足す。作り直しのたびの再導入の案内は採らない | 明示の操作なので案内だけにしない。再導入の案内は作り直すたびに打ち直しを求め、`uninstall` した利用者にも「外した」知らせが消えて出る |
...
| 再起動は `ndf-next` のブロックと既存のラッパーの経路で行い、ラッパーに「今すぐ再起動」の副命令を足さない | 同じ経路なら、答え待ち・背景の処理・アイドル・上限・空回りと守りがそのまま効く。Skill の Bash からラッパーへ直接送ると、応答を終える前に `/exit` が入る |
...
| 再開用のコマンドに承認・同意・判断の結果を書かない | 次のセッションの claude はそれを人の入力として読むため、承認ゲートを越える経路になる。承認は課題の本文と Pull Request から次のセッションが読み直す |
| 承認ゲートを越えない守り（G1〜G3）は、送り込みを待たずに既存の `/exit` に入れる | 質問の表示中にラッパーが書いた `\r` が選択肢 1 を決めると実測で分かった。10.17.4〜10.17.6 のカットポイントの `/exit` にも経路があり、`/ndf:restart` はそれを利用者が打てる形で増やす |
...
| 質問のシグナルファイルは `mark`（Stop）でも消す | `Esc` で取り消すと `PostToolUse` が来ずシグナルファイルが残り、切り替えが永久に止まる。Stop が起きたなら質問は表示されていない |
| 2 つ目以降のセッションへ、最初のセッションの引数から会話ごと・セッションごとのものを除いた残りを引き継ぐ（#936） | 全部引き継ぐと `--resume` や最初のプロンプトで前の会話へ戻る。何も引き継がないと devbase の alias の `--dangerously-skip-permissions` が落ちる。対話シェルに alias を聞く案は、利用者の rc が走り手で付けた引数を拾えない |
| 知らない選択肢も値を取る規則のまま引き継ぐ | 落とすより付けるほうが起動の方針を失わない |
| 送り込みは実測と不変条件までをこの文書で決め、実装は #931 に分ける | 許可の一覧・判定・モデルからの要求の形を持ち、導入のリリースを送り込みのレビューで待たせない |
...
- **`startup` はシェルの設定・ラッパーの rc・読み込み先のファイルを書かない。** 書くのは在るコピー・コピーの版・
  旧いコピーの置き直しと、状態の親の `rc-noticed` だけで、例外も含めて終了コード 0 で終わる
...
- **ラッパーは、質問のシグナルファイルがある間・シグナルファイルの後に応答の行がある間は、子の端末へ何も書かない**
...
10.17.4〜10.17.6 の囲みは alias を直に持ち、旧いコピーを指す。
...
| E6 | 報告 | 囲みを足すか置き換えたパスを `rc-added`（無ければ）と `rc-user` に足す | `<読み込み先> から <ラッパーの rc> を読むようにした（バックアップ <パス>）。次に開くシェルから効く` と `ラッパーの本体 <複製>（版 <版>）` |
...
形（読み込みの行か直の alias か、自動の囲みか、閉じが無いか）、ラッパーの rc の有無、コピーと旧いコピーそれぞれが
今のプラグインの `relay.py` と同じか違うか無いか、コピーの版を 1 行ずつ示す。`~/.bash_profile` の囲みが無い行は
macOS でだけ示す。何も書かない。
...
**質問のシグナルファイル `question`**（作業ディレクトリのファイル。`next.json` の形は変えない）:
...
**終わらせる手順（G3）。** (1)〜(5) がそろい、停止のシグナルファイル・`count.lock`・1 日の上限・空回りを通った後:
...
| ある | `count.lock` を 5 秒まで待って取り直し、1 日の上限と空回りを判定し直してから、読み直したシグナルファイルの `command` と `cwd` で起動する。取れなければ `count-lock`、上限・空回りならその理由で `stop` の行を書き、`次の区間を起動できない（<理由>）。次のコマンド:` と中身を出して終了コード 2 |
...
| # | 不変条件 | 実現 |
| --- | --- | --- |
| G1 | 質問の表示中は、子の端末へ何も書かない | 質問のシグナルファイル（上の「承認ゲートを越えない守り」） |
...
- **移行:** 10.17.4〜10.17.6 の囲みは旧いコピーを指したまま動き、hook は囲みの中を書き換えない（向け直すのは明示の
...
- **devbasex/devbase#253 の前の devbase** では `install` は囲みを使い、コンテナを作り直すと囲みが消えてコピーと
...
| 再開用のコマンドの中身 | 定型に限るのはモデルで、機械のチェックは無い |
...
| `install` が bash / zsh（`ZDOTDIR`）でコピー・コピーの版・ラッパーの rc・囲み・バックアップを作り、2 回目は足さず、空の HOME・末尾の改行が無い設定・`rc-added` の記録がある状態からも足すこと | `test_install_bash_first_then_idempotent` ほか `test_install_*` |
...
| 質問のシグナルファイルがある間・シグナルファイルの後に `assistant` の行がある間は `/exit` が届かず、`attachment` の行では止まらないこと。`mark` がシグナルファイルを消すこと | `test_guard_question_blocks_exit`・`test_guard_reply_after_mark_blocks`・`test_guard_attachment_row_does_not_block`・`test_guard_mark_clears_question` |
...
- [#936](https://github.com/devbasex/ai-plugins/issues/936) — 2 つ目以降のセッションへ起動の方針の引数を引き継ぐ
