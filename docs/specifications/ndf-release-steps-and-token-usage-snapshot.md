# リリースコマンドの宣言と、版ごとのトークン消費の記録

リポジトリに固有のリリースの手順を、`release` の本文ではなくリポジトリの宣言 `.ndf/release.json` が
「リリースコマンド」として持つ。`release` は手順 3 で、宣言されたコマンドを 1 行のコマンド（配布物の
...
| 機能 | 誰が使うか |
| --- | --- |
| リポジトリが宣言したリリースコマンドを、リリースの段階に合わせて走らせる | `release` を実行するエージェント（どのリポジトリでも） |
| リリースコマンドを宣言する | リポジトリの管理者 |
...
| 要素 | 置き場所 | 責務 |
| --- | --- | --- |
| `release` の本文 | `plugins/ndf/skills/release/SKILL.md` | 手順 3 で宣言されたリリースコマンドを走らせる。完了の判定に「コマンドが 0 で終わり、手引きの判断を済ませた」を持つ |
...
| 用語 | 意味 |
| --- | --- |
| リリースコマンド | 宣言の `steps` が持つ 1 つのコマンド。リリースの段階ごとに `release` の手順 3 で走る |
| 宣言 | リポジトリ側に置く `.ndf/release.json`。無ければ何も走らない |
| 段階 | リリースの種類。`production`（本番リリース: 正式版を公開する・本番へ反映する）/ `verification`（検証リリース: 開発版を公開する・検証環境へ反映する） |
...
- **記録はリリースの Pull Request に含め、別の Pull Request にしない。** マージとリリースの手順が増えない。
  記録は `docs/` の下で配布物ではないため、`release` の「リリースそのものを目的とする変更は対象外」の
  規定にもそのまま収まる
...
- **同じ日に別の版の記録があるときは `-2` を付けて別に作り、同じ版の記録は書き直す。** 別の版の記録を
  ...
  増やさない
- **コマンドに書いてよい場所を宣言させ、外を変えたら失敗にする。** コマンドは任意のコマンドで、リリースの Pull Request に
...
| このリポジトリの宣言が本番リリースでコマンドを 1 つ持つ | 同上。実物では `release-steps.py check --root .` が 0、`run --root . --stage production --version 10.17.9 --dry-run` がコマンドを 1 つ挙げる |
