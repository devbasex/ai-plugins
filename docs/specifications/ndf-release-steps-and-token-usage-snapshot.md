# 配布のコマンドの宣言と、版ごとのトークン消費の記録

リポジトリに固有の配布の手順を、`release` の本文ではなくリポジトリの宣言 `.ndf/release.json` が
「配布のコマンド」として持つ。`release` は手順 3 で、宣言されたコマンドを 1 行のコマンド（配布物の
`plugins/ndf/scripts/release-steps.py`）で走らせるだけである。このリポジトリはコマンドを 1 つ宣言し、
正式版を出すたびに版ごとのトークン消費と所要時間の記録（`docs/metrics/ndf-token-usage/<集計日>.md` /
`.json`）を配布の Pull Request へ載せる。この文書は、コマンドの実行と記録の書き出しの契約と、決めたことの
理由を残す。
...
このリポジトリの `.ndf/release.json` はコマンドを 1 つ宣言しており、上のコマンドは根で次を実行する。
...
| 機能 | 誰が使うか |
| --- | --- |
| リポジトリが宣言した配布のコマンドを、配布の段階に合わせて走らせる | `release` を実行するエージェント（どのリポジトリでも） |
| 配布のコマンドを宣言する | リポジトリの管理者 |
| 正式版を出すときに、版ごとのトークン消費と所要時間の記録を書き出す | ai-plugins の配布の担い手 |
| 前の行との差が大きい版を知らせ、読み取りを書く手引きを示す | 同上 |

**`release` の本文は「宣言があれば走らせる」の 1 文だけを持つ。** 宣言の無いリポジトリでは何も
...
| 要素 | 置き場所 | 責務 |
| --- | --- | --- |
| `release` の本文 | `plugins/ndf/skills/release/SKILL.md` | 手順 3 で宣言された配布のコマンドを走らせる。完了の判定に「コマンドが 0 で終わり、手引きの判断を済ませた」を持つ |
| コマンドの参照 | `plugins/ndf/skills/release/references/release-steps.md` | 宣言の書き方・段階の値・終了コード |
| 宣言の定義 | `plugins/ndf/skills/release/schemas/release.schema.json` | 宣言の JSON Schema |
| コマンドの実行 | `plugins/ndf/scripts/release-steps.py` | 宣言を読み、段階に合うコマンドを順に実行する。書いてよい場所の外が変わったら失敗にする |
| このリポジトリの宣言 | `.ndf/release.json` | コマンドは 1 つ（トークン消費の記録） |
...
GitHub へ出すのは `release` の既存の手順（配布の Pull Request）で、コマンドは push しない。

## 用語

| 用語 | 意味 |
| --- | --- |
| 配布のコマンド | 宣言の `steps` が持つ 1 つのコマンド。配布の段階ごとに `release` の手順 3 で走る |
...
（`check-skill-repo-assumptions.py`）にも反する。宣言にすれば、本文は「宣言があれば走らせる」の 1 文で
...
- **同じ日に別の版の記録があるときは `-2` を付けて別に作り、同じ版の記録は書き直す。** 別の版の記録を
  ...
  増やさない
- **コマンドに書いてよい場所を宣言させ、外を変えたら失敗にする。** コマンドは任意のコマンドで、配布の Pull Request に
  何が混ざるかを本文から読めない。宣言させれば、差分にコマンドの分として何が入りうるかが宣言から決まる
...
| 引数 | 既定 | 意味 |
| --- | --- | --- |
| `--root` | `.` | リポジトリの根。宣言を `<root>/.ndf/release.json` から読み、コマンドの作業ディレクトリにする |
...
| 終了コード | 意味 |
| --- | --- |
| 0 | 宣言が無い（`run` は何も出力しない）、段階に合うコマンドが無い、またはすべてのコマンドが 0 で終わり書いてよい場所の中だけが変わった |
...
1. 宣言の `steps` を書いた順に見て、`stage` が `--stage` と同じか `any` のコマンドだけを選ぶ
2. `command` の各要素の `{version}` を `--version` で置き換える。置き換えるのはこの 1 つだけである
3. コマンドの前の控えを取り、コマンドを `timeout_seconds` を上限に実行する。コマンドの標準出力はそのまま流す
4. コマンドの後の控えと比べ、`コマンド: <name> → <終了コード>` と、変えたパスを 1 行ずつ
...
| 宣言なしで `run` は出力なし・0、`check` は 2。段階が合うコマンドだけを実行し `{version}` を置き換える。`any` は両方で走る。`--root` と別のディレクトリから呼んでもコマンドは `--root` で走る。`guide:` とコマンドの標準出力が出る。`--dry-run` は実行しない | `plugins/ndf/scripts/tests/test_release_steps.py`（一時リポジトリに `git init` と宣言を作り `--root` で渡す） |
...
| このリポジトリの宣言が本番への配布でコマンドを 1 つ持つ | 同上。実物では `release-steps.py check --root .` が 0、`run --root . --stage production --version 10.17.9 --dry-run` がコマンドを 1 つ挙げる |
