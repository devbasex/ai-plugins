...
| Skill | 配布 | 役割 | 判定 |
| --- | --- | --- | --- |
| `development-workflow` | CXK | 変更を 5 つのモードへ分類し工程へ振り分ける。判定基準の唯一の置き場所 | 未測定 |
| `requirements-design` | CXK | 要求から受け入れ条件と仕様を起こす | 未測定 |
| `tdd-cycle` | CXK | 失敗するテスト → 最小実装 → 整理のサイクル | 未測定 |
| `safe-refactoring` | CXK | コードスメル起点のリファクタリングと現状固定テスト | 未測定 |
...
チェックが失敗する）。実測では、旧書式で宣言していたトリガ語が届かず新書式で届いた例があり、
戻すことが解決になるとは限らない。
