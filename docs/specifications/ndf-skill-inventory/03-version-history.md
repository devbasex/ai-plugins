...
途中で作った中間の版で、リリースはしていない。したがってリリース済みの v7.0.0 から見ると 30 個のままである。

統合の根拠は本書「判断基準」節の「機能が他 Skill と重複するものは、統合の対象とし、内容は
統合先へ残す」である。観点を独立 Skill として置くと、発動条件が「コードを書くとき」以外に
書けず、常に該当するトリガは発動判定として働かない。リファクタリングの起点である
...
Skill 数以外は全 family 合計（ndf + playwright-kit）である。v7.1.0 はリリースしていない中間の版だが、
31 個へ増えた地点の実測として残す。

| 指標 | v7.0.0 | v7.1.0（未リリース） | v8.0.0 | 運用値 |
| --- | ---: | ---: | ---: | ---: |
...
| Skill | 配布 | 役割 | 判定 |
| --- | --- | --- | --- |
| `release-verification` | CXK | リリースされた成果物を、利用者の環境で受け入れ条件に照らして確かめる | 未測定 |
...
対する証跡を求めるもので、`release-verification` の対象はリリースされた成果物と利用者の環境である。
マージ前には用意できない条件（実物のデータ、サービス一式、導入済みの環境、リリースの手続き
そのもの）を扱う。
...
Skill を 35 個から **36 個**へ増やした。ミッションの Pull Request をマージした後に版を上げて
リリースする工程が、どの担当にも属していなかった（#188）。追加した 1 個は起動実績をまだ持たない
ため、判定は次回の測定まで保留する。

| Skill | 配布 | 役割 | 判定 |
| --- | --- | --- | --- |
| `release` | CXK | ミッションのマージが終わった後に版を上げてリリースし、 `release-verification` へ渡す | 未測定 |

工程の並びは `merged` → `release` → `release-verification` である。`merged` はマージ後の
後片付けを終えたところで「このマージがミッションの最後だったか」を報告し、最後だった場合に
`release` へ渡す。`release` は版を上げてリリースし、`release-verification` の開始条件である
「版のリリースが終わっていること」を満たす。この 3 つを分けるのは、担い手が違うためである。
後片付けは各 Pull Request の担当が行い、版を上げるのはミッションの最後のマージを行った側が
行い、リリースされた成果物を確かめるのは利用者の環境を持つ側が行う。
...
版数を見出しに置いていないのは、この変更を含む版をまだリリースしていないためである。#188 が決めた
規則は「ミッション単位でマージが終わった後に版を上げる」であり、この変更自身で版を上げると
最初の適用で規則を破る。版は次のミッションのマージ後に `release` の工程で上げる。
