# cross-review: 参加する CLI が 1 者でも使えないと収束ループを開始できず、再開で渡した引数が黙って無視される → 利用可能な参加者だけで開始し、毎ラウンド 2 スロットを確保し、再開で渡した引数は反映されるか反映しないと知らされる
...
**各ラウンドに 2 つのスロットが確保される。** 利用可能な参加者が 1 者のときは、同じランタイムの 2 つ目が
スロットを埋める（#892）。0 者なら初期化が失敗する。1 スロットで回るのは、利用者が 1 者指定を渡したときだけである。

**中断した収束ループを引数を変えて再開すると、その引数は反映されるか、反映しないと知らされる。**
黙って捨てられる引数は無い。

**利用可能な参加者の決定・スロットの埋め方・再開の反映の 3 つの規則は、収束ループを回す 2 つの Skill が
...
| スロット名 | `SEAT_PATTERN` / `seat_runtime` | ランタイム名か、その名前に `-2`〜`-9` を付けた形 |
| スロットの埋め方 | `review_seats` | 利用可能な参加者とフォールバックから 2 スロットを返すライブラリの関数 |
| フォールバック | `participants.fallback` | スロットが足りないときに使う者。#892 の後に作る状態では空で、#892 の前に作った状態だけがホストを持ちうる |
| 実装担当の決め方 | `choose_implementer` | cross-refactoring の参加者・ホスト・名指しから実装担当 1 者と決め方を返すライブラリの関数 |
| 1 者指定 | `--only` | 担当を 1 者に固定する引数 |
| 全員を要する指定 | `--require-all` | 認証確認の失敗が 1 者でもあれば初期化を止める引数 |
...
| 扱う | 扱わない |
| --- | --- |
| ライブラリの利用可能な参加者の解決・認証確認・スロットの埋め方・実装担当の決め方・スロット名・再開の反映 | cross-refactoring の参加者プール・実装担当の使い方・再開の表（[cross-refactoring の参加者](cross-refactoring-participants.md)と[想定最大時間の確定仕様](cross-refactoring-time-budget.md)が持つ） |
| cross-review の新規と再開の初期化、担当の決まる順、スロット名が流れる経路、完了報告の参加者の節 | 起動した後に分かる使えなさで担当を自動的に外す仕組み |
...
**認証確認が 1 件の失敗でその場で終了すると、ゲートになる。** 4 つの CLI が
揃っていない環境では収束ループを開始できない。使える CLI が 2 者あっても、3 者目が
入っていなければ初期化ごと止まる。

**利用可能な参加者が 2 者に満たないラウンドの扱いが無かった。** 担当を決める関数は参加者プールを
「全ランタイム − ホスト」の定数から作り、利用可能な参加者を入力に取らない。そのため一部が使えない
...
| 決定 | 理由 |
| --- | --- |
| 利用可能な参加者を決める規則をライブラリの 1 つの関数へ置き、Skill は結果を状態ファイルと終了コードへ写すだけにする | 規則を Skill ごとに書くと、参加者プールの作り方・除外のチェック・確認の扱いが 2 か所にでき、片方だけが古くなる |
| 認証確認は止めずに結果だけを返す形にし、確認コマンド・未認証の文言・時間切れの秒数は変えない | 確認と中断が 1 つの関数にあると、利用可能な参加者で回す経路から呼べない。止めるかどうかは全員を要する指定が決める |
...
| スロットは 2 つとし、利用可能な参加者 → 同じランタイムの 2 つ目の順で埋める | 各ラウンドで 2 つの目で見ることを最優先にする。同じ言語モデルの 2 つの文脈より、違う言語モデルの 2 つの文脈のほうが観点が分かれる |
| cross-review の参加者プールにホストを含め、ホストを別に確かめてフォールバックに使わない（#892） | レビュー担当は CLI プロセスとして起動するため、ホストと同じランタイムでもホストの会話の作業文脈は持ち込まれない。ホストが利用可能な参加者に居ないのは確認を通らなかったか外す指定で外したかのどちらかで、前者は確かめ直しても通らず、後者でスロットへ戻すと利用者の指定に反する |
| 参加者プールの関数は引数のホストを残す | 呼び出し側の形を変えずに済み、ホストになれない名前を弾くチェックの置き場所が残る |
| 既定の参加者プールから agy を外し、claude / codex / kiro とホストにする（#786 で前の「外さない」を置き換えた） | agy はテストを背景で起動して結果を残さずに終わることがあり、起動し直しても残らなければ収束が止まる。実運用は既に毎回 `--exclude agy` を渡していた。戻すときは `--include agy`。プロンプトの指示（テストと背景の処理を起動しない）だけで直すことは採らなかった。担当が従うかを確かめる手段が無く、従わなかったときの損失（収束の停止）が大きい |
| 参加者プールに無い者の外す指定は、止めずに無視して 1 行で知らせる | 既定に agy が無くても、`--exclude agy` を書いた既存の手順が止まらずに通る。無視した名前は外した者と分けて記録する（同じ記録にすると後から参加者プールを読み違える）。判定はライブラリに置き、cross-refactoring も同じく無視する（同じ引数の意味を Skill で変えない）。綴りの誤りは引数の型が止める |
| 1 者指定で名指しした者は、参加者プールに無くても参加者にする（#786） | 名指しした時点で利用者の意図は明らかで、同じ意図を足す指定と 2 つの引数で書かせない |
| 状態ファイルに版を持たせず、ホストだけを持つ状態の再開の分岐 1 つにだけ #892 の前の参加者プール（全ランタイム − ホスト）を式で置く | 参加者の記録を持つ状態は保存された値からスロットを決めるため、関数を変えてもスロットは変わらない。版を持たせると、分岐が 1 か所で済むところへ書く側と読む側の両方に版の扱いが要る |
| 再開で担当に関わる引数を渡したときの作り直しは、保存された参加者プールではなく今の既定から解決する | 担当に関わる引数を渡すのは明示の作り直しであり、スロットを保つ互換は引数を渡さない再開に限る。保存された参加者プールを使う形はコードと規約の追記が要る |
| フォールバックの候補は、利用可能な参加者に含まれない者だけを使う | #892 の前に作った状態のフォールバック（ホスト）が利用可能な参加者に入っているとき、ホストをフォールバックにも使うと同じスロット名が 2 つ並ぶ |
| 1 者指定のときはフォールバックをしない | 利用者が 1 スロットと決めた指定である |
| 担当の単位をスロット名にし、1 つ目のスロット名はランタイム名そのままにする | 同じランタイムの 2 つ目を立てるには、結果ファイルの stem と状態ファイルの鍵を分ける名前が要る。フォールバックが要らない実行では、従来と同じ名前しか現れない |
| 接尾辞の区切りをハイフンにする | ランタイム名にハイフンを含むものが無いため、シェル側の切り出しとライブラリの解釈が同じ規則になる |
| 監視では、スロット名の形に合わない名前をそのまま返す | 担当名を任意の骨格で受ける cross-refactoring の経路がある。形で弾くとその経路が壊れる |
...
| 担当に関わる引数を渡した再開でだけ、認証確認をやり直して参加者を作り直す | 途中で担当が入れ替わると、前のラウンドの記録と突き合わせられなくなる |
| 作り直しのとき、渡さなかった引数は状態ファイルの値で補う | 初期値へ戻すと「明示した引数だけを反映する」が破れる |
| 指定を外す値は予約語 `none` にする | スケルトンは値のあるときだけ引数を渡すため、空文字列では指定を外せない |
| 参加者の作り直しは、記録へ 1 件として積む | 項目ごとに積むと 1 回の再開で最大 7 件になり、完了報告で読みにくい |
| スケルトンは 1 者指定のシェル変数で担当を絞り直さず、ラウンドの開始が返すスロットを使う | 返る一覧は 1 者指定とフォールバックを反映済みである。もう一度絞ると、状態ファイルとシェル変数がずれたときに起動も監視も誰にも当たらない |
...
| 条件 | 破れたときの扱い |
| --- | --- |
| 利用可能な参加者の並びは、ランタイムの固定の順である | 解決の中で並べ直すため、入力の順序に依らない |
| 利用可能な参加者が 3 者のときのスロットは、この変更の前の輪番と同じ値になる | 4 つのホスト × ラウンド 1〜12 の全組で一致することをテストが固定する |
| 1 ラウンドのスロットは 2 つで、同じスロット名が 2 つ並ばない | フォールバックの候補から利用可能な参加者を除いて選ぶ |
| 利用可能な参加者もフォールバックも無ければ、スロットを返さず失敗する | 割り当ての失敗（`AssignmentError`）を上げ、初期化は状態ファイルを作らずに終了コード 1 |
...
| スロット名 | スロット名からランタイムを引く。形のチェックを持つ | 同 `seat_runtime` / `SEAT_PATTERN` |
| 再開の反映 | 表に従って状態へ書き、記録へ積み、知らせる行を返す | `lib/statefile.py` の `apply_resume_args` / `ResumeField` |
| 利用可能な参加者の決定（cross-review） | ライブラリを呼び、フォールバックを空として参加者の記録を返す。利用可能な参加者が 0 者のときと失敗を終了コード 1 へ写す | `cross-review/scripts/state.py` の `_resolve_reviewers` |
| 担当の読み出し | 決まった順でスロットを返す。前のラウンドのチェックへその担当を渡す | 同 `_round_reviewers` / `_guard_previous_round` |
| スロットの受け口 | スロット名を受け、起動する CLI をスロット名から引く | `read-result` の引数の型、`launch-reviewer.sh`、`critique.sh`、`lib/monitor.py`、`measure.py` |
| 完了報告の参加者の節 | 参加した者・外した者・確認を通らなかった者・フォールバック・再開で変えた値を出す | `cross-review/scripts/state.py` の `_print_participants` |

要素の関係（辺は呼び出し）:

```mermaid
graph TD
    PL[参加者プールの既定] --> RP[利用可能な参加者の解決]
    RP --> PA[認証確認]
    RI[初期化] --> RP
    RI --> RA[再開の反映]
    RR[担当の読み出し] --> RS[スロットの埋め方]
    RC[スロットの受け口] --> SR[スロット名]
```

### 利用可能な参加者の決め方

参加者を決め、認証確認を通った者を利用可能な参加者として記録する。順序は次のとおりである。
...
| 3 | 1 者指定があれば、参加者をその 1 者にする（順 1 で参加者プールへ足し、外す者との矛盾も弾いてあるため参加者に含まれる） |
| 4 | 参加者へ認証確認を行う。確認を飛ばす環境変数（`NDF_SKIP_AUTH_CHECK`）が立っていれば全員を通ったものとし、飛ばした目印を真にする |
| 5 | 全員を要する指定が真で通らない者がいれば、欠けた者と理由を並べて失敗する |
| 6 | 通った者を利用可能な参加者、通らなかった者と理由を通らなかった者として返す |

**外した者へは確認コマンドを呼ばない。** 確認の回数は参加者の数を超えない（#892 の前にあった、
フォールバックのためのホストの 1 回は無くした）。
...
3 つ目をまとめて返すことで、どの理由でも参加者の解決は続き、利用可能な参加者だけで収束ループが
始まる。

cross-review の新規の初期化は、この結果に空のフォールバックを足して状態ファイルへ書く。

```mermaid
graph TD
    H[ホストを確定] --> P[参加者プールの既定を引く]
    P --> RP[利用可能な参加者の解決]
    RP -->|名前の矛盾 / 欠け| F[状態ファイルを作らず終了コード 1]
    RP -->|通った| N{利用可能な参加者が 1 者以上}
    N -->|はい| W[状態ファイルを書く]
    N -->|いいえ| F
```

利用可能な参加者が 1 者のときは、スロットの埋め方が同じランタイムの 2 つ目で 2 つ目のスロットを埋める。

**1 者指定のときは、この分岐へ入らない。** フォールバックを行わず、指定した 1 者が確認を通らない
ときだけ終了コード 1 で終わる。確認を通らない担当がスロットに入ると、レビューが行われないまま
収束するためである。
...
| 利用可能な参加者の数 | スロット |
| ---: | --- |
| 3 以上 | ラウンド番号を利用可能な参加者の数で割った余りの位置と、その次の位置の 2 者。並びは利用可能な参加者の順 |
| 2 | その 2 者 |
| 1 | その 1 者と、フォールバックのうち利用可能な参加者に含まれない先頭の者。そのような者が無ければ、その 1 者の 2 つ目 |
| 0 | フォールバックの先頭と、その 2 つ目。フォールバックが空なら失敗する |

**#892 の後に作る状態はフォールバックが空であるため、1 者の行は 2 つ目で埋まり、0 者の行には
初期化の段階で至らない。** フォールバックを使う行は、#892 の前に作った状態を再開したときだけ働く。
...
**担当の単位はスロット名である。** 形はランタイム名か、その名前にハイフンと 2〜9 を付けたもの
（`^(claude|codex|agy|kiro)(-[2-9])?$`）で、後者が同じランタイムの 2 つ目以降を表す。

| 受け口 | スロット名の扱い |
| --- | --- |
| 結果の受け口の担当の引数 | スロット名の形を型としてチェックする。通らなければ終了コード 2 |
| レビューの起動（`launch-reviewer.sh`）と反証の起動（`critique.sh`） | スロット名の形を先頭でチェックし、起動する CLI を `${SEAT%%-*}` で選ぶ。結果ファイルの stem はスロット名で組む |
| 監視（`lib/monitor.py`） | 位置引数はスロット名の形か `both` を通す。CLI 固有のログのチェックはスロット名からランタイムを引いて選ぶ。スロット名の形に合わない名前はそのまま返す |
| 計測（`measure.py`） | 担当の記録を数えるとき、名前の一覧ではなくスロット名の形に一致する鍵を数える |

利用可能な参加者が 1 者（`codex`）のラウンドでスロット名が流れる経路は次のとおりである。
...
| 2 | 1 者指定がある | その 1 者だけ |
| 3 | 参加者の記録がある | 利用可能な参加者とフォールバックから 2 スロット |
| 4 | ホストがある | 全ランタイム − ホストの 3 者から 2 スロット（この変更の前の輪番と同じ値）。#892 で参加者プールがホストを含む形へ変わったため、この分岐だけは変更の前の参加者プールを式で持つ |
| 5 | どれも無い | `codex` / `agy` |
...
| 反映し、参加者を作り直す | `--only` | 上に加えて、認証確認をやり直して参加者の記録を置き換える。`none` で 1 者指定を外す |
| 参加者を作り直す | `--exclude` / `--include` / `--require-all` | 利用可能な参加者を解決し直して参加者の記録を置き換える。`none` で一覧を空へ戻す |
...
**作り直しの参加者プールは、状態ファイルに保存された値ではなく今の既定である。** #892 の前に
始めたループを外す指定で作り直すと、利用可能な参加者にホストが加わる。引数を渡さない再開ではスロットは変わらない。

**作り直しの失敗は、状態ファイルを書き換える前に起きる。** 利用可能な参加者の解決を先に呼び、通って
から状態ファイルへ書く。

**外す指定を渡さない作り直しは、外した者と無視した除外の両方を記録から足し戻す**（#786。
`recorded_exclusions`）。無視した除外を落とすと、別の引数で再開しただけで完了報告から「既定の
参加者プールに無かった者」が消える。**無視した名前を足す指定か 1 者指定に渡したときだけ、その名前は
足し戻さない**（新しい指定を優先する。外す指定 agy で始めた実行を 1 者指定 agy で再開すると、
矛盾で止まらず agy 1 者で回る）。外した者と足す指定の重なりは今どおり矛盾として止める。再開で
1 者指定に `none` を渡すと、既定の参加者プールへ戻る。
...
参加者の記録を持たない状態ファイルでは「利用可能な参加者: 記録なし」の 1 行だけを出す。確認を飛ばした
...
| 項目 | 型 | 意味 |
| --- | --- | --- |
| `participants` | オブジェクト（下の表） | 利用可能な参加者の解決の結果。項目が無いのは、この変更の前に始めた実行である |
| `resume_changes` | オブジェクトの配列 | 再開で変えた値の記録。追記だけを行う。要素は `at` / `field` / `from` / `to` |

参加者の記録の中身は 9 項目である。ライブラリが 8 項目を返し、フォールバックだけを cross-review が足す。

| 項目 | 型 | 意味 |
| --- | --- | --- |
| `pool` | 文字列の配列 | 参加者プールの既定。ランタイムの固定の順 |
| `included` | 文字列の配列 | 足した者。空は「足していない」 |
| `excluded` | 文字列の配列 | 外した者。空は「外していない」 |
| `ignored_exclude` | 文字列の配列 | 外す指定をしたが参加者プールに無かったため無視した者（#786）。項目が無い状態ファイルは空として読む。外す指定を渡さない再開は、外した者とともにこの名前も足し戻す（足す指定か `--only` に渡した名前は除く） |
| `available` | 文字列の配列 | 利用可能な参加者。1 者指定があればその 1 者だけ |
| `unavailable` | オブジェクト | 確認を通らなかった者と理由 |
| `probe_skipped` | 真偽値 | 確認を飛ばしたか。通らなかった者が空である理由を区別する |
| `require_all` | 真偽値 | 全員を要する指定の値。新規の既定は偽 |
| `fallback` | 文字列の配列 | スロットのフォールバックに使う者。#892 の後に作る状態では常に空。#892 の前に作った状態では、利用可能な参加者が 2 者に満たないときにホストを持ちうる |

**ラウンドの記録の担当と鍵にはスロット名が入りうる。** 過去のラウンドの担当は再開で書き換えない。
...
| `--only RUNTIME` | 4 つの名前か `none` | 未指定 | `none` は無しと同じ。参加者プールに無い名前も参加者になる | 渡せば反映。`none` で外す |
...
**名前のチェックは 2 段階に分かれる。** 綴りは引数の型が弾き（終了コード 2）、参加者プールとの関係は
ライブラリが弾く（終了コード 1）。後者に当たるのは、足す者と外す者が重なるとき、1 者指定と外す者が
矛盾するとき、`none` と名前を混ぜたときである。参加者プールに無い名前は弾かない（#786）。外す
指定なら無視して `ℹ` の 1 行を出し、1 者指定なら参加者にする。ホストを外す指定は受け付け、ホストを除いた参加者で始まる。
ホストを足す指定は既に参加者プールにいるため結果を変えず、失敗にもしない。

### 出力と終了コード

標準出力の形は変わらない。増えるのは標準エラーの行と、スロット名が取りうる値である。

| 場面 | 標準エラー | 終了コード | 状態ファイル |
| --- | --- | ---: | --- |
| 新規で全員が使える | 参加者プールと利用可能な参加者の 1 行 | 0 | 作る |
| 新規で確認を通らない者がいる | 通らなかった者と理由を 1 者 1 行 | 0 | 作る |
| 新規で利用可能な参加者が 1 者 | 同じランタイムの 2 つ目で埋めることを 1 行 | 0 | 作る |
| 新規で利用可能な参加者が 0 者 | 利用可能な参加者がいない理由 | 1 | 作らない |
...
| スロットを埋めた | `⚠ 使える者が 1 者のため、席を同じランタイムの 2 つ目で埋めます（観点が減ります）` |
...
| 項目 | 無いときの読み方 |
| --- | --- |
| `participants` | ホストがあれば、全ランタイム − ホストの 3 者からスロットを決める（この変更の前の輪番と同じ値）。ホストも無ければ `codex` / `agy` |
...
| スロットの埋め方が 3 者で従来の値と一致し、2 / 1 / 0 者で規則どおりに埋めること。スロット名の形。cross-review の参加者プールの既定が、4 つのホストのどれでも claude / codex / kiro とホストであること（ホストが agy なら 4 者。`test_review_pool_is_the_default_three_and_the_host`） | `plugins/ndf/skills/cross-refactoring/tests/test_assignment.py` |
...
| スロット名が結果の受け口・起動・監視・計測を通ること | 同 `test_seat_names.py` |
| 前のラウンドのチェックが、そのラウンドの担当で結果を数えること | 同 `test_state_round_guard.py` |
| 手順書が 1 者指定のシェル変数で担当を絞り直さないこと | 同 `test_skill_layout.py` |
| 既定の参加者プールが 4 つのホストで claude / codex / kiro とホストになり（ホストが agy なら 4 者）、ホスト claude のスロットがラウンド 1〜3 で codex+kiro / claude+kiro / claude+codex になること。足す指定 agy で従来の輪番に戻ること（#786） | `plugins/ndf/scripts/tests/test_lib_assignment.py` |
| 参加者プールに無い者の外す指定が例外にならず、無視した除外に残って外した者と混ざらないこと。1 者指定が参加者プールに無い者を参加者にし、足した者に書かないこと。1 者指定と外す指定の同じ名前は例外のままであること | 同 `test_lib_participants.py` / `test_lib_assignment.py` |
...
- [issue #892](https://github.com/devbasex/ai-plugins/issues/892) — cross-review の参加者プールにホストを含め、ホストのフォールバックを無くす
- [issue #687](https://github.com/devbasex/ai-plugins/issues/687) — 各ラウンドで 2 者を確保する規則と置き場所
- [issue #478](https://github.com/devbasex/ai-plugins/issues/478) — 認証確認をゲートから把握へ
...
- [PR #910](https://github.com/devbasex/ai-plugins/pull/910) — 参加者プールにホストを含める実装（#892）
- [issue #786](https://github.com/devbasex/ai-plugins/issues/786) — 既定の参加者プールから agy を外し、参加者プールに無い者の外す指定を無視する
...
- [`cross-review` のレビュワーの参加者プールと終了基準](../../plugins/ndf/skills/cross-review/docs/05-pool-and-convergence.md)
