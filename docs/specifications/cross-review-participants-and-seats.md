...
| 既定の参加の母集合から agy を外し、claude / codex / kiro とホストにする（#786 で前の「外さない」を置き換えた） | agy はテストを背景で起動して結果を残さずに終わることがあり、起動し直しても残らなければ収束が止まる。実運用は既に毎回 `--exclude agy` を渡していた。戻すときは `--include agy` |
| 参加の母集合に無い者の外す指定は、止めずに無視して 1 行で知らせる（#786） | 既定から agy を外した後も、`--exclude agy` を書いた既存の手順が止まらずに通る。無視した名前は外した者と分けて記録する（同じ記録にすると後から母集合を読み違える） |
...
| 適用の輪番は「ラウンド番号を参加者の数で割った余り」の式を保つ | ラウンド 1 が 2 番目の者から始まる。参加者はランタイムの固定の順に並ぶため、既定の参加者でホストが最初に適用しないのはホストが claude / codex のときだけで、agy / kiro のときはラウンド 1 の担当がホストになる |
...
同じで、除数だけが参加者の数になる。ラウンド 1 は 2 番目の者から始まる。参加者はランタイムの
固定の順に並ぶため、既定の参加者でホストが最初に適用しないのはホストが claude / codex のとき
だけで、agy / kiro のときはラウンド 1 の担当がホストになる。呼び出す側は cross-refactoring だけで、使い方は
...
| `ignored_exclude` | 文字列の配列 | 外す指定をしたが参加の母集合に無かったため無視した者（#786）。項目が無い状態ファイルは空として読む。外す指定を渡さない再開は、外した者とともにこの名前も足し戻す（足す指定にも渡した名前は除く） |
