...
| 参加の母集合 | `review_pool(host)` / `refactor_pool(host)` | Skill ごとに決まる参加者の出発点。cross-review はホストを含む全ランタイムの 4 者（#892） |
...
| 既定の参加の母集合から agy を外さない。外すなら `--exclude agy` を渡す | agy を外すかは #786 が扱う。既定を変えると、agy が確認を通る環境の利用者の振る舞いが別の理由で変わる |
...
混ぜたときである。cross-review は参加の母集合が全ランタイムであるため（#892）、参加の母集合に
無い名前を渡す 2 つは起きない。ホストを外す指定は受け付け、ホストを除いた参加者で始まる。
