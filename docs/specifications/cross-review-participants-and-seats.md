...
スロットを埋める。0 者なら初期化が失敗する。1 スロットで回るのは、利用者が 1 者指定を渡したときだけである。
...
| cross-review の参加者プールにホストを含め、ホストを別に確かめてフォールバックに使わない | レビュー担当は CLI プロセスとして起動するため、ホストと同じランタイムでもホストの会話の作業文脈は持ち込まれない。ホストが利用可能な参加者に居ないのは確認を通らなかったか外す指定で外したかのどちらかで、前者は確かめ直しても通らず、後者でスロットへ戻すと利用者の指定に反する |
| 参加者プールの関数は引数のホストを残す | 呼び出し側の形を変えずに済み、ホストになれない名前を弾くチェックの置き場所が残る |
| 既定の参加者プールから agy を外し、claude / codex / kiro とホストにする | agy はテストを背景で起動して結果を残さずに終わることがあり、起動し直しても残らなければ収束が止まる。実運用は既に毎回 `--exclude agy` を渡していた。戻すときは `--include agy`。プロンプトの指示（テストと背景の処理を起動しない）だけで直すことは採らなかった。担当が従うかを確かめる手段が無く、従わなかったときの損失（収束の停止）が大きい |
...
| 1 者指定で名指しした者は、参加者プールに無くても参加者にする | 名指しした時点で利用者の意図は明らかで、同じ意図を足す指定と 2 つの引数で書かせない |
...
| 再開で担当に関わる引数を渡したときの作り直しは、保存された参加者プールを使わず、今の既定から解決する | 担当に関わる引数を渡すのは明示の作り直しであり、スロットを保つ互換は引数を渡さない再開に限る。保存された参加者プールを使う形はコードと規約の追記が要る |
...
| 担当の単位をスロット名にし、1 つ目のスロット名はランタイム名そのままにする | 同じランタイムの 2 つ目を立てるには、結果ファイルの stem と状態ファイルの鍵を分ける名前が要る。フォールバックが要らない実行では、ランタイム名と同じスロット名しか現れない |
...
| 計測（`measure.py`） | 担当の記録を数えるとき、スロット名の形に一致する鍵を数える（名前の一覧は使わない） |
...
| 4 | ホストがある | 全ランタイム − ホストの 3 者から 2 スロット。参加者プールはホストを含むため、この分岐だけはホストを除いた参加者プールを式で持つ |
| 5 | どれも無い | `codex` / `agy` |
...
**作り直しの参加者プールは今の既定で、状態ファイルに保存された値を使わない。** ホストを含まない参加者プールで
...
| `ignored_exclude` | 文字列の配列 | 外す指定をしたが参加者プールに無かったため無視した者。項目が無い状態ファイルは空として読む。外す指定を渡さない再開は、外した者とともにこの名前も足し戻す（足す指定か `--only` に渡した名前は除く） |
...
矛盾するとき、`none` と名前を混ぜたときである。参加者プールに無い名前は弾かない。外す
...
| 既定の参加者プールが 4 つのホストで claude / codex / kiro とホストになり（ホストが agy なら 4 者）、ホスト claude のスロットがラウンド 1〜3 で codex+kiro / claude+kiro / claude+codex になること。足す指定 agy で agy を含む 4 者の輪番になること | `plugins/ndf/scripts/tests/test_lib_assignment.py` |
...
- [PR #910](https://github.com/devbasex/ai-plugins/pull/910) — 参加者プールにホストを含める実装
