...
| 置き場所 | 長い出力と報告のコピーを書くファイルの絶対パス。supervisor が必ず渡し、worker はコピーを書き終えた後に完了の目印 `<置き場所>.done` を作る（#901。理由は [待ち方の仕様](ndf-token-waits-and-context-cut.md) の「途中の通知を受けたとき」） |
...
8 観点 × 5 つの実行方式）と、仕事ごとの選び方の表は `work-vessels.md` にある。実行方式と #845 の判断の 3 つのレベルの
対応は次のとおりである。

| 実行方式 | #845 のレベル | 主に渡す仕事 |
| --- | --- | --- |
| インライン実行 | レベル 3（その会話の LLM） | 小さな読解・1 コマンドの実行 |
| サブエージェント | レベル 3 | 読む量が大きく返す量が小さい読解（worker の `調査` / `集計`）、worktree を書き換える修正（worker の `修正`） |
| CLI 実行 | レベル 3（別の LLM） | 別の視点のレビュー・提案 |
| 最小構成の `claude -p` | レベル 2（分類の判断） | 分類の判断 1 つ。#841 が入るまでは選ばず、インライン実行にする |
| スクリプト | レベル 1 | 決まった手順（進捗記録・集計・チェックの実行・待ち）、600 秒を超える待ち（背景で起動して完了通知を待つ） |
