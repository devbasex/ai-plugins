...
| [cross-refactoring-time-budget.md](cross-refactoring-time-budget.md) | cross-refactoring を想定最大時間（`--budget-minutes`）に収める 1 回の計画実行（#933）。5 フェーズ、引数と廃止した 5 引数、実装担当の決め方、候補の切り出し、計画の取り込み、予備時間（`plan.reserve` の 4 キー）と締め切り、時間の上限の表（`state["limits"]`）と段の監視の上限、Jev の使い方と D5 の前倒し、配分テーブルと履歴、状態ファイルの版 2、報告。式と係数は `cross-refactoring` の `docs/02-plan-and-implement.md` の「締め切り」が正 |
...
| [ndf-instruction-files-check.md](ndf-instruction-files-check.md) | エージェント向け指示書の検査（`instructions-check.py`）。宣言 `.ndf/instructions.json` で決まる判定の強さ、即時読み込みと出た版の段落の判定、扱いの目印、観点の調べ直し。呼び方と宣言の書き方は `release` の `references/instruction-files.md` が正 |
