...
| [ndf-agent-layers-unattended-run.md](ndf-agent-layers-unattended-run.md) | `development-workflow` の工程を conductor / supervisor / worker の 3 層で通す運転。フェーズ 5 つ、報告の 2 層、続けさせる回数、上限（429）で中断した層の再開。手順は `development-workflow` の `references/agent-layers.md` が正 |
