...
| [cross-refactoring-verify-and-final-gate.md](cross-refactoring-verify-and-final-gate.md) | cross-refactoring のテストの追加と実装の取り込み、項目ごとの範囲テストによる検証と修正、危険の印（D1〜D5）と全体のテストの 1 回、落ちたテストだけでの見分け（揺れ・元からの失敗・変更が原因）、項目の単位の取り消し、最終ゲートと最終ゲートの修正の打ち切り（1 回目は時計で打ち切らない）、履歴へ追記する時点（#933）。手順は `cross-refactoring` の `docs/` が正 |
...
| [ndf-relay-segment-restart.md](ndf-relay-segment-restart.md) | カットポイントで claude を起動し直すラッパー（`relay.py`）。`ndf-next` のブロックを印へ写す Stop hook、擬似端末の子としての起動と素通しの条件、静止・上限・空回り・停止の印、作業ディレクトリと記録の形、ラッパーの下で文脈量の拒否を止め続けること。利用者向けの案内は `development-workflow` の `references/relay.md`、次のコマンドの形は `context-window.md` が正 |
