...
では、用語集の設定と用語集が揃うまで設計の工程へ入らない。要求の正は課題の本文に置き、`issues/` のファイルは
設計 Pull Request と一緒にコミットするコピーにする（`plugins/ndf/scripts/spec-copy.py`）。
...
| 何を読むか | 正本 |
| --- | --- |
| 用語集の設定と用語集の JSON の形・用語チェックの規則・副命令の一覧 | `plugins/ndf/skills/requirements-design/references/glossary-format.md` |
| 要求の工程の手順 0（用語集を用意する）・3a（ドメインイベント）・3b（語の突き合わせ）・7（課題の本文とコピー） | `plugins/ndf/skills/requirements-design/SKILL.md` |
| ドメインモデルの節の書き方・コンテキストマップの 5 つの関係・集約の 4 つの規則・エンティティと値オブジェクトの区別 | `plugins/ndf/skills/design/references/domain-model.md` |
| 設計の工程の手順 0（入口の検査）・2（語の突き合わせ）・5（コピーの確かめ） | `plugins/ndf/skills/design/SKILL.md` |
...
| 構成要素 | 置き場所 | 責務 |
| --- | --- | --- |
| 用語集のスクリプト | `plugins/ndf/scripts/glossary.py` | 用語集の設定と用語集の読み込み。`gate` / `init` / `candidates` / `render` / `check` / `diff` の 6 つの副命令 |
...
| 指摘の数え方（実験版） | `plugins/ndf/scripts/experimental/review-terms-count.py` | 設計 Pull Request の指摘を、語・定義と食い違いの語の並びで数える。台帳は `docs/ndf-experiments.md` |

## 用語

語の意味は用語集が正である。ai-plugins の語は [docs/glossary.md](../glossary.md) を読む
（「プロジェクトの用語集」のコンテキストに、ユビキタス言語・用語集・コンテキスト・ドメインモデルの節・
廃止した語・不変条件・未登録の語、「NDF の開発ワークフロー」のコンテキストに、用語集の設定・用語チェック・
モデルレビュー・詳細レビュー・仕様のコピーがある）。
...
**2 つは公開された言語の関係にある。** NDF のスクリプトは、プロジェクトが書いた用語集を用語集の設定が指す形式
...
| 決定 | 理由 |
| --- | --- |
| NDF の語彙とこのリポジトリの用語集は、日本のエンジニア業界でよく使う語を使う。業界に定着したカタカナ語・英語があればそれを採り、直訳の和語を語にも意味の文にも使わず、複合語は「の」でつながず業界で使う形にする。ほかのプロジェクトの用語集の語は、そのプロジェクトの領域で決める | NDF の想定利用者はエンジニアである。業界の語は説明なしで通じ、直訳の和語は読み手が意味を引き直す必要がある。業界の語が NDF の別の語とぶつかるときは複合語で分け、1 語 1 意味を保つ。ほかのプロジェクトの語は、その領域の読み手に合わせる |
| 要求の正は課題の本文にし、`issues/` のファイルは設計 Pull Request と一緒にコミットするコピーにする | 課題を読む人は GitHub の上で受け入れ条件を読める必要がある。メインディレクトリの `issues/` は push するまで誰にも見えない。コピーは設計と要求を同じ差分でレビューするために残す。置き場を設定で選ぶ形は採らない（NDF の工程は課題の本文の「進行」と盤面を前提にしている） |
| 用語集の形式は JSON だけにする | `.ndf/` のほかの設定と同じ形式で、Python の標準ライブラリで読める。YAML は依存（PyYAML）が要り、入れていないプロジェクトでチェックが動かなくなる。用語集の設定に `format` を置くため、形式は値を足せば増やせる |
...
| 入口の検査は `gate` の 1 つにし、`design` の手順 0 と設計のフェーズの先頭のステップから呼ぶ | 3 層で回すときは設計のフェーズのステップが、手で回すときは `design` の手順が必ず通る。hook はそのときのモードを知らない。`documentation` は工程表に `design` を持つが、振る舞いを変えないモードに入る |
| ai-plugins の CI は、設計のブランチ（`design/` で始まる）でだけ語の規則を見る | CI は変更のモードを知らない。`light` の Pull Request が `issues/` の文書を足しても語の規則で落とさない。ほかのプロジェクトの CI へ設定を配らない（CI の形はプロジェクトごとに違い、汎用の経路は設計のフェーズのステップが担う） |
| モデルレビューは 1 ラウンド目だけにする | 設計 Pull Request のラウンドの上限は 3 で、モデルレビューを収束まで回すと詳細レビューが回らないことがある。承認ゲートを 2 つに分けてモデルを先に承認する形は、承認ゲートの数を増やさない条件に反する |
| 語を扱う処理は `glossary.py` にまとめ、既存の検査に足さない | `instructions-check.py` と `doc-lint.py` は読む設定も規則の出所も違う。追加した行の差分の取り方だけを `doc-lint.py` と揃える |
...
| 当たった規則を止める項目を用語集の設定に持たない | 当たった規則はすべて落とす。プロジェクトが変えられるのは、どの文書を見るか（`check.paths`）と、どの表を語とみなすか（`check.term_sections`）と、正本に認める置き場（`check.source_paths`）である |
| 設計の工程で足す語は `source` を空にし、`pending_source` に設計文書を書く | 設計文書は確定前で、承認の後も確定仕様になるまで書き換わり、確定の時点で消える。正本にすると、消えた文書を指す |
| 語ごとにコードの識別子を用語集に持たせ、基本形（snake_case）を 1 つだけ持ち、ほかの書き方は機械で導く | 日本語のプロジェクトでは語と識別子が別に決まり、同じ概念が場所ごとに別の名前になる（承認ゲートが `gate` / `approval` / `checkpoint`）。書き方ごとに持つと、同じ事実を 4 か所に持ち、ずれる |
...
| --- | --- | --- | --- | --- |
| 用語集 | プロジェクトの変更（Pull Request）。NDF のスクリプトでは `init`・`render`・`spec-finalize` だけが書く | 用語集のファイル（用語集の設定の `source`） | コンテキスト（`id` で識別）・用語（コンテキストと語の組で識別） | 意味・廃止した語・正本・確定前の出所 |
| 用語集の設定 | プロジェクトの変更。`glossary.py init` が初めの 1 回を書く | `.ndf/glossary.json` | — | 形式・置き場・チェックの対象 |

人が読む文書は集約に入らない。用語集から `render` で導く生成物で、独立して変わらない。
...
| I5 | 用語集の設定 | 設計の工程を持つモード（`standard` / `legacy-refactor`）では、用語集の設定・用語集・文書が揃うまで設計の工程へ入らない | `gate` が終了コード 1 で止め、作る手順を出す |
| I6 | 用語集の設定 | 用語チェックのスクリプトは、特定のプロジェクトの語を持たない。語はすべて用語集から読む | テストが、ai-plugins の語を 1 つも含まない用語集で全規則を通す |
| I7 | 用語集 | 用語集の設定に `check.source_paths` があるとき、語の正本（URL を除く）はそれに当たるパスを指す | `check` が `unconfirmed_source` で落とす |

### ドメインイベント

| # | イベント | 発生元 | 受け手 |
| --- | --- | --- | --- |
| E1 | 用語集の設定の無いまま設計に入ろうとした | `gate` | 利用者・conductor（作る手順を読む） |
...
| E7 | モデルレビューを確定した | `cross-review` の 1 ラウンド目 | 2 ラウンド目以降のレビュー |
| E8 | 設計を確定仕様にした | `plan-to-spec-steps.py spec-finalize` | 用語集（正本を確定仕様へ移す） |
| E9 | 変更がリリースされた | `merged` | 振り返り・`issue-upkeep`（`diff` を報告に載せる） |
...
- 3 つのファイルの一部だけが無い状態は「設定なし」に含む。`init` は用語集の設定の置き場に従って欠けたものだけを作る
- 「壊れている」から「設定なし」への遷移は無い。`init` は在るファイルを読めなくても上書きしない
  （上書きすると利用者が採った語が消える）
...
| 要求の手順 7 | すべて | 要求の全文を課題の本文へ書く。`standard` では `spec-copy.py write` でコピーを作る |
...
| 設計の手順 5 | `standard` | `spec-copy.py check` が 0 であることを確かめてから設計 Pull Request を出す。1 なら `write` でコピーを作り直す（本文が正） |
...
用語集の設定の無いプロジェクトでは、`check` と `diff` は 0 を返し、振り返りと `issue-upkeep` は何も載せない。
止めるのは `gate` だけで、`gate` を打つのは設計の工程の入口と要求の工程の手順 0（止めずに `init` へ進む）
である。用語集の設定の無い既存のプロジェクトは、次に設計の工程へ入るときに用語集を作る手順へ案内される。

### 入出力の契約（`glossary.py`）

6 つの副命令の結果は、`lib/step_result.py` の形の 1 行の JSON（`tool: "glossary"`）で標準出力の最後の行に
出す。語と行の一覧は `items` に入る。`--root` の既定は `.` で、用語集の設定はそこの `.ndf/glossary.json` から読む。

| 副命令 | 入力 | 出力（`items`） | 終了コード |
| --- | --- | --- | --- |
| `gate --mode M` | モード | 止めたときは作る手順の 3 行（`init` のコマンド・候補の集め方・`requirements-design` の手順 0） | 0 = 通す（`M` が `standard` / `legacy-refactor` 以外、または設定・用語集・文書が揃い読める）。1 = 設定・用語集・文書のどれかが無い。2 = 在るが読めない |
| `init [--source P] [--document P]` | 置き場（既定 `docs/glossary/glossary.json`・`docs/glossary.md`）。設定があればその `source` / `document` を使い、引数は設定が無いときだけ使う | 作ったファイル（`{name, result: "created"}`） | 0 = 3 つのうち欠けたものを作った、またはすべてある。2 = 書けない・在る設定か用語集が読めない・置き場が境界を越える |
| `candidates [--limit N]` | 追跡しているファイル（`git ls-files`。設定の `source` / `document`・シンボリックリンク・1,000,000 バイトを超えるファイル・UTF-8 で読めないファイルを除く） | `{term, count, kind, first}`。`kind` は `table`（`.md` の `term_sections` の節の表の 1 列目）・`bold`（`.md` の 12 字以下の太字で 3 回以上。`:` / `：` で終わる札は除く）・`type`（`.md` 以外の `class` / `interface` / `struct` / `enum` / `type X =` の名前）。用語集に既にある語は除く。多い順に並べ、`--limit`（既定 100）件まで | 0。0 件なら `summary` に「スクラッチ」 |
| `render [--check]` | 用語集 | 書いた文書。`--check` は書かずに一致を見る | 0 = 書いた・一致した。1 = `--check` で一致しない。2 = 設定が無い・読めない |
| `check [--diff BASE \| --file P...] [--rules all\|structure]` | 起点の ref か、ファイル | `{rule, path, line, term, detail}` | 0 = 当たりなし（設定が無いときも 0 で、`summary` に「宣言が無い」）。1 = 当たりあり。2 = 設定・用語集・ファイルが読めない |
| `diff --base REF [--head REF]` | 2 つの版（`--head` の既定は worktree） | `{change, context, term, before, after}`。`change` は `added` / `meaning_changed` / `deprecated` / `removed` | 0（設定が無いときも 0）。片方の版に用語集が無ければ、無い側を空として比べる。`REF` そのものが無ければ 2 |

**パスはリポジトリの境界の内側に限る。** 用語集の設定の `source` / `document` / `check.paths` と `--source` /
...
| 規則 | 見るもの | 当たる条件 |
| --- | --- | --- |
| `schema` | 用語集 | `contexts[]` に `id` / `name` が無い・`id` が重なる。`terms[]` がオブジェクトでない・`term` / `context` / `meaning` が無い・`deprecated` が配列でない・`source` か `pending_source` が文字列でない・`context` が宣言されていない（I3）・1 文字の廃止した語（照合できない）・廃止した語が同じコンテキストの生きた語と重なる（I2）・`code` が英小文字の snake_case でない・`deprecated_code` が snake_case の文字列の配列でない・同じコンテキストで同じ `code` が 2 回ある・廃止した識別子が同じコンテキストの生きた識別子と重なる |
| `duplicate` | 用語集 | 同じコンテキストで同じ語が 2 回ある（I1） |
| `unconfirmed_source` | 用語集 | 用語集の設定の `check.source_paths` が空でなく、`source` が URL（`://` を含む）でも、`check.source_paths` の glob に当たるパスでもない（I7） |
...
| `deprecated_code` | コード（`.py` / `.sh` / `.js` / `.ts`）の追加した行（`--file` ならファイル全体） | 廃止した識別子か、その書き方を変えた形がコメントの外（`.py` / `.sh` の `#`、`.js` / `.ts` の `//` と `/* */` の外。文字列の中は当てる）に現れ、その形がどの生きた識別子の書き方にも無い |

- 用語の表の 1 列目は、前後の `**` / `__` / `` ` `` を外した語で照合する。節は、同じか浅い見出しが来るまで続く
- 照合から外すもの: コードブロック・インラインコード・用語集の設定の `document` の文書。廃止した語を説明のために
  書くときはインラインコードで囲む
...
### 設計のフェーズのステップ（`supervise.py`）

| ステップ | 種類 | 中身 | 遷移 |
| --- | --- | --- | --- |
| `glossary` | run | `glossary.py gate --mode <モード> --root .` | 0 → `design`。0 以外は `on_fail` を置かずに止まる（作る手順はステップの結果の `summary` に載る） |
...
| `push-glossary-gate` | run | fast だけのステップ。`push-glossary` と同じ | `gate` |
| `gate` | judge | ゲート 1。`review` と `glossary-recheck` を入力に持ち、当たりが残っていればゲート 1 の承認資料に載せる | — |

**語の直しは 1 回だけ試す。** 残った当たりは語を採るかという利用者の判断であることが多く、往復させても
減らない。`pace: fast` で当たりが残ると、`mvv` の判定へ渡さずにゲート 1 の judge へ回して利用者の承認を
求める（`mvv-gate.py` は用語チェックの結果を読まない）。

### モデルレビューと詳細レビュー（`cross-review`）

| 項目 | 形 |
| --- | --- |
| ラウンドの種類を決める関数 | `classifications.review_stage(kind, round_no, has_model)`。`design` かつ 1 ラウンド目かつ `has_model` なら `model`、ほかの `design` は `detail`、`code` は `None` |
...
| 担当へ渡す観点 | `launch-reviewer.sh` がそのラウンドの種類の文字列を追加レビュー観点へ差し込む。`review_instructions_by_stage` が無い状態ファイルは `review_instructions` を使う |
| 抜ける条件 | `model` のラウンドが収束しても抜けない（`verdict: model_confirmed`、`MODEL_CONFIRMED=1` を出して終了コード 2）。修正を挟まずに詳細レビューのラウンドへ進む。`detail` のラウンドは収束の判定で抜ける。ラウンドの上限 3 と承認ゲートの数は変わらない |

| レビュー | 見るもの | 観点 |
| --- | --- | --- |
| モデルレビュー | ドメインモデルの節と用語集の差分だけ | コンテキストをまたぐのに関係の宣言が無くないか。集約の持ち主が 1 つに決まっているか。不変条件どうしが矛盾しないか・1 行 1 条件か。ドメインイベントに受け手があり、要求の番号を引き継いでいるか。用語の表が同じ変更の用語集に反映されているか。節の外への指摘は書かない |
| 詳細レビュー | 残りの節 | 設計の観点に、用語集どおりか（用語集に無い語・廃止した語で書いていないか）・不変条件を破っていないか・コンテキストの境界を越えていないか（宣言した関係の外で、ほかのコンテキストの集約を書き換えていないか）を足す。ドメインモデルの節は確定したものとして扱う |

「確定」は、1 ラウンド目のモデルレビューが終わった時点を指す。ゲート 1 は詳細レビューの後の 1 回だけである。
...
| `check <課題> <ファイル>` | 食い違う節の見出しと差分（`items`） | 0 = 本文の各 `##` の節（`## 進行` を除く）がコピーに同じ中身である。1 = 無い節か中身の違う節がある。2 = 読めない |

コピーにしか無い節（計画など）は許す。比べるのは本文にある節だけで、行末の空白と節の後ろの空行は比べない。
...
| `check.source_paths` | 文字列の配列 | 許す | 語の正本（`terms[].source`）に認めるパスの glob。確定した文書（確定仕様・リリースした手順）だけを書く。空か無いときは正本の置き場を見ない |

`init` が書く用語集の設定は、`check.paths` に `issues/*-requirements.md`・`issues/*-design.md`・
...
ai-plugins の用語集の設定は `source` に `docs/glossary/glossary.json`、`document` に `docs/glossary.md`、
...
- 用語集の設定・用語集・文書のどれかが無いと、`gate --mode standard` / `legacy-refactor` が 1 と作る手順の 3 行を返し、
  設計のフェーズの先頭の `glossary` のステップが `on_fail` を持たないこと。`light` / `operation` / `documentation` では 0 であること
...
- `render` がコンテキストごとの見出しと 6 列の表を書き、用語集だけを変えた差分で `check` が `stale_document` で落ちること
...
- snake_case でない `code` / `deprecated_code` と、同じコンテキストの同じ `code` で `schema` が当たり、別のコンテキストの同じ `code` は当たらないこと
- コードの追加した行に廃止した識別子の 4 つの書き方のどれかが出ると `deprecated_code` が当たり、生きた識別子の内側・コメントの中・コード以外のファイル・`--rules structure` では当たらず、文字列の中（行をまたぐ文字列を含む）では当たること
...
- `diff` が 4 つの `change` を返し、用語集の設定の無いリポジトリで `check` と `diff` が 0 であること
- `review_stage` が design の 1 ラウンド目で `model`、2 以降と `has_model` が偽のときに `detail`、code で `None` を返し、モデルレビューの APPROVE で抜けないこと。担当のプロンプトにそのラウンドの種類の観点が入ること
- `spec-copy.py write` の出力が本文の `## 進行` より前と一致し、コピーにしか無い節があっても `check` が 0、本文の節の中身を変えると 1 であること
- `spec-finalize` が、消した設計を `pending_source` に持つ語の `source` を確定仕様へ移し、文書を作り直して同じコミットに入れること
- 用語チェックが ai-plugins の設計文書 1 本に対して 5 秒以内に終わること
